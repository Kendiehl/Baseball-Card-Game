...
function playSoundLocal(type) {
  try {
    ...
    if (type === 'cardFlip') {
      ...
      // Dramatic sliding sound: whoosh + dirt spray
      ...
      // Sharp mitt pop for tag out at home
      const dur = 0.15, b = c.createBuffer(1, c.sampleRate * dur, c.sampleRate), d = b.getChannelData(0);
      for (let i = 0; i < d.length; i++) { const t = i / c.sampleRate; d[i] = (Math.random() * 2 - 1) * Math.exp(-t / 0.008) * 0.55 + Math.sin(t * 500) * Math.exp(-t / 0.015) * 0.3; }
      const s = c.createBufferSource(); s.buffer = b; s.connect(c.destination); s.start(n);
    }
  } catch (e) {}
}
...
const crowdNoiseRef = { instance: null };

function createCrowdNoise(audioCtx) {
  if (!audioCtx) return null;
  const c = audioCtx;

  // Brown noise buffer (warmer than white noise — sounds like crowd murmur)
  ...
  // Bandpass filter to shape crowd-like frequencies (300–2000 Hz)
  const bandpass = c.createBiquadFilter();
  bandpass.type = 'bandpass';
  bandpass.frequency.value = 800;
  bandpass.Q.value = 0.5;

  // Main gain (base volume)
  const mainGain = c.createGain();
  mainGain.gain.value = 0.04;

  // Reaction gain (for swells/dips on events)
  const reactionGain = c.createGain();
  reactionGain.gain.value = 1.0;

  noiseSource.connect(bandpass);
  bandpass.connect(reactionGain);
  reactionGain.connect(mainGain);
  ...
  return {
    ...
    reactionGain,
    noiseSource,
    cheer(intensity = 1.0, duration = 1.5) {
      const now = c.currentTime;
      reactionGain.gain.cancelScheduledValues(now);
      reactionGain.gain.setValueAtTime(reactionGain.gain.value, now);
      reactionGain.gain.linearRampToValueAtTime(1.0 + intensity * 3.5, now + 0.15);
      reactionGain.gain.exponentialRampToValueAtTime(1.0, now + duration);
    },
    roar() {
      const now = c.currentTime;
      reactionGain.gain.cancelScheduledValues(now);
      reactionGain.gain.setValueAtTime(reactionGain.gain.value, now);
      reactionGain.gain.linearRampToValueAtTime(6.0, now + 0.2);
      reactionGain.gain.setValueAtTime(5.5, now + 1.5);
      reactionGain.gain.exponentialRampToValueAtTime(1.0, now + 4.0);
    },
    groan() {
      const now = c.currentTime;
      reactionGain.gain.cancelScheduledValues(now);
      reactionGain.gain.setValueAtTime(reactionGain.gain.value, now);
      reactionGain.gain.linearRampToValueAtTime(2.0, now + 0.1);
      reactionGain.gain.exponentialRampToValueAtTime(0.6, now + 1.2);
      reactionGain.gain.linearRampToValueAtTime(1.0, now + 2.0);
    },
    murmur() {
      const now = c.currentTime;
      reactionGain.gain.cancelScheduledValues(now);
      reactionGain.gain.setValueAtTime(reactionGain.gain.value, now);
      reactionGain.gain.linearRampToValueAtTime(1.5, now + 0.2);
      reactionGain.gain.exponentialRampToValueAtTime(1.0, now + 1.0);
    },
    tension() {
      const now = c.currentTime;
      reactionGain.gain.cancelScheduledValues(now);
      reactionGain.gain.setValueAtTime(reactionGain.gain.value, now);
      reactionGain.gain.linearRampToValueAtTime(3.0, now + 0.3);
      reactionGain.gain.setValueAtTime(2.8, now + 0.8);
      reactionGain.gain.exponentialRampToValueAtTime(1.0, now + 2.5);
    },
    setMuted(muted) {
      mainGain.gain.value = muted ? 0 : 0.04;
    },
    destroy() {
      try { noiseSource.stop(); } catch(e) {}
      try { noiseSource.disconnect(); bandpass.disconnect(); reactionGain.disconnect(); mainGain.disconnect(); } catch(e) {}
    }
  };
}

function triggerCrowdReaction(outcome) {
  const cn = crowdNoiseRef.instance;
  if (!cn) return;
  if (outcome === 'homeRun') cn.roar();
  else if (['single', 'double', 'triple'].includes(outcome)) cn.cheer(outcome === 'triple' ? 1.0 : outcome === 'double' ? 0.7 : 0.4);
  else if (outcome === 'error') cn.cheer(0.5);
  else if (['strikeout', 'doublePlay'].includes(outcome)) cn.groan();
  else if (['flyOut', 'lineOut', 'groundOut', 'foulOut'].includes(outcome)) cn.groan();
  else if (['walk', 'hbp'].includes(outcome)) cn.murmur();
  else if (outcome === 'platePlay') cn.tension();
}
...
function SVGSlidingRunner({ x, y, color, progress = 0 }) {
  // Runner in a feet-first sliding pose, with dust cloud particles
  const slideOffset = progress * 15; // slides forward
  const dustOpacity = Math.max(0, 1 - progress * 1.5);
  return (
    <g transform={`translate(${x - slideOffset},${y})`}>
      {/* Dust cloud particles */}
      {dustOpacity > 0.1 && (
        <g opacity={dustOpacity}>
          <circle cx={12} cy={8} r={4 + progress * 6} fill="#c9a25c" opacity={0.3} />
          <circle cx={18} cy={4} r={3 + progress * 5} fill="#c9a25c" opacity={0.25} />
          <circle cx={8} cy={12} r={3 + progress * 4} fill="#c9a25c" opacity={0.2} />
          <circle cx={20} cy={10} r={2 + progress * 3} fill="#c9a25c" opacity={0.15} />
        </g>
      )}
      {/* Runner body — horizontal sliding pose */}
      ...
      <line x1={6} y1={8} x2={13} y2={12} stroke={color} strokeWidth={2.2} strokeLinecap="round" />
      {/* Arms back */}
      <line x1={-6} y1={8} x2={-10} y2={4} stroke={color} strokeWidth={2} strokeLinecap="round" />
      {/* Head */}
...
function BaseballField({ bases, defColor, offColor, ballPos, pitchPhase, batterPhase, showFW, fwKey, movingRunners, movingFielder, batterSide = 'R', platePlay = null }) {
...
function BaseballCardGameInner() {
  ...
  // Sync crowd noise mute state with sound toggle
  useEffect(() => { if (crowdNoiseRef.instance) crowdNoiseRef.instance.setMuted(!soundOn); }, [soundOn]);
  useEffect(() => { return () => { if (animRef.current) cancelAnimationFrame(animRef.current); if (runnerAnimRef.current) cancelAnimationFrame(runnerAnimRef.current); if (fielderAnimRef.current) cancelAnimationFrame(fielderAnimRef.current); if (plateAnimRef.current) cancelAnimationFrame(plateAnimRef.current); if (crowdNoiseRef.instance) { crowdNoiseRef.instance.destroy(); crowdNoiseRef.instance = null; } }; }, []);
  ...
  function animatePlatePlay(dur = 1200, onComplete) {
    const s = performance.now();
    function tick(now) {
      const t = Math.min(1, (now - s) / dur);
      setPlatePlay({ progress: t });
      if (t < 1) plateAnimRef.current = requestAnimationFrame(tick);
      else { setTimeout(() => { setPlatePlay(null); if (onComplete) onComplete(); }, 400); }
    }
  ...
  function startGame() {
    ...
    // Initialize ambient crowd noise
    try {
      if (crowdNoiseRef.instance) { crowdNoiseRef.instance.destroy(); crowdNoiseRef.instance = null; }
      if (!sharedAudioCtx) sharedAudioCtx = new (window.AudioContext || window.webkitAudioContext)();
      if (sharedAudioCtx.state === 'suspended') sharedAudioCtx.resume();
      crowdNoiseRef.instance = createCrowdNoise(sharedAudioCtx);
      if (!sndRef.current) crowdNoiseRef.instance.setMuted(true);
    } catch(e) {}
  }
  ...
  function processOutcome(oc, card, nar, animated = false, result = null) {
    ...
    if (sndRef.current) triggerCrowdReaction(oc);
    ...
    // Play at the plate — ~25% chance when runner scores from 3rd on a single, error, ground out, or sac fly
    const hadRunnerOn3rd = g.bases[2] !== null;
    const isClosePlay = ['single', 'error', 'groundOut', 'flyOut', 'lineOut'].includes(oc);
    const runsScored = res.runs > 0;
    const isPlatePlay = hadRunnerOn3rd && isClosePlay && runsScored && Math.random() < 0.25;
    if (isPlatePlay) {
      const scoringRunner = g.bases[2];
      if (sndRef.current) triggerCrowdReaction('platePlay');
      animatePlatePlay(1200);
      if (sndRef.current) setTimeout(() => playSoundLocal('slideHome'), 400);
      const plateNarrations = [
        `${scoringRunner} slides home safely! SAFE at the plate!`,
        `Here comes the throw... ${scoringRunner} slides... SAFE!`,
        `${scoringRunner} barrels home! Slide... SAFE at the plate!`,
        `Close play at the plate! ${scoringRunner} slides in safely!`
      ];
      const plateText = pick(plateNarrations);
      setTimeout(() => { addLog(plateText, 'play'); setAnnounceText(plateText); }, 600);
    }
    ...
    if (nO >= 3) setTimeout(() => transitionHalf(), isPlatePlay ? 3000 : 1500);
  ...
  return (
    <div className="min-h-screen bg-stone-100 p-2 font-sans select-none overflow-x-hidden text-gray-900 text-[10px]">
      ...
      <BaseballField bases={displayBases} defColor={gs.current.half === 0 ? '#1e40af' : '#b91c1c'} offColor={gs.current.half === 0 ? '#b91c1c' : '#1e40af'} ballPos={ballPos} pitchPhase={pitchPhase} batterPhase={batterPhase} showFW={showFW} fwKey={fwKey} movingRunners={movingRunners} movingFielder={movingFielder} batterSide={batterSide} platePlay={platePlay} />
