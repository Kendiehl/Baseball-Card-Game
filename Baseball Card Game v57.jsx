...
function simulateGameInstant(team1Players, team2Players) {
  ...
  while (safety-- > 0) {
    ...
    if (bD) {
      stats[bt][batter].ab++;
      // Dropped fly ball errors (Backlog #103) charge an AB but no hit / no RBI.
      if (!simDroppedFly && ['single', 'double', 'triple', 'homeRun'].includes(oc)) { stats[bt][batter].h++; if (oc === 'homeRun') stats[bt][batter].hr++; gs.hits[bt]++; }
      if (oc === 'bunt' && res.outsAdded === 0) { stats[bt][batter].h++; gs.hits[bt]++; }
      if (!simDroppedFly && res.runs > 0) stats[bt][batter].rbi += res.runs;
...
function SVGPitcher({ x, y, color = '#1e40af', phase = 'idle', fromStretch = false }) {
  ...
  return (
    <div className="relative w-full max-w-[750px] mx-auto overflow-hidden rounded-xl shadow-2xl">
      ...
      <svg viewBox="0 0 620 500" className="w-full h-auto block bg-blue-100 font-sans">
        ...
        {!rosinAct && <SVGPitcher x={F.P.x} y={F.P.y - 14} color={defColor} phase={pitcherTipCap ? 'tipCap' : pitchPhase} fromStretch={fromStretch} />}
...
function BaseballCardGameInner() {
  ...
  function doDrawCard() {
    ...
    const isBangBang = oc === 'groundOut' && !gs.current.bases[0] && Math.random() < 0.15;
    if (isBangBang) {
      const bbTexts = [
    ...
    const isDivingStop = oc === 'groundOut' && !nar.isBangBang && !nar.isChallenge && Math.random() < 0.06;
    if (isDivingStop) {
    ...
    if (oc === 'groundOut' && !gs.current.bases[0] && gs.current.inning >= 6 && Math.random() < 0.04) {
  ...
  function processOutcome(oc, card, nar, animated = false, result = null) {
    ...
    if (bD) { S.ab++; if (!nar.isDroppedFly && ['single', 'double', 'triple', 'homeRun'].includes(oc)) { S.h++; if (oc === 'homeRun') S.hr++; g.hits[bt]++; } if (oc === 'bunt' && res.outsAdded === 0) { S.h++; g.hits[bt]++; } g.count = { balls: 0, strikes: 0 }; g.batterIdx[bt] = (g.batterIdx[bt] + 1) % 9; }
    if (res.runs > 0) {
    ...
    if (res.runs > 0 && oc !== 'error' && !nar.isDroppedFly && oc !== 'doublePlay') S.rbi += res.runs;
    if (oc === 'error' || nar.isDroppedFly) g.errors[1 - bt]++;
    // Clutch hit detection: hit with RISP and 2 outs (dropped fly errors don't count as hits)
    const isHit = (!nar.isDroppedFly && ['single', 'double', 'triple', 'homeRun'].includes(oc)) || (oc === 'bunt' && res.outsAdded === 0);
    ...
    // (dropped fly errors don't count as a "double" toward the cycle)
    if (!nar.isDroppedFly && ['single', 'double', 'triple', 'homeRun'].includes(oc)) {
      S.hitTypes.add(oc);
  ...
  function replayPlay(idx) {
    ...
    setTimeout(() => {
      ...
      else if (p.isInfieldSingle && p.outcome === 'single') { const infFk = INF_MAP[p.sub] || 'SS'; animateFielder(infFk, F[infFk], ba.target, ba.dur); }
