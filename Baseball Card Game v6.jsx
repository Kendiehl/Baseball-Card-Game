...
const INF_MAP = {
  'the shortstop': 'SS',
  'the second baseman': 'F2B',
  'the third baseman': 'F3B',
  'the first baseman': 'F1B',
  'the catcher': 'C'
};
...
function playSoundLocal(type) {
  try {
    ...
    if (type === 'cardFlip') {
      ...
      // Realistic riffle shuffle: multiple rapid card snaps with varying timing
      const totalDur = 1.8, numSnaps = 28;
      for (let i = 0; i < numSnaps; i++) {
        const snapTime = n + (i / numSnaps) * totalDur * 0.6 + (totalDur * 0.2);
        // Speed up in the middle of the riffle, slow at edges
        ...
        for (let j = 0; j < d.length; j++) {
          d[j] = (Math.random() * 2 - 1) * Math.exp(-j / (c.sampleRate * 0.003)) * snapVol;
        }
...
function SVGBatter({ x, y, color = '#dc2626', phase = 'stance', side = 'R' }) {
  ...
  const l = side === 'L', f = l ? -1 : 1;
  const isBunt = phase === 'bunt';
  const s = phase === 'swing';
  const ba = isBunt ? (l ? 5 : -5) : s ? (l ? -70 : 70) : (l ? 30 : -30);
  const batLen = isBunt ? -14 : -18;
  const batX = isBunt ? (4 * f) : (6 * f);
  const batY = isBunt ? -6 : -2;
  return (<g transform={`translate(${x},${y})`}><ellipse cx={0} cy={14} rx={5} ry={2} fill="rgba(0,0,0,.12)" /><line x1={0} y1={0} x2={0} y2={8} stroke={color} strokeWidth={2.8} strokeLinecap="round" /><line x1={0} y1={8} x2={-3 * f} y2={14} stroke={color} strokeWidth={2.2} strokeLinecap="round" /><line x1={0} y1={8} x2={3 * f} y2={14} stroke={color} strokeWidth={2.2} strokeLinecap="round" />{isBunt ? (
    <g><line x1={-3 * f} y1={-1} x2={8 * f} y2={-5} stroke={color} strokeWidth={1.8} strokeLinecap="round" /><line x1={3 * f} y1={0} x2={8 * f} y2={-5} stroke={color} strokeWidth={1.8} strokeLinecap="round" /><line x1={8 * f} y1={-5} x2={-8 * f} y2={-5} stroke="#b8860b" strokeWidth={2.5} strokeLinecap="round" /></g>
  ) : (
    <g transform={`rotate(${ba},${batX},${batY})`}><line x1={batX} y1={batY} x2={batX} y2={batLen} stroke="#b8860b" strokeWidth={2.5} strokeLinecap="round" /></g>
...
function BaseballCardGameInner() {
  ...
  function transitionHalf() {
    const g = gs.current;
    const halfTeam = teams[g.half].name;
    const runsThisHalf = g.innings[g.half][g.innings[g.half].length - 1] || 0;
    const halfLabel = g.half === 0 ? '▲ Top' : '▼ Bottom';

    // Build summary
    const summary = {
      label: `${halfLabel} ${g.inning}`,
      team: halfTeam,
      runs: runsThisHalf,
      outs: 3,
      inning: g.inning,
      awayScore: g.innings[0].reduce((a, b) => a + b, 0),
      homeScore: g.innings[1].reduce((a, b) => a + b, 0),
      awayName: teams[0].name,
      homeName: teams[1].name,
    };
    setInningSummary(summary);

    // Show summary for 2.5s, then transition
    setTimeout(() => {
  ...
  function replayPlay(idx) {
    ...
    setTimeout(() => { if (doesSwing(p.outcome)) { setBatterPhase(p.outcome === 'bunt' ? 'bunt' : 'swing'); if (sndRef.current && !['ball', 'strike', 'walk', 'hbp'].includes(p.outcome)) playSoundLocal('hit'); animateRunners(computeRunnerPaths([...p.preBases], p.outcome === 'bunt' ? 'single' : p.outcome), bt === 0 ? '#b91c1c' : '#1e40af', p.outcome === 'homeRun' ? 6500 : 2500); } }, 1050);
    setTimeout(() => {
