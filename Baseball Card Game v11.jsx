...
function calculatePlayResult(currentgs, type, batterName) {
  ...
  if (['flyOut', 'lineOut'].includes(type)) {
    // Sacrifice fly: runner on 3rd tags up and scores with fewer than 2 outs
...
// ===== 3b. AMBIENT CROWD NOISE SYSTEM =====
const crowdNoiseManager = {
  _noiseNode: null,
  _gainNode: null,
  _filterNode: null,
  _running: false,
    ...
    try {
      ...
      for (let ch = 0; ch < 2; ch++) {
        ...
        for (let i = 0; i < bufLen; i++) {
          ...
          b6 = white * 0.115926;
          d[i] *= (1 + 0.3 * Math.sin(i / ctx.sampleRate * 0.8 * Math.PI * 2));
        }
      }
      ...
      this._filterNode.Q.value = 0.5;

      this._gainNode = ctx.createGain();
      this._gainNode.gain.value = this._baseVolume;

      this._noiseNode.connect(this._filterNode);
  ...
  stop() {
    if (!this._running) return;
    try {
      if (this._noiseNode) { this._noiseNode.stop(); this._noiseNode.disconnect(); }
      if (this._filterNode) this._filterNode.disconnect();
      if (this._gainNode) this._gainNode.disconnect();
      this._noiseNode = null; this._filterNode = null; this._gainNode = null;
      this._running = false;
    } catch (e) {}
  },

  react(event) {
    if (!this._running || !this._gainNode || !sharedAudioCtx) return;
    const ctx = sharedAudioCtx, now = ctx.currentTime;
    ...
    switch (event) {
      case 'homeRun':
        g.linearRampToValueAtTime(0.35, now + 0.15);
        g.linearRampToValueAtTime(0.28, now + 2.0);
        g.linearRampToValueAtTime(this._baseVolume, now + 5.0);
        break;
      case 'hit':
        g.linearRampToValueAtTime(0.18, now + 0.1);
        g.linearRampToValueAtTime(this._baseVolume, now + 2.0);
        break;
      case 'strikeout':
        g.linearRampToValueAtTime(0.03, now + 0.1);
        g.linearRampToValueAtTime(0.12, now + 0.4);
        g.linearRampToValueAtTime(this._baseVolume, now + 1.5);
        break;
      case 'walk':
        g.linearRampToValueAtTime(0.09, now + 0.2);
        g.linearRampToValueAtTime(this._baseVolume, now + 1.5);
        break;
      case 'out':
        g.linearRampToValueAtTime(0.10, now + 0.1);
        g.linearRampToValueAtTime(this._baseVolume, now + 1.0);
        break;
      case 'error':
        g.linearRampToValueAtTime(0.02, now + 0.05);
        g.linearRampToValueAtTime(0.15, now + 0.3);
        g.linearRampToValueAtTime(this._baseVolume, now + 2.0);
        break;
      case 'platePlay':
        g.linearRampToValueAtTime(0.25, now + 0.3);
        g.linearRampToValueAtTime(this._baseVolume, now + 2.5);
        break;
      case 'gameOver':
        g.linearRampToValueAtTime(0.30, now + 0.1);
        g.linearRampToValueAtTime(0.25, now + 3.0);
        g.linearRampToValueAtTime(0.0, now + 6.0);
        break;
      default: break;
    }
  },
  ...
  setEnabled(enabled) {
    if (enabled && !this._running) this.start();
    else if (!enabled && this._running) this.stop();
  }
};
...
function SVGSlidingRunner({ x, y, color, progress = 0 }) {
  ...
  const dustOpacity = Math.max(0, 1 - progress * 1.2);
    ...
    <g transform={`translate(${x - slideOffset},${y})`}>
      {dustOpacity > 0.05 && (
        <g opacity={dustOpacity}>
          <circle cx={14} cy={8} r={5 + progress * 10} fill="#c9a25c" opacity={0.25} />
          <circle cx={20} cy={3} r={4 + progress * 8} fill="#c9a25c" opacity={0.2} />
          <circle cx={8} cy={12} r={3 + progress * 7} fill="#c9a25c" opacity={0.18} />
          <circle cx={25} cy={6} r={2 + progress * 9} fill="#d4b068" opacity={0.15} />
          <circle cx={16} cy={-2} r={2 + progress * 6} fill="#c9a25c" opacity={0.12} />
          <circle cx={12} cy={-4 - progress * 8} r={3 + progress * 4} fill="#c9a25c" opacity={0.1 * dustOpacity} />
          <circle cx={22} cy={-2 - progress * 6} r={2 + progress * 3} fill="#d4b068" opacity={0.08 * dustOpacity} />
        </g>
      )}
      <ellipse cx={0} cy={14} rx={8} ry={2} fill="rgba(0,0,0,.12)" />
      <line x1={-6} y1={8} x2={6} y2={8} stroke={color} strokeWidth={2.8} strokeLinecap="round" />
      <line x1={6} y1={8} x2={14} y2={10} stroke={color} strokeWidth={2.2} strokeLinecap="round" />
      <line x1={-6} y1={6} x2={-16} y2={10} stroke={color} strokeWidth={2} strokeLinecap="round" />
      <circle cx={-8} cy={4} r={4.5} fill="#f5d0a0" />
      <path d={`M-12.5,3 Q-8,-3 -3.5,3`} fill={color} />
      {progress > 0.3 && (
        <g transform={`translate(${-24 + collisionPhase * 4},${-6})`} opacity={Math.min(1, (progress - 0.3) * 3)}>
          <line x1={0} y1={4} x2={0} y2={10} stroke="#1e3a5f" strokeWidth={2.8} strokeLinecap="round" />
          <line x1={0} y1={10} x2={-4} y2={14} stroke="#1e3a5f" strokeWidth={2.2} strokeLinecap="round" />
          <line x1={0} y1={10} x2={4} y2={14} stroke="#1e3a5f" strokeWidth={2.2} strokeLinecap="round" />
          <circle cx={7} cy={2} r={4} fill="#8B4513" stroke="#5a3010" strokeWidth={0.6} />
          <circle cx={0} cy={0} r={4.5} fill="#f5d0a0" />
          <path d={`M-4.5,-1 Q0,-7 4.5,-1`} fill="#1e3a5f" />
          <rect x={-3} y={-1} width={6} height={4} rx={1} fill="none" stroke="#555" strokeWidth={0.6} opacity={0.5} />
        </g>
      )}
      {collisionPhase > 0 && collisionPhase < 0.5 && (
        <g opacity={(0.5 - collisionPhase) * 2}>
          <circle cx={-14} cy={6} r={8 + collisionPhase * 12} fill="#fff" opacity={0.3} />
        </g>
      )}
      {safeTextOpacity > 0 && (
        <g opacity={safeTextOpacity}>
          <rect x={-28} y={-28} width={40} height={16} rx={3} fill="#16a34a" />
          <text x={-8} y={-18} textAnchor="middle" dominantBaseline="middle" fill="#fff" fontSize="10" fontWeight="bold" fontFamily="sans-serif">SAFE!</text>
        </g>
      )}
    </g>
  );
        ...
        return (
          <g key={`bunting-${i}`} transform={`translate(${pos.x},${pos.y})`}>
            {buntingColors.map((color, ci) => {
              ...
              return (
                <path key={ci} d={`M${offsetX - w * 0.17},0 Q${offsetX},${h * 0.9} ${offsetX + w * 0.17},0`} fill={color} opacity={0.85} stroke={color === '#ffffff' ? '#ddd' : color} strokeWidth={0.5} />
              );
            })}
            <line x1={-w * 0.5} y1={0} x2={w * 0.5} y2={0} stroke="#8B7355" strokeWidth={1.5 * s} />
            <circle cx={-w * 0.33} cy={0} r={1.5 * s} fill="#fbbf24" /><circle cx={0} cy={0} r={1.5 * s} fill="#fbbf24" /><circle cx={w * 0.33} cy={0} r={1.5 * s} fill="#fbbf24" />
          </g>
        );
      })}
      <g>
        <rect x={235} y={42} width={150} height={18} rx={3} fill="#1e3a5f" opacity={0.9} />
        <text x={310} y={54} textAnchor="middle" dominantBaseline="middle" fill="#ffd700" fontSize="9" fontWeight="bold" fontFamily="sans-serif" letterSpacing="3">POSTSEASON</text>
      </g>
    </g>
  );
}

function BaseballFieldContainer({ bases, defColor, offColor, ballPos, pitchPhase, batterPhase, showFW, fwKey, movingRunners, movingFielder, batterSide = 'R', platePlay = null, isPlayoff = false }) {
...
function BaseballCardGameInner() {
  ...
  useEffect(() => { sndRef.current = soundOn; spdRef.current = autoSpeed; if (phase === 'playing') crowdNoiseManager.setEnabled(soundOn); }, [soundOn, autoSpeed, phase]);
  useEffect(() => { return () => { if (animRef.current) cancelAnimationFrame(animRef.current); if (runnerAnimRef.current) cancelAnimationFrame(runnerAnimRef.current); if (fielderAnimRef.current) cancelAnimationFrame(fielderAnimRef.current); if (plateAnimRef.current) cancelAnimationFrame(plateAnimRef.current); crowdNoiseManager.stop(); }; }, []);
  ...
  function startGame() {
    ...
    setPlayHistory([]); setReplayIdx(-1); setHalfStartIdx(0); setDrawnPile([]); setDispensedCard(null); setDispensing(false); setOutcome(null); setShowFW(false); setAutoPlay(false); setBallPos(null); setPitchPhase('idle'); setBatterPhase('stance'); setAnnounceText(''); setPlatePlay(null); setActiveBatter(teams[0].players[0]); shuffleDeck(); setPhase('playing'); rerender();
    if (sndRef.current) crowdNoiseManager.start();
  }
      ...
      animateBall(linePts(F.P, { x: F.HP.x, y: F.HP.y - 5 }, 16), 550, () => {
        if (!doesSwing(oc)) {
          if (stolenBaseEvent) {
            ...
            if (sbe.success) {
              if (sndRef.current) { setTimeout(() => playSoundLocal('stolenBase'), 600); setTimeout(() => crowdNoiseManager.react('stolenBase'), 700); }
              setTimeout(() => {
  ...
  function processOutcome(oc, card, nar, animated = false, result = null) {
    ...
    if (sndRef.current) {
      if (oc === 'homeRun') crowdNoiseManager.react('homeRun');
      else if (['single', 'double', 'triple'].includes(oc)) crowdNoiseManager.react('hit');
      else if (oc === 'strikeout' || (oc === 'strike' && g.count.strikes >= 3)) crowdNoiseManager.react('strikeout');
      else if (oc === 'error') crowdNoiseManager.react('error');
      else if (['walk', 'hbp'].includes(oc)) crowdNoiseManager.react('walk');
      else if (['flyOut', 'lineOut', 'groundOut', 'foulOut', 'doublePlay'].includes(oc)) crowdNoiseManager.react('out');
    }
    ...
    if (isClosePlay) {
      ...
      if (sndRef.current) crowdNoiseManager.react('platePlay');
      animatePlatePlay(1800); 
      if (sndRef.current) setTimeout(() => playSoundLocal('slideHome'), 600);
      const plateText = pick([
        `${scoringRunner} comes barreling home! The throw is there... SAFE! What a play at the plate!`,
        `Here comes the throw... ${scoringRunner} slides headfirst... collision at the plate... SAFE!`,
        `Close play at the plate! ${scoringRunner} slides in under the tag! SAFE!`,
        `The catcher blocks the plate but ${scoringRunner} slides around the tag! SAFE!`,
        `${scoringRunner} barrels into home! The umpire signals... SAFE! The crowd goes wild!`
      ]);
    ...
    if (g.gameOver) { if (sndRef.current) crowdNoiseManager.react('gameOver'); procRef.current = false; rerender(); return; }
    if (nO >= 3) setTimeout(() => transitionHalf(), isClosePlay ? 3500 : 1500);
  ...
  if (phase === 'setup') {
    return (
      <div className="min-h-screen bg-gray-100 flex flex-col items-center p-10 font-sans text-gray-900">
        <h1 className="text-4xl font-black text-blue-900 mb-2 tracking-tight uppercase">⚾ Card Baseball v11</h1>
        ...
        <label className="mt-8 flex items-center gap-3 cursor-pointer select-none bg-white px-6 py-3 rounded-xl shadow-md border border-gray-200 hover:border-blue-300 transition-colors">
          ...
          <span className="text-xs text-gray-400 italic">Adds bunting &amp; banners to the stadium</span>
        </label>
        <button onClick={startGame} className="mt-6 px-16 py-5 bg-green-600 hover:bg-green-700 text-white font-black text-2xl rounded-full shadow-2xl active:scale-95 transition-all cursor-pointer">START SIMULATION</button>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-stone-100 p-2 font-sans select-none overflow-x-hidden text-gray-900 text-[10px]">
      ...
      <BaseballFieldContainer bases={displayBases} defColor={gs.current.half === 0 ? '#1e40af' : '#b91c1c'} offColor={gs.current.half === 0 ? '#b91c1c' : '#1e40af'} ballPos={ballPos} pitchPhase={pitchPhase} batterPhase={batterPhase} showFW={showFW} fwKey={fwKey} movingRunners={movingRunners} movingFielder={movingFielder} batterSide={batterSide} platePlay={platePlay} isPlayoff={isPlayoff} />
      ...
      <div className="max-w-[750px] mx-auto flex justify-center items-center gap-4 mt-6">
        ...
        <div className="flex bg-white rounded-lg border shadow-sm overflow-hidden h-10 text-gray-900">
          ...
          <button disabled={replayIdx === -1 || replayIdx >= playHistory.length - 1 || procRef.current} onClick={() => replayPlay(replayIdx + 1)} className="px-4 hover:bg-gray-50 disabled:opacity-20 cursor-pointer">►</button>
        </div>
        ...
        <button onClick={() => setShowCardLibrary(true)} className="h-10 px-4 bg-white rounded-lg border shadow-sm text-blue-600 font-bold hover:bg-blue-50 cursor-pointer uppercase text-gray-900">🃏 Cards</button>
        <button onClick={() => setConfirmReset(true)} className="h-10 px-4 bg-white rounded-lg border shadow-sm text-gray-400 font-bold hover:text-red-500 cursor-pointer uppercase text-gray-900">Reset</button>
      </div>
            ...
            <div className="overflow-y-auto p-4">
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4 text-gray-900">
                {SUITS.map(suit => {
                  ...
                  return (
                    <div key={suit} className={`rounded-xl border ${suitBg} p-3`}>
      ...
      <div className="max-w-[750px] mx-auto mt-6 flex gap-4 h-52 overflow-hidden items-stretch text-gray-900">
        <div ref={logRef} className="flex-1 bg-white p-3 rounded-xl shadow-inner overflow-y-auto"><h3 className="text-[10px] font-bold text-gray-400 uppercase mb-2 tracking-widest text-gray-900">Play-by-Play</h3>{log.slice().reverse().map((l, i) => <div key={i} className={`text-xs mb-1 ${l.type === 'homer' ? 'text-orange-600 font-bold' : 'text-gray-700'}`}>{l.text}</div>)}</div>
