...
const TEAM_COLORS = ['#b91c1c', '#1e40af'];
const HIT_OUTCOMES = new Set(['single', 'double', 'triple', 'error']);
const GROUND_OUTCOMES = new Set(['groundOut', 'doublePlay', 'bunt']);

const CARD_MAP = {
  'A♣':'walk','2♣':'groundOut','3♣':'ball','4♣':'foulBall','5♣':'flyOut','6♣':'groundOut','7♣':'strikeout','8♣':'bunt','9♣':'single','10♣':'strike','J♣':'groundOut','Q♣':'foulOut','K♣':'ball',
  'A♦':'homeRun','2♦':'double','3♦':'flyOut','4♦':'ball','5♦':'strike','6♦':'double','7♦':'strikeout','8♦':'single','9♦':'groundOut','10♦':'lineOut','J♦':'ball','Q♦':'hbp','K♦':'doublePlay',
  'A♥':'lineOut','2♥':'flyOut','3♥':'single','4♥':'single','5♥':'single','6♥':'flyOut','7♥':'strikeout','8♥':'flyOut','9♥':'single','10♥':'homeRun','J♥':'flyOut','Q♥':'strike','K♥':'double',
  'A♠':'error','2♠':'foulOut','3♠':'triple','4♠':'groundOut','5♠':'groundOut','6♠':'strike','7♠':'doublePlay','8♠':'ball','9♠':'foulBall','10♠':'flyOut','J♠':'single','Q♠':'walk','K♠':'lineOut'
};

const F = {
  HP:{x:310,y:430}, B1:{x:420,y:325}, B2:{x:310,y:225}, B3:{x:200,y:325}, MOUND:{x:310,y:340},
  C:{x:310,y:455}, UMP:{x:310,y:468}, P:{x:310,y:335}, F1B:{x:405,y:300}, F2B:{x:360,y:248}, SS:{x:260,y:248}, F3B:{x:215,y:300}, LF:{x:180,y:190}, CF:{x:310,y:140}, RF:{x:440,y:190},  
};
const FENCE_Y = 110; 

// ===== 2. UTILITY & LOGIC HELPERS =====

function INIT_GS() { return { inning: 1, half: 0, outs: 0, bases: [null, null, null], count: { balls: 0, strikes: 0 }, batterIdx: [0, 0], innings: [[0], [0]], hits: [0, 0], errors: [0, 0], gameOver: false, message: null }; }
function INIT_STATS(teamsData) { const s = [{}, {}]; for (let t = 0; t < 2; t++) { for (const p of teamsData[t].players) { s[t][p] = { ab: 0, h: 0, hr: 0 }; } } return s; }
...
function doesSwing(oc) { return ['single', 'double', 'triple', 'homeRun', 'error', 'groundOut', 'flyOut', 'lineOut', 'foulOut', 'foulBall', 'doublePlay', 'strikeout', 'bunt'].includes(oc); }

function calculatePlayResult(currentgs, type, batterName) {
  let nextBases = [null, null, null], runs = 0, outsAdded = 0, isSacFly = false;
  const bases = currentgs.bases;
  if (['walk', 'hbp'].includes(type) || (type === 'ball' && currentgs.count.balls === 3)) { const res = resolveWalkForce(bases, batterName); return { nextBases: res.bases, runs: res.runs, outsAdded: 0 }; }
  if (['single', 'double', 'triple', 'homeRun', 'error'].includes(type)) { const res = advanceRunners(bases, type, batterName); return { nextBases: res.bases, runs: res.runs, outsAdded: 0 }; }
  if (type === 'bunt') {
    let nb = [null, null, null], r = 0;
    if (bases[2]) r++; if (bases[1]) nb[2] = bases[1]; if (bases[0]) nb[1] = bases[0];
    if (Math.random() < 0.25) { nb[0] = batterName; return { nextBases: nb, runs: r, outsAdded: 0 }; }
    return { nextBases: nb, runs: r, outsAdded: 1 };
  }
  if (type === 'groundOut') {
    if (bases[0]) { nextBases[0] = batterName; if (bases[1] && !bases[2]) nextBases[2] = bases[1]; else if (bases[1] && bases[2]) { nextBases[2] = bases[2]; } return { nextBases, runs: 0, outsAdded: 1 }; }
    else { if (currentgs.outs < 2 && bases[2]) runs = 1; nextBases[1] = bases[1]; nextBases[2] = (currentgs.outs < 2 && bases[2]) ? null : bases[2]; return { nextBases, runs, outsAdded: 1 }; }
  }
  if (type === 'doublePlay') { if (bases[0]) { nextBases[2] = bases[2]; return { nextBases, runs: 0, outsAdded: 2 }; } else return { nextBases: [null, bases[1], bases[2]], runs: 0, outsAdded: 1 }; }
  if (['flyOut', 'lineOut', 'foulOut', 'strikeout'].includes(type)) {
    if (['flyOut', 'lineOut'].includes(type) && bases[2] && currentgs.outs < 2) { return { nextBases: [bases[0], bases[1], null], runs: 1, outsAdded: 1, isSacFly: true }; }
...
function resolveWalkForce(b, batterName) {
  let n = [...b], r = 0;
  if (n[0]) { if (n[1]) { if (n[2]) r++; n[2] = n[1]; } n[1] = n[0]; }
  n[0] = batterName; return { bases: n, runs: r };
}
...
function advanceRunners(bases, type, batterName) {
  ...
  if (type === 'single' || type === 'error') { for (const r of rs) { if (r.pos + 1 > 2) runs++; else nb[r.pos + 1] = r.name; } nb[0] = batterName; }
  else if (type === 'double') { for (const r of rs) { if (r.pos + 2 > 2) runs++; else nb[r.pos + 2] = r.name; } nb[1] = batterName; }
  else if (type === 'triple') { runs += rs.length; nb[2] = batterName; }
  else if (type === 'homeRun') { runs += rs.length + 1; }
  return { bases: nb, runs };
}

function narrateLocal(oc, bat, bases, rs, outs) {
  const sc = rs > 0 ? (rs === 1 ? ` 1 run scores!` : ` ${rs} runs score!`) : '';
  let dir = 'cf', sub = '', variant = Math.floor(Math.random() * 3);
  const rd = pick(['lf', 'cf', 'rf']);
  const selectedInf = pick(Object.keys(INF_MAP).filter(k => k !== 'the catcher'));
  switch (oc) {
    case 'single': return { text: `${bat} hits a single ${DIRS[rd][variant]}.${sc}`, dir: rd, sub: rd, variant };
    case 'double': return { text: `${bat} drives a double ${DIRS[rd][variant]}!${sc}`, dir: rd, sub: rd, variant };
    case 'triple': return { text: `${bat} smashes a triple ${pick(['lf', 'rf'])}${DIRS[rd][variant].replace('center','')}!${sc}`, dir: pick(['lf','rf']), sub: pick(['lf','rf']), variant };
    case 'homeRun': return { text: rs > 1 ? `${bat} crushes a ${rs}-run HOMER ${DIRS[rd][variant]}!` : `${bat} launches a solo HOMER ${DIRS[rd][variant]}!`, dir: rd, sub: rd, variant };
    ...
    case 'error': return { text: `Error! ${bat} reaches first.${sc}`, dir: rd, sub: rd, variant };
    case 'groundOut': return { text: `${bat} grounds out to ${selectedInf}.${sc}`, dir: 'inf', sub: selectedInf, variant };
    case 'flyOut': { const sac = bases[2] && outs < 2; return { text: sac ? `${bat} hits a sacrifice fly ${DIRS[rd][variant]}. Runner tags and scores!` : `${bat} flies out ${DIRS[rd][variant]}.`, dir: rd, sub: rd, variant }; }
    case 'lineOut': return { text: `${bat} lines out to ${selectedInf}.`, dir: 'inf', sub: selectedInf, variant };
    case 'foulOut': return { text: `${bat} pops out in foul territory.`, dir: 'foul', sub: 'the catcher', variant: 0 };
    case 'strikeout': return { text: `${bat} strikes out.`, dir: 'none', sub: '', variant: 0 };
    case 'bunt': return { text: `${bat} drops a bunt toward ${selectedInf}.${sc}`, dir: 'inf', sub: selectedInf, variant };
...
function getBallTgtLocal(dir, oc, sub, variant) {
  const hp = F.HP, jitter = () => (Math.random() - 0.5) * 6;
  ...
  let fp = { ...F[OF_MAP[sub] || 'CF'] }, dm = variant === 1 ? 0.82 : variant === 2 ? 1.25 : 1.0;
  if (oc === 'single' || oc === 'error') { const t = Math.max(0.78, Math.min(0.85 * dm, 0.95)); fp.x = lerp(hp.x, fp.x, t) + jitter(); fp.y = lerp(hp.y, fp.y, t) + jitter(); }
  else if (oc === 'double') { const gap = (sub === 'cf') ? (Math.random() > 0.5 ? 55 : -55) : (sub === 'lf' ? 30 : -30); fp.x = fp.x + gap + (fp.x - hp.x) * 0.15 + jitter(); fp.y = lerp(fp.y, FENCE_Y, 0.75 * Math.max(dm, 1.15)) + jitter(); }
  else if (oc === 'triple') { fp = { x: (sub === 'rf' ? (F.RF.x+F.CF.x)/2 : (F.LF.x+F.CF.x)/2), y: FENCE_Y + 10 }; fp.x += jitter()*2; }
  else if (oc === 'homeRun') { const dx = fp.x - hp.x; fp.x = hp.x + dx * 2.8; fp.y = -180; }
  else if (['flyOut', 'lineOut'].includes(oc)) { fp.y = lerp(fp.y, FENCE_Y, variant === 2 ? 0.4 : variant === 1 ? -0.2 : 0); }
...
function genBallAnimLocal(oc, dir, sub, variant) {
  ...
  if (['double', 'triple'].includes(oc)) {
    ...
    if (hf) { const ft = { x: t.x, y: FENCE_Y + 2 }, bt = { x: t.x + (Math.random() - 0.5) * 12, y: FENCE_Y + 18 }; return { path: [...arcPts(s, ft, arcH, 15), ...linePts(ft, bt, 6)], dur: 2200, target: bt, carom: true }; }
  }
...
function computeRunnerPaths(oldBases, outcomeType) {
  ...
  if (['single', 'error'].includes(outcomeType)) { if (oldBases[2]) ps.push([b3, hp]); if (oldBases[1]) ps.push([b2, b3]); if (oldBases[0]) ps.push([b1, b2]); ps.push([hp, b1]); }
  else if (outcomeType === 'double') { if (oldBases[2]) ps.push([b3, hp]); if (oldBases[1]) ps.push([b2, b3, hp]); if (oldBases[0]) ps.push([b1, b2, b3]); ps.push([hp, b1, b2]); }
  else if (outcomeType === 'triple') { if (oldBases[2]) ps.push([b3, hp]); if (oldBases[1]) ps.push([b2, b3, hp]); if (oldBases[0]) ps.push([b1, b2, b3, hp]); ps.push([hp, b1, b2, b3]); }
  else if (outcomeType === 'homeRun') { if (oldBases[2]) ps.push([b3, hp]); if (oldBases[1]) ps.push([b2, b3, hp]); if (oldBases[0]) ps.push([b1, b2, b3, hp]); ps.push([hp, b1, b2, b3, hp]); }
  else if (outcomeType === 'sacFly') { if (oldBases[2]) ps.push([b3, hp]); }
  else if (['walk', 'hbp'].includes(outcomeType)) { if (oldBases[0]) { if (oldBases[1]) { if (oldBases[2]) ps.push([b3, hp]); ps.push([b2, b3]); } ps.push([b1, b2]); } ps.push([hp, b1]); }
  else if (['groundOut', 'doublePlay'].includes(outcomeType)) { if (oldBases[2]) ps.push([b3, hp]); if (oldBases[1]) ps.push([b2, b3]); if (oldBases[0]) ps.push([b1, b2]); ps.push([hp, b1]); } 
  return ps;
}
...
function playSoundLocal(type) {
  try {
    ...
    if (type === 'cardFlip') { const b = c.createBuffer(1, c.sampleRate * .06, c.sampleRate), d = b.getChannelData(0); for (let i = 0; i < d.length; i++) d[i] = (Math.random() * 2 - 1) * Math.exp(-i / (c.sampleRate * .008)) * .3; const s = c.createBufferSource(); s.buffer = b; s.connect(c.destination); s.start(n); }
    else if (type === 'hit') { const b = c.createBuffer(1, c.sampleRate * .2, c.sampleRate), d = b.getChannelData(0); for (let i = 0; i < d.length; i++) { const t = i / c.sampleRate; d[i] = (Math.random() * 2 - 1) * Math.exp(-t / .02) * .5 + Math.sin(t * 800) * Math.exp(-t / .02) * .3; } const s = c.createBufferSource(); s.buffer = b; s.connect(c.destination); s.start(n); }
  } catch (e) {}
}
...
function CardAreaComp({ drawnPile, dispensedCard, dispensing, onDraw, canDraw, autoPlay, gameOver, shuffling, shuffleCards }) {
  return (
    <div className="flex items-start gap-8 justify-center flex-wrap p-2 relative font-sans min-h-[140px]">
      <style>{`
        @keyframes riffleLeft { 0% { transform: translateX(-80px) rotate(-20deg) scale(0.9); opacity: 0; } 15% { transform: translateX(-50px) rotate(-15deg) scale(1); opacity: 1; } 60% { transform: translateX(0px) rotate(0deg); } 100% { transform: translateX(0px) rotate(0deg); opacity: 1; } }
        @keyframes riffleRight { 0% { transform: translateX(80px) rotate(20deg) scale(0.9); opacity: 0; } 15% { transform: translateX(50px) rotate(15deg) scale(1); opacity: 1; } 60% { transform: translateX(0px) rotate(0deg); } 100% { transform: translateX(0px) rotate(0deg); opacity: 1; } }
      `}</style>
      {shuffling && (
        <div className="absolute inset-0 z-50 flex items-center justify-center bg-gray-100/95 rounded-xl shadow-inner text-blue-900">
          <div className="flex flex-col items-center gap-2 text-center">
            <div className="text-sm uppercase font-black text-gray-400 mb-4">Deck Shuffle</div>
            <div className="relative w-48 h-32 flex justify-center items-center">
              {shuffleCards.map((card, i) => (<div key={i} className="absolute" style={{ zIndex: i, animation: `${i % 2 === 0 ? 'riffleLeft' : 'riffleRight'} 1.2s cubic-bezier(0.45, 0, 0.55, 1) forwards`, animationDelay: `${(i * 0.04)}s` }}><MiniCard card={card} style={{ width: 45, height: 63, fontSize: 8 }} /></div>))}
            </div>
          </div>
        </div>
      )}
      <div className="flex flex-col items-center flex-shrink-0">
        <div className="relative w-28 h-44 bg-gradient-to-b from-gray-600 to-gray-800 rounded-t-2xl rounded-b-lg border-x-4 border-t-4 border-gray-400 shadow-2xl flex flex-col items-center overflow-hidden">
          <div className="bg-black/40 w-full text-center py-1 border-b border-gray-500 mb-2"><span className="text-[9px] text-yellow-500 font-bold uppercase tracking-tighter">Shuffler</span></div>
          <button disabled={!canDraw} onClick={onDraw} className={`mt-1 w-20 py-3 rounded-lg border-b-4 transition-all active:translate-y-1 active:border-b-0 ${canDraw ? 'bg-red-600 border-red-800 shadow-red-950/40 cursor-pointer' : 'bg-gray-700 opacity-50 cursor-not-allowed'}`}>
            <div className={`text-[10px] text-white font-black ${canDraw && !autoPlay ? 'animate-pulse' : ''}`}>{autoPlay ? 'PAUSE' : gameOver ? 'OVER' : '⚾ DRAW'}</div>
          </button>
          <div className="mt-8 w-24 h-1 bg-black rounded-full shadow-[0_-2px_4px_rgba(255,255,255,0.2)]" />
          {dispensedCard && ( <div className={`absolute left-4 z-10 transition-all duration-500 ${dispensing ? 'top-24 scale-100' : 'top-16 scale-75 opacity-0'}`}><MiniCard card={dispensedCard} /></div> )}
        </div>
      </div>
      <div className="min-w-[90px] pt-4">
        <div className="text-[10px] text-gray-400 font-bold uppercase mb-2 tracking-widest text-center text-gray-900">Inning Stack ({drawnPile.length})</div>
        <div className="relative w-16 h-24 mx-auto">
            {drawnPile.slice(-5).map((card, i) => (<div key={i} className="absolute" style={{ top: Math.min(i * 0.6, 12), left: (i % 3) * 0.8, zIndex: i }}><MiniCard card={card} style={{ width: 50, height: 70, fontSize: 10 }} /></div>))}
        </div>
      </div>
    </div>
  );
...
function SVGBatter({ x, y, color = '#dc2626', phase = 'stance', side = 'R' }) {
  ...
  const l = side === 'L', f = l ? -1 : 1, s = phase === 'swing', ba = s ? (l ? -70 : 70) : (l ? 30 : -30);
  return (<g transform={`translate(${x},${y})`}><ellipse cx={0} cy={14} rx={5} ry={2} fill="rgba(0,0,0,.12)" /><line x1={0} y1={0} x2={0} y2={8} stroke={color} strokeWidth={2.8} strokeLinecap="round" /><line x1={0} y1={8} x2={-3 * f} y2={14} stroke={color} strokeWidth={2.2} strokeLinecap="round" /><line x1={0} y1={8} x2={3 * f} y2={14} stroke={color} strokeWidth={2.2} strokeLinecap="round" /><g transform={`rotate(${ba},${6 * f},-2)`}><line x1={6 * f} y1={-2} x2={6 * f} y2={-18} stroke="#b8860b" strokeWidth={2.5} strokeLinecap="round" /></g><circle cx={0} cy={-5} r={5} fill="#f5d0a0" /><path d={`M${-5 * f},-6 Q0,-13 ${5 * f},-6`} fill={color} /></g>);
}
function SVGCatcher({ x, y, color = '#1e40af', isMoving = false }) {
  const b = isMoving ? Math.sin(Date.now() / 50) * 2 : 0;
  return (<g transform={`translate(${x},${y + b})`}><ellipse cx={0} cy={8} rx={6} ry={2} fill="rgba(0,0,0,.1)" /><line x1={0} y1={-4} x2={0} y2={2} stroke={color} strokeWidth={3} strokeLinecap="round" /><line x1={0} y1={2} x2={-5} y2={0} stroke={color} strokeWidth={2.2} strokeLinecap="round" /><line x1={-5} y1={0} x2={-5} y2={7} stroke={color} strokeWidth={2.2} strokeLinecap="round" /><line x1={0} y1={2} x2={5} y2={0} stroke={color} strokeWidth={2.2} strokeLinecap="round" /><line x1={5} y1={0} x2={5} y2={7} stroke={color} strokeWidth={2.2} strokeLinecap="round" /><circle cx={9.5} cy={-3.5} r={4.5} fill="#8B4513" stroke="#5a3010" strokeWidth={.6} /><circle cx={0} cy={-9} r={5} fill="#f5d0a0" /><path d="M-5,-10 Q0,-17 5,-10" fill={color} /><rect x={-3.5} y={-8} width={7} height={5} rx={1} fill="none" stroke="#555" strokeWidth={.7} opacity={0.5}/></g>);
}
...
function BaseballField({ bases, defColor, offColor, ballPos, pitchPhase, batterPhase, showFW, fwKey, movingRunners, movingFielder, batterSide = 'R' }) {
  ...
  const staticField = useMemo(() => {
    ...
    const aL = Math.atan2(flx - 310, 430 - fl.y), aR = Math.atan2(frx - 310, 430 - fry);
    ...
    return (
      <>
        ...
        <g clipPath="url(#fc1)" opacity="0.05"> {Array.from({length:22},(_,i)=><rect key={i} x={0} y={50+i*22} width={620} height={11} fill="#fff"/>)} </g>
        ...
        <path d={dp} fill="#c9a25c" opacity="0.75" />
        <path d={`M${flx} ${fl.y-7} A${fc.rx} ${fc.ry} 0 0 1 ${frx} ${fry-7}`} fill="none" stroke="#d4a800" strokeWidth={2.5} opacity="0.9"/>
        {marks.map((m,i)=>{
          const a = aL + (aR - aL) * m.frac, px = 310 + 330 * Math.sin(a), py = 430 - 370 * Math.cos(a);
          return (<g key={i}><rect x={px-12} y={py-7} width={24} height={13} rx={2} fill="#0d3d08" opacity="0.9"/><text x={px} y={py+0.5} textAnchor="middle" dominantBaseline="middle" fill="#ffd700" fontSize="8" fontWeight="bold" fontFamily="sans-serif">{m.label}</text></g>);
        })}
        ...
        <line x1={hp.x} y1={hp.y} x2={flx} y2={fl.y} stroke="#fff" strokeWidth={1.8} opacity="0.5" /><line x1={hp.x} y1={hp.y} x2={frx} y2={fry} stroke="#fff" strokeWidth={1.8} opacity="0.5" />
        <circle cx={310} cy={350} r={42} fill="#4ab82a" opacity="0.5" /><polygon points={`${hp.x},${hp.y + 6} ${hp.x - 7},${hp.y + 1} ${hp.x - 7},${hp.y - 3} ${hp.x + 7},${hp.y - 3} ${hp.x + 7},${hp.y - 3} ${hp.x + 7},${hp.y + 1}`} fill="#fff" />
      </>
    );
  }, []);
  return (
    <div className="relative w-full max-w-[750px] mx-auto overflow-hidden rounded-xl shadow-2xl">
      ...
      <svg viewBox="0 0 620 500" className="w-full h-auto block bg-blue-100 font-sans">
        ...
        {['LF','CF','RF','F3B','SS','F2B','F1B','C'].map(k => movingFielder?.id !== k && <SVGFielder key={k} x={F[k].x} y={F[k].y} color={defColor} glove={k === 'F2B' || k === 'F1B' || k === 'RF' ? 'left' : 'right'} />)}
        ...
        {ballPos && ( <g><ellipse cx={ballPos.x} cy={ballPos.y + 3} rx={3} ry={1.2} fill="rgba(0,0,0,.12)" /><circle cx={ballPos.x} cy={ballPos.y} r={3.5} fill="#fff" stroke="#c00" strokeWidth={0.7} /></g> )}
      </svg>
    </div>
  );
}
...
function BaseballCardGameInner() {
  ...
  const [statsTeamIdx, setStatsTeamIdx] = useState(0);
  ...
  function dispatchFielderAnim(oc, sub, ba) {
    if (['flyOut', 'lineOut', 'foulOut'].includes(oc)) {
      const fk = (oc === 'foulOut') ? 'C' : (OF_MAP[sub] || INF_MAP[sub]);
      if (fk) animateFielder(fk, F[fk], ba.target, ba.dur);
    } else if (GROUND_OUTCOMES.has(oc)) {
      const fk = INF_MAP[sub] || 'SS'; animateFielder(fk, F[fk], ba.target, ba.dur);
    } else if (HIT_OUTCOMES.has(oc)) {
      const ofk = OF_MAP[sub] || 'CF'; animateFielder(ofk, F[ofk], ba.target, ba.dur + 500);
    }
  }
  ...
  function startGame() {
    ...
    setPlayHistory([]); setReplayIdx(-1); setHalfStartIdx(0); setDrawnPile([]); setDispensedCard(null); setOutcome(null); setShowFW(false); setAutoPlay(false); setBallPos(null); setPitchPhase('idle'); setBatterPhase('stance'); setAnnounceText(''); setActiveBatter(teams[0].players[0]); shuffleDeck(); setPhase('playing'); rerender();
  }
  ...
  function doDrawCard() {
    ...
    const nar = narrateLocal(oc, batter, gs.current.bases, res.runs, gs.current.outs);
        ...
        if (!doesSwing(oc)) {
          setTimeout(() => {
            animateBall(linePts(F.C, F.P, 16), 500, () => {
              setBallPos(null); processOutcome(oc, card, nar, false, res);
            });
          }, 300);
        }
      }); 
    ...
    if (doesSwing(oc)) {
      setTimeout(() => { 
        ...
        const isSacFly = res.isSacFly && ['flyOut', 'lineOut'].includes(oc);
        if (HIT_OUTCOMES.has(oc) || GROUND_OUTCOMES.has(oc) || oc === 'homeRun' || isSacFly) {
           animateRunners(computeRunnerPaths([...gs.current.bases], isSacFly ? 'sacFly' : (oc === 'bunt' ? 'single' : oc)), TEAM_COLORS[bt], oc === 'homeRun' ? 6500 : 2500);
           if (['homeRun','single','double','triple'].includes(oc) || (oc==='bunt' && res.outsAdded===0)) setTimeout(() => setBatterPhase('gone'), 500);
           setTimeout(() => { gs.current.bases = res.nextBases; rerender(); }, oc === 'homeRun' ? 6500 : 2500);
        }
      }, 1050);
      setTimeout(() => {
        setPitchPhase('idle'); setOutcome(oc);
        const ba = genBallAnimLocal(oc, nar.dir, nar.sub, nar.variant);
        const isH = HIT_OUTCOMES.has(oc), isG = GROUND_OUTCOMES.has(oc);
        dispatchFielderAnim(oc, nar.sub, ba);
    ...
    } else setTimeout(() => setAnnounceText(nar.text), 1100);
  }
  ...
  function replayPlay(idx) {
    ...
    setTimeout(() => { if (doesSwing(p.outcome)) { setBatterPhase('swing'); if (sndRef.current && !['ball', 'strike', 'walk', 'hbp'].includes(p.outcome)) playSoundLocal('hit'); animateRunners(computeRunnerPaths([...p.preBases], p.outcome === 'bunt' ? 'single' : (p.resultData?.isSacFly ? 'sacFly' : p.outcome)), TEAM_COLORS[bt], p.outcome === 'homeRun' ? 6500 : 2500); } }, 1050);
    setTimeout(() => {
      ...
      dispatchFielderAnim(p.outcome, p.sub, ba);
  ...
  const displayBases = replayIdx !== -1 && playHistory[replayIdx] ? playHistory[replayIdx].preBases : gs.current.bases;
  ...
  if (phase === 'setup') {
    return (
      <div className="min-h-screen bg-gray-100 flex flex-col items-center p-10 font-sans text-gray-900">
        <h1 className="text-4xl font-black text-blue-900 mb-2 tracking-tight uppercase">⚾ Card Baseball</h1>
  ...
  return (
    <div className="min-h-screen bg-stone-100 p-2 font-sans select-none overflow-x-hidden text-gray-900 text-[10px]">
      ...
      <div className="max-w-[750px] mx-auto flex flex-col items-start mt-2 px-2">
        ...
        {gs.current.gameOver && <div className="w-full text-center text-red-600 font-black text-[13px] animate-pulse py-1 bg-red-50 rounded-lg border border-red-100 uppercase tracking-widest shadow-sm">GAME OVER — {gs.current.message}</div>}
      </div>
      ...
      <div className="max-w-[750px] mx-auto flex flex-col items-center gap-4 mt-6">
        <div className="flex justify-center items-center gap-4 flex-wrap">
          ...
          <div className="flex bg-white rounded-lg border shadow-sm overflow-hidden h-10 text-gray-900">
            <button disabled={playHistory.length <= halfStartIdx || procRef.current} onClick={() => { const cur = replayIdx === -1 ? playHistory.length : replayIdx; const prev = cur > halfStartIdx ? (replayIdx === -1 ? playHistory.length-1 : replayIdx-1) : halfStartIdx; replayPlay(prev); }} className="px-4 hover:bg-gray-50 border-r disabled:opacity-20 cursor-pointer text-gray-900 text-gray-900">◄</button>
            <div className="px-4 flex items-center font-bold text-gray-500 uppercase text-[9px] tracking-widest">{replayIdx !== -1 ? `Play ${replayIdx + 1}` : 'REPLAY'}</div>
          ...
          <div className="flex bg-white rounded-lg border shadow-sm overflow-hidden h-10 p-1 gap-2 items-center text-gray-900">
            <button onClick={() => setAutoPlay(!autoPlay)} className={`px-4 h-8 rounded-md font-bold text-[9px] transition-colors cursor-pointer ${autoPlay ? 'bg-green-600 text-white' : 'bg-gray-100 text-gray-900'}`}>{autoPlay ? '⏸ PAUSE' : '▶ AUTO'}</button>
            <div className="flex flex-col items-center px-2 text-gray-900">
                <span className="text-[7px] text-gray-400 font-bold uppercase mb-0.5 leading-none">Speed</span>
                <input type="range" min={1500} max={6500} step={500} value={8000 - autoSpeed} onChange={e => setAutoSpeed(8000 - Number(e.target.value))} className="w-20 h-1 accent-blue-600 cursor-pointer text-gray-900" />
            </div>
          </div>

          <button onClick={() => setShowCardLibrary(true)} className="h-10 px-4 bg-white rounded-lg border shadow-sm text-blue-600 font-bold hover:bg-blue-50 cursor-pointer uppercase text-[9px]">🃏 Cards</button>
          <button onClick={() => setConfirmReset(true)} className="h-10 px-4 bg-white rounded-lg border shadow-sm text-gray-400 font-bold hover:text-red-500 cursor-pointer uppercase text-[9px]">Reset</button>
        </div>
        
        {confirmReset && ( 
          <div className="p-4 bg-red-50 border border-red-200 rounded-xl flex items-center justify-center gap-4 text-xs font-bold text-red-700 animate-in fade-in shadow-sm text-gray-900">
            <span>RESTART GAME?</span>
            <button onClick={startGame} className="px-6 py-1 bg-red-600 text-white rounded font-black cursor-pointer shadow-sm hover:bg-red-700 text-gray-900">YES</button>
            <button onClick={() => setConfirmReset(false)} className="px-6 py-1 bg-white border border-red-200 rounded cursor-pointer text-red-700 hover:bg-red-50 font-bold">NO</button>
          </div> 
        )}
      </div>
      ...
      {showCardLibrary && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/60 text-gray-900" onClick={() => setShowCardLibrary(false)}>
          <div className="bg-white rounded-2xl shadow-2xl max-w-3xl w-full mx-4 max-h-[85vh] overflow-hidden flex flex-col text-gray-900" onClick={e => e.stopPropagation()}>
            ...
            <div className="overflow-y-auto p-4 grid grid-cols-2 md:grid-cols-4 gap-4 text-gray-900">
                {SUITS.map(suit => (
                    <div key={suit} className={`rounded-xl border p-3 ${RED_SUITS.has(suit) ? 'bg-red-50 border-red-200' : 'bg-gray-50 border-gray-200'}`}>
                        <div className={`text-center font-black text-2xl mb-2 ${RED_SUITS.has(suit) ? 'text-red-600' : 'text-gray-900'}`}>{suit}</div>
                        <div className="space-y-1.5 text-gray-900">
                            {VALUES.map(v => (
                                <div key={v+suit} className="flex items-center justify-between bg-white rounded px-2 py-1 shadow-sm border border-gray-100 text-gray-900">
                                    <span className={`font-bold text-xs ${RED_SUITS.has(suit) ? 'text-red-600' : 'text-gray-900'}`}>{v}{suit}</span>
                                    <span className="text-[9px] font-bold text-gray-500">{CARD_MAP[v+suit]}</span>
                                </div>
                            ))}
                        </div>
                    </div>
      ...
      <div className="max-w-[750px] mx-auto mt-6 flex gap-4 h-52 overflow-hidden items-stretch text-gray-900">
        <div className="flex-1 bg-white p-3 rounded-xl shadow-inner overflow-y-auto text-gray-900"><h3 className="text-[10px] font-bold text-gray-400 uppercase mb-2 tracking-widest text-gray-900">Play-by-Play</h3>{reversedLog.map((l, i) => <div key={i} className={`text-xs mb-1 ${l.type === 'homer' ? 'text-orange-600 font-bold' : 'text-gray-700'}`}>{l.text}</div>)}</div>
        <div className="w-80 bg-white p-3 rounded-xl shadow-inner overflow-hidden flex flex-col text-gray-900">
          <div className="flex justify-between items-center mb-2">
             <h3 className="text-[10px] font-bold text-gray-400 uppercase tracking-widest text-gray-900">Live Stats</h3>
          ...
          {showStats ? (
            <div className="overflow-y-auto flex-1">
              <div className="flex mb-2 rounded-lg overflow-hidden border border-gray-200">
                {[0, 1].map(t => (<button key={t} onClick={() => setStatsTeamIdx(t)} className={`flex-1 py-1 text-[9px] font-bold uppercase transition-colors cursor-pointer ${statsTeamIdx === t ? (t === 0 ? 'bg-blue-900 text-white' : 'bg-red-600 text-white') : 'bg-gray-50 text-gray-500'}`}>{teams[t].name.split(' ').pop()}</button>))}
              </div>
              <table className="w-full text-left border-collapse text-gray-900">
                <thead className="sticky top-0 bg-white"><tr className="border-b border-gray-100 text-gray-400 uppercase font-bold text-[9px]"><th className="py-1">Player</th><th className="py-1 text-center w-8">AB</th><th className="py-1 text-center w-8">H</th><th className="py-1 text-center w-8">HR</th><th className="py-1 text-right w-12 text-right">AVG</th></tr></thead>
                <tbody>{teams[statsTeamIdx].players.map((p, i) => { const s = statsRef.current[statsTeamIdx][p] || { ab: 0, h: 0, hr: 0 }; const isCurrent = statsTeamIdx === gs.current.half && gs.current.batterIdx[statsTeamIdx] === i; return (<tr key={i} className={`${isCurrent ? 'bg-blue-50 font-bold text-blue-900' : 'text-gray-700'} border-b border-gray-50/50 hover:bg-stone-50`}><td className="py-1.5 truncate max-w-[100px]">{p}</td><td className="py-1.5 text-center">{s.ab}</td><td className="py-1.5 text-center">{s.h}</td><td className="py-1.5 text-center">{s.hr}</td><td className="py-1.5 text-right font-mono">{baValue(p, statsTeamIdx)}</td></tr>);})}</tbody></table></div>
