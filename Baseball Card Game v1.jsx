...
// Fence arc — wider to align with foul lines through 1B/3B
const FENCE = {cx:310, cy:430, rx:330, ry:370};

// ===== SOUND =====
function playSound(type,vol=0.5){try{const c=new(window.AudioContext||window.webkitAudioContext)();const n=c.currentTime;const v=Math.max(.05,Math.min(1,vol));if(type==='cardFlip'){const b=c.createBuffer(1,c.sampleRate*.06,c.sampleRate);const d=b.getChannelData(0);for(let i=0;i<d.length;i++)d[i]=(Math.random()*2-1)*Math.exp(-i/(c.sampleRate*.008))*.3;const s=c.createBufferSource();const f=c.createBiquadFilter();f.type='highpass';f.frequency.value=2000;s.buffer=b;s.connect(f);f.connect(c.destination);s.start(n)}else if(type==='hit'){const b=c.createBuffer(1,c.sampleRate*.2,c.sampleRate);const d=b.getChannelData(0);for(let i=0;i<d.length;i++){const t=i/c.sampleRate;d[i]=(Math.random()*2-1)*Math.exp(-t/.02)*.5+Math.sin(t*800)*Math.exp(-t/.05)*.3}const s=c.createBufferSource();const g=c.createGain();s.buffer=b;s.connect(g);g.connect(c.destination);g.gain.setValueAtTime(.5,n);s.start(n)}else if(type==='homerun'){const b=c.createBuffer(1,c.sampleRate*.15,c.sampleRate);const d=b.getChannelData(0);for(let i=0;i<d.length;i++)d[i]=(Math.random()*2-1)*Math.exp(-i/(c.sampleRate*.015))*.6;const s=c.createBufferSource();s.buffer=b;s.connect(c.destination);s.start(n);[523.25,659.25,783.99,1046.5].forEach((fr,i)=>{const o=c.createOscillator();const g=c.createGain();o.connect(g);g.connect(c.destination);o.type='triangle';o.frequency.value=fr;const t=n+.15+i*.12;g.gain.setValueAtTime(0,t);g.gain.linearRampToValueAtTime(.18,t+.04);g.gain.exponentialRampToValueAtTime(.001,t+.5);o.start(t);o.stop(t+.5)})}else if(type==='out'){const o=c.createOscillator();const g=c.createGain();o.connect(g);g.connect(c.destination);o.frequency.setValueAtTime(280,n);o.frequency.exponentialRampToValueAtTime(140,n+.25);g.gain.setValueAtTime(.1,n);g.gain.exponentialRampToValueAtTime(.001,n+.3);o.start(n);o.stop(n+.3)}else if(type==='walk'){[440,520].forEach((f,i)=>{const o=c.createOscillator();const g=c.createGain();o.connect(g);g.connect(c.destination);o.frequency.value=f;const t=n+i*.1;g.gain.setValueAtTime(.08,t);g.gain.exponentialRampToValueAtTime(.001,t+.15);o.start(t);o.stop(t+.15)})}else if(type==='strike'){const o=c.createOscillator();const g=c.createGain();o.connect(g);g.connect(c.destination);o.type='square';o.frequency.value=220;g.gain.setValueAtTime(.07,n);g.gain.exponentialRampToValueAtTime(.001,n+.12);o.start(n);o.stop(n+.12)}else if(type==='ball'){const o=c.createOscillator();const g=c.createGain();o.connect(g);g.connect(c.destination);o.frequency.value=350;g.gain.setValueAtTime(.05,n);g.gain.exponentialRampToValueAtTime(.001,n+.1);o.start(n);o.stop(n+.1)}else if(type==='crowd'){const dur=.4+v*1.2;const b=c.createBuffer(1,c.sampleRate*dur,c.sampleRate);const d=b.getChannelData(0);for(let i=0;i<d.length;i++){const e=Math.sin(Math.PI*i/d.length);d[i]=(Math.random()*2-1)*e*(.08+v*.25)}const s=c.createBufferSource();const f=c.createBiquadFilter();const g=c.createGain();f.type='bandpass';f.frequency.value=800+v*600;f.Q.value=.4;s.buffer=b;s.connect(f);f.connect(g);g.connect(c.destination);g.gain.setValueAtTime(v*.4,n);s.start(n)}else if(type==='gameOver'){[523.25,587.33,659.25,783.99,659.25,783.99,1046.5].forEach((fr,i)=>{const o=c.createOscillator();const g=c.createGain();o.connect(g);g.connect(c.destination);o.type='triangle';o.frequency.value=fr;const t=n+i*.15;g.gain.setValueAtTime(0,t);g.gain.linearRampToValueAtTime(.15,t+.05);g.gain.exponentialRampToValueAtTime(.001,t+.35);o.start(t);o.stop(t+.35)})}else if(type==='firework'){const b=c.createBuffer(1,c.sampleRate*.3,c.sampleRate);const d=b.getChannelData(0);for(let i=0;i<d.length;i++){const t=i/c.sampleRate;d[i]=(Math.random()*2-1)*Math.exp(-t/.08)*.2*Math.sin(t*150)}const s=c.createBufferSource();s.buffer=b;s.connect(c.destination);s.start(n+Math.random()*.3)}setTimeout(()=>c.close(),3000)}catch(e){}}
...
function advanceRunners(bases,type){let nb=[null,null,null],runs=0;const rs=[];if(bases[2])rs.push({pos:3,name:bases[2]});if(bases[1])rs.push({pos:2,name:bases[1]});if(bases[0])rs.push({pos:1,name:bases[0]});if(type==='single'||type==='error'){for(const r of rs){if(r.pos+1>3)runs++;else nb[r.pos]=r.name}nb[0]='batter'}else if(type==='double'){for(const r of rs){if(r.pos+2>3)runs++;else nb[r.pos+1]=r.name}nb[1]='batter'}else if(type==='triple'){runs+=rs.length;nb[2]='batter'}else if(type==='homeRun'){runs+=rs.length+1}else if(type==='groundOut'){for(const r of rs){if(r.pos+1>3)runs++;else nb[r.pos]=r.name}}else if(type==='doublePlay'){if(bases[0])nb=[null,bases[1]||null,bases[2]||null];else{for(const r of rs){if(r.pos+1>3)runs++;else nb[r.pos]=r.name}}}return{bases:nb,runs}}
...
// Clamp a point to stay inside the fence ellipse with margin
...
function getBallTgt(dir,oc){
  ...
  // Infield targets — between mound and baselines
  const infTgts=[
    {x:F.SS.x+j()*.4, y:F.SS.y+j()*.4},
    {x:F.F2B.x+j()*.4, y:F.F2B.y+j()*.4},
    {x:F.F3B.x+j()*.4, y:F.F3B.y+j()*.4},
    {x:F.F1B.x+j()*.4, y:F.F1B.y+j()*.4},
  ];

  // Shallow outfield (for singles — between infield and outfielders)
  const shallowOF={
    lf:{x:lerp(F.F3B.x,F.LF.x,.55)+j(), y:lerp(F.F3B.y,F.LF.y,.55)+j()},
    cf:{x:lerp(F.SS.x+F.F2B.x,F.CF.x*2,.25)+j(), y:lerp(F.SS.y,F.CF.y,.45)+j()},
    rf:{x:lerp(F.F1B.x,F.RF.x,.55)+j(), y:lerp(F.F1B.y,F.RF.y,.55)+j()},
  };

  // Deep outfield (for doubles — near outfielders)
  const deepOF={
    lf:{x:F.LF.x+j(), y:F.LF.y+j()*.5},
    cf:{x:F.CF.x+j(), y:F.CF.y+j()*.5+15},
    rf:{x:F.RF.x+j(), y:F.RF.y+j()*.5},
  };

  // Warning track (for triples — near fence)
  const wallArea={
    lf:{x:F.LF.x-25+j(), y:F.LF.y-25+j()*.3},
    cf:{x:F.CF.x+j(), y:F.CF.y-15+j()*.3},
    rf:{x:F.RF.x+25+j(), y:F.RF.y-25+j()*.3},
  };

  // Foul territory
  const foulTgts=[
    {x:hp.x-75+j()*.5, y:hp.y-40+j()*.5},
    {x:hp.x+75+j()*.5, y:hp.y-40+j()*.5},
    {x:hp.x-55+j()*.3, y:hp.y-70},
    {x:hp.x+55+j()*.3, y:hp.y-70},
  ];

  let t;
  if(oc==='homeRun'){
    const hrDir=dir||pick(['lf','cf','rf']);
    const fence=wallArea[hrDir]||wallArea.cf;
    return {x:fence.x, y:fence.y-40}; // OVER the fence
  }
...
function genBallAnim(oc,dir){
  const s={x:F.HP.x,y:F.HP.y-10};
  const t=getBallTgt(dir,oc);

  // Pitch-only outcomes — ball goes pitcher to catcher
  if(['ball','strike','strikeout','walk','hbp'].includes(oc))
    return{path:linePts(F.P,F.C,12),dur:650,type:'pitch'};

  // Foul ball — medium arc into foul territory, visible trajectory
  if(oc==='foulBall')
    return{path:arcPts(s,t,50+Math.random()*30,20),dur:1250,type:'foul'};

  // Foul out — high pop-up caught in foul territory
  if(oc==='foulOut')
    return{path:arcPts(s,t,90,24),dur:1600,type:'fly'};

  // Home run — towering arc OVER the fence
  if(oc==='homeRun')
    return{path:arcPts(s,t,120,32),dur:2600,type:'hr'};

  // Fly out — high arc to outfielder (caught)
  if(oc==='flyOut'){
    const h=dir==='inf'?60:90;
    return{path:arcPts(s,t,h,26),dur:1750,type:'fly'};
  }

  // Line out — fast, low trajectory to fielder (caught)
  if(oc==='lineOut'){
    const h=dir==='inf'?8:18;
    return{path:arcPts(s,t,h,18),dur:850,type:'line'};
  }

  // Ground out / double play — ball rolls on ground to infielder
  if(['groundOut','doublePlay'].includes(oc))
    return{path:linePts(s,t,16),dur:900,type:'ground'};

  // Single — low line drive or grounder through infield
  if(oc==='single'||oc==='error'){
    const h=dir==='inf'?8:25;
    return{path:arcPts(s,t,h,20),dur:1200,type:'hit'};
  }

  // Double — gap shot, medium arc
  if(oc==='double')
    return{path:arcPts(s,t,40,24),dur:1450,type:'hit'};

  // Triple — ball to the wall, medium-high arc
  if(oc==='triple')
    return{path:arcPts(s,t,50,26),dur:1550,type:'hit'};
...
function SVGFielder({x,y,color='#1e40af',glove='right'}){
  ...
  return (
    ...
    <line x1={0} y1={8} x2={3.5} y2={14} stroke={color} strokeWidth={2.2} strokeLinecap="round"/>
    <line x1={0} y1={2} x2={-gx} y2={5} stroke={color} strokeWidth={2} strokeLinecap="round"/>
    <line x1={0} y1={2} x2={gx} y2={0} stroke={color} strokeWidth={2} strokeLinecap="round"/>
    <circle cx={gx*1.1} cy={-1} r={3.5} fill="#8B4513" stroke="#5a3010" strokeWidth={.6}/>
    <circle cx={0} cy={-5} r={5} fill="#f5d0a0"/>
    <path d={`M-5.5,-6 Q0,-13 5.5,-6`} fill={color}/>
    <line x1={-5.5} y1={-6} x2={-8.5} y2={-6.5} stroke={color} strokeWidth={1.8} strokeLinecap="round"/>
    </g>
  );
}

// Pitcher — with wind-up/throw phases
function SVGPitcher({x,y,color='#1e40af',phase='idle'}){
  ...
  return (
    <g transform={`translate(${x},${y})`}>
    <ellipse cx={0} cy={14} rx={6} ry={2.5} fill="rgba(0,0,0,.12)"/>
    <line x1={bodyLean} y1={0} x2={bodyLean} y2={8} stroke={color} strokeWidth={2.8} strokeLinecap="round"/>
    <line x1={bodyLean} y1={8} x2={bodyLean-3.5} y2={14} stroke={color} strokeWidth={2.2} strokeLinecap="round"/>
    <line x1={bodyLean} y1={8} x2={bodyLean+3.5} y2={14} stroke={color} strokeWidth={2.2} strokeLinecap="round"/>
    {/* Glove hand */}
    <line x1={bodyLean} y1={2} x2={bodyLean-7} y2={4} stroke={color} strokeWidth={2} strokeLinecap="round"/>
    <circle cx={bodyLean-8} cy={4} r={3.5} fill="#8B4513" stroke="#5a3010" strokeWidth={.6}/>
    {/* Throwing arm */}
    <g transform={`rotate(${armAngle},${bodyLean},2)`}>
      <line x1={bodyLean} y1={2} x2={bodyLean+8} y2={-4} stroke={color} strokeWidth={2} strokeLinecap="round"/>
      {phase!=='throw'&&<circle cx={bodyLean+9} cy={-5} r={2.5} fill="#fff" stroke="#c00" strokeWidth={.5}/>}
    </g>
    <circle cx={bodyLean} cy={-5} r={5} fill="#f5d0a0"/>
    <path d={`M${bodyLean-5.5},-6 Q${bodyLean},-13 ${bodyLean+5.5},-6`} fill={color}/>
    <line x1={bodyLean-5.5} y1={-6} x2={bodyLean-8.5} y2={-6.5} stroke={color} strokeWidth={1.8} strokeLinecap="round"/>
    </g>
  );
}
...
function SVGCatcher({x,y,color='#1e40af'}){
  return (
    ...
    <line x1={5} y1={0} x2={5} y2={7} stroke={color} strokeWidth={2.2} strokeLinecap="round"/>
    {/* Mitt */}
    <line x1={0} y1={-1} x2={8} y2={-3} stroke={color} strokeWidth={2} strokeLinecap="round"/>
    <circle cx={9.5} cy={-3.5} r={4.5} fill="#8B4513" stroke="#5a3010" strokeWidth={.6}/>
    {/* Head + mask */}
    <circle cx={0} cy={-9} r={5} fill="#f5d0a0"/>
    <path d={`M-5,-10 Q0,-17 5,-10`} fill={color}/>
    <rect x={-3.5} y={-8} width={7} height={5} rx={1} fill="none" stroke="#555" strokeWidth={.7} opacity={.5}/>
    </g>
  );
}
...
function SVGUmpire({x,y}){
  return (
    ...
    <rect x={-3.5} y={-5} width={7} height={4} rx={1} fill="none" stroke="#555" strokeWidth={.6} opacity={.4}/>
    </g>
  );
}
...
function SVGBatter({x,y,color='#dc2626',phase='stance'}){
  if(phase==='gone') return null; // batter running, don't show at plate
  ...
  return (
    ...
    <line x1={bodyTwist} y1={0} x2={bodyTwist} y2={8} stroke={color} strokeWidth={2.8} strokeLinecap="round"/>
    <line x1={bodyTwist} y1={8} x2={bodyTwist-3} y2={14} stroke={color} strokeWidth={2.2} strokeLinecap="round"/>
    <line x1={bodyTwist} y1={8} x2={bodyTwist+3} y2={14} stroke={color} strokeWidth={2.2} strokeLinecap="round"/>
    {/* Arms holding bat */}
    <line x1={bodyTwist} y1={2} x2={bodyTwist+6} y2={swinging?4:-2} stroke={color} strokeWidth={2} strokeLinecap="round"/>
    {/* Bat */}
    <g transform={`rotate(${batAngle},${bodyTwist+6},${swinging?4:-2})`}>
      <line x1={bodyTwist+6} y1={swinging?4:-2} x2={bodyTwist+6} y2={swinging?4-16:-2-16} stroke="#b8860b" strokeWidth={2.5} strokeLinecap="round"/>
    </g>
    <circle cx={bodyTwist} cy={-5} r={5} fill="#f5d0a0"/>
    <path d={`M${bodyTwist-5},-6 Q${bodyTwist},-13 ${bodyTwist+5},-6`} fill={color}/>
    <line x1={bodyTwist+5} y1={-6} x2={bodyTwist+8} y2={-6.5} stroke={color} strokeWidth={1.8} strokeLinecap="round"/>
    </g>
  );
}

// Standing runner on base — mini fielder-like figure
function RunnerOnBase({x,y,color}){
  return (
    <g transform={`translate(${x},${y-10})`}>
      <ellipse cx={0} cy={14} rx={5} ry={2} fill="rgba(0,0,0,.12)"/>
      <line x1={0} y1={0} x2={0} y2={8} stroke={color} strokeWidth={2.5} strokeLinecap="round"/>
      <line x1={0} y1={8} x2={-3} y2={14} stroke={color} strokeWidth={2} strokeLinecap="round"/>
      <line x1={0} y1={8} x2={3} y2={14} stroke={color} strokeWidth={2} strokeLinecap="round"/>
      <line x1={0} y1={2} x2={-4} y2={5} stroke={color} strokeWidth={1.8} strokeLinecap="round"/>
      <line x1={0} y1={2} x2={4} y2={5} stroke={color} strokeWidth={1.8} strokeLinecap="round"/>
      <circle cx={0} cy={-4} r={4.5} fill="#f5d0a0"/>
      <path d={`M-4.5,-5 Q0,-11 4.5,-5`} fill={color}/>
    </g>
  );
}

// Running player figure — animated legs and arms
function SVGRunner({x,y,color,t=0,facingRight=true}){
  const cycle=Math.sin(t*Math.PI*6); // leg/arm cycle based on animation progress
  const legSpread=cycle*5;
  const armSwing=cycle*4;
  const flip=facingRight?1:-1;
  return (
    <g transform={`translate(${x},${y-12})`}>
      {/* Shadow */}
      <ellipse cx={0} cy={16} rx={6} ry={2.5} fill="rgba(0,0,0,.15)"/>
      {/* Body — slight forward lean */}
      <line x1={flip*2} y1={0} x2={0} y2={8} stroke={color} strokeWidth={2.5} strokeLinecap="round"/>
      {/* Legs — cycling */}
      <line x1={0} y1={8} x2={-legSpread*flip} y2={15} stroke={color} strokeWidth={2} strokeLinecap="round"/>
      <line x1={0} y1={8} x2={legSpread*flip} y2={15} stroke={color} strokeWidth={2} strokeLinecap="round"/>
      {/* Arms — pumping opposite to legs */}
      <line x1={flip*1.5} y1={2} x2={flip*1.5+armSwing*flip} y2={6} stroke={color} strokeWidth={1.8} strokeLinecap="round"/>
      <line x1={flip*1.5} y1={2} x2={flip*1.5-armSwing*flip} y2={6} stroke={color} strokeWidth={1.8} strokeLinecap="round"/>
      {/* Head */}
      <circle cx={flip*2.5} cy={-4} r={4.5} fill="#f5d0a0"/>
      <path d={`M${flip*2.5-4.5},-5 Q${flip*2.5},-11 ${flip*2.5+4.5},-5`} fill={color}/>
    </g>
  );
}
...
function BaseballField({bases, defColor, offColor, ballPos, pitchPhase, batterPhase, showFW, fwKey, movingRunners}){
  ...
  // Compute foul poles: extend lines from HP through B1/B3 to fence ellipse
  // Line: P(t) = HP + t*(Base - HP). Solve for t where ((x-cx)/rx)²+((y-cy)/ry)²=1
  const foulPole=(base)=>{
  ...
  const fl=foulPole(F.B3); // left foul pole
  const fr=foulPole(F.B1); // right foul pole
  const flx=fl.x, fly=fl.y, frx=fr.x, fry=fr.y;

  // Infield dirt shape — realistic arc from 1B side to 3B side
  const dirtPath = `M ${hp.x-40} ${hp.y+5}
    L ${F.B3.x-22} ${F.B3.y+5}
    Q ${F.B3.x-15} ${F.B3.y-15} ${F.B3.x} ${F.B3.y-18}
    Q ${F.B2.x-30} ${F.B2.y-5} ${F.B2.x} ${F.B2.y-22}
    Q ${F.B2.x+30} ${F.B2.y-5} ${F.B1.x} ${F.B1.y-18}
    Q ${F.B1.x+15} ${F.B1.y-15} ${F.B1.x+22} ${F.B1.y+5}
    L ${hp.x+40} ${hp.y+5} Z`;
  ...
  return(
    <div style={{position:'relative',width:'100%',maxWidth:750,margin:'0 auto'}}>
      ...
      <svg viewBox="0 0 620 500" style={{width:'100%',height:'auto',display:'block',borderRadius:14,overflow:'hidden',boxShadow:'0 4px 24px rgba(0,0,0,.12)'}}>
        ...
        {/* Outfield fence — solid green wall */}
        ...
        {/* Pitcher's MOUND — raised dirt circle */}
        ...
        {/* Infield grass cutout (circle between bases) */}
        <circle cx={310} cy={350} r={42} fill="#4ab82a" opacity=".5"/>

        {/* Base paths — faint white lines */}
        ...
        {/* Animated running players */}
        {movingRunners&&movingRunners.map((r,i)=>(
          <SVGRunner key={`mr${i}`} x={r.x} y={r.y} color={r.color} t={r.t} facingRight={r.facingRight}/>
        ))}
        ...
        {/* Ball */}
        {ballPos&&<g>
          <ellipse cx={ballPos.x} cy={ballPos.y+3} rx={3} ry={1.2} fill="rgba(0,0,0,.12)"/>
          <circle cx={ballPos.x} cy={ballPos.y} r={3.5} fill="#fff" stroke="#c00" strokeWidth={.7}/>
        </g>}

        {/* Distance signs ON the fence */}
        {(()=>{
          ...
          return marks.map((m,i)=>{
            ...
            const px=fc.cx+fc.rx*Math.sin(a);
            const py=fc.cy-fc.ry*Math.cos(a);
            return <g key={i}>
              ...
              <text x={px} y={py+.5} textAnchor="middle" dominantBaseline="middle"
                fill="#ffd700" fontSize="8" fontWeight="bold" fontFamily="sans-serif">{m.label}</text>
            </g>;
          });
        })()}
      </svg>
...
function CardArea({drawnPile, dispensedCard, dispensing, onDraw, canDraw, autoPlay, gameOver, shuffling, shuffleCards}){
  return(
    <div style={{display:'flex',alignItems:'flex-start',gap:16,justifyContent:'center',flexWrap:'wrap',padding:'0 8px',position:'relative'}}>
      ...
      {shuffling&&<div style={{position:'absolute',top:0,left:0,right:0,bottom:0,zIndex:20,display:'flex',alignItems:'center',justifyContent:'center',background:'rgba(232,228,216,.9)',borderRadius:10}}>
        <div style={{display:'flex',flexDirection:'column',alignItems:'center',gap:8}}>
          <div style={{fontSize:12,fontWeight:700,color:'#1a3a5c',
            background:'#fff',padding:'4px 14px',borderRadius:6,boxShadow:'0 2px 6px rgba(0,0,0,.12)'}}>
            🔀 Shuffling 52 Cards...
          </div>
          <div style={{position:'relative',width:180,height:80}}>
            {/* Left half of deck */}
            {shuffleCards.slice(0,8).map((card,i)=>{
              return <div key={`sl${i}`} style={{
                position:'absolute',
                left:10+i*1.5,top:i*0.5,
                animation:`shuffleRiffle .4s ease-in-out ${i*.05}s infinite alternate`,
                zIndex:i,opacity:.9
              }}>
                <MiniCard card={card} style={{width:36,height:50,fontSize:7,padding:'1px 2px',
                  boxShadow:'0 1px 4px rgba(0,0,0,.15)'}}/>
              </div>;
            })}
            {/* Right half of deck */}
            {shuffleCards.slice(8,16).map((card,i)=>{
              return <div key={`sr${i}`} style={{
                position:'absolute',
                right:10+i*1.5,top:i*0.5,
                animation:`shuffleRiffleR .4s ease-in-out ${i*.05}s infinite alternate`,
                zIndex:i,opacity:.9
              }}>
                <MiniCard card={card} style={{width:36,height:50,fontSize:7,padding:'1px 2px',
                  boxShadow:'0 1px 4px rgba(0,0,0,.15)'}}/>
      ...
      {/* Card Machine — entire area is clickable */}
      <div style={{display:'flex',flexDirection:'column',alignItems:'center',flexShrink:0}}>
        <div style={{position:'relative',width:100,height:152,cursor:canDraw?'pointer':'default',userSelect:'none',
          transition:'transform .1s'}}
          onClick={canDraw?onDraw:undefined}
          onMouseDown={e=>{if(canDraw)e.currentTarget.style.transform='scale(.95)'}}
          onMouseUp={e=>e.currentTarget.style.transform='scale(1)'}
          onMouseLeave={e=>e.currentTarget.style.transform='scale(1)'}
          role="button" tabIndex={0} onKeyDown={e=>{if(canDraw&&(e.key==='Enter'||e.key===' '))onDraw()}}>
          {/* Machine body */}
          <div style={{width:100,height:152,borderRadius:'12px 12px 8px 8px',
            background:'linear-gradient(180deg,#5a5a5a 0%,#444 30%,#333 100%)',
            border:canDraw?'2px solid #5a5':'2px solid #666',
            boxShadow:canDraw?'0 4px 16px rgba(0,0,0,.3),0 0 12px rgba(80,180,80,.25)':'0 4px 12px rgba(0,0,0,.2)',
            position:'relative',overflow:'hidden',transition:'border-color .3s, box-shadow .3s'}}>
            {/* "CARD" label */}
            <div style={{background:'#222',margin:'8px 12px 0',padding:'2px 6px',borderRadius:3,textAlign:'center'}}>
              <span style={{color:'#c9a227',fontSize:10,fontWeight:700,letterSpacing:1.5,fontFamily:'system-ui'}}>CARD</span>
            </div>
            {/* "CLICK HERE" colored block */}
            <div style={{margin:'4px 10px',padding:'4px 6px',borderRadius:4,textAlign:'center',
              background:canDraw?'linear-gradient(135deg,#2d8c1f,#1a6b12)':'#555',
              transition:'background .3s'}}>
              <span style={{color:'#fff',fontSize:9,fontWeight:800,letterSpacing:1,fontFamily:'system-ui',
                animation:canDraw&&!autoPlay?'drawPulse 1.5s ease-in-out infinite':'none'}}>
                {autoPlay?'⏸ AUTO':gameOver?'GAME OVER':'⚾ CLICK HERE'}
              </span>
            </div>
            {/* "DEALER" label */}
            <div style={{background:'#222',margin:'0 12px',padding:'2px 6px',borderRadius:3,textAlign:'center'}}>
              <span style={{color:'#c9a227',fontSize:10,fontWeight:700,letterSpacing:1.5,fontFamily:'system-ui'}}>DEALER</span>
            </div>
            {/* Card slot */}
            <div style={{width:74,height:4,background:'#111',borderRadius:2,margin:'6px auto 0',boxShadow:'inset 0 1px 3px rgba(0,0,0,.5)'}}/>
            {/* Card stack visual */}
            <div style={{margin:'5px auto 0',width:52,height:30,background:'linear-gradient(135deg,#1e3a5f,#0f2440)',borderRadius:3,border:'1px solid #c9a227',display:'flex',alignItems:'center',justifyContent:'center'}}>
              <span style={{color:'#c9a227',fontSize:14}}>⚾</span>
            </div>
          </div>
        </div>
        {/* Dispensed card area — reserved space below machine so it doesn't overlap */}
        <div style={{height:90,position:'relative',width:100,overflow:'visible',zIndex:1}}>
          {dispensedCard && (
            <div style={{position:'absolute',top:dispensing?4:-60,left:20,transition:dispensing?'top .35s cubic-bezier(.34,1.56,.64,1)':'none'}}>
              <MiniCard card={dispensedCard}/>
            </div>
          )}
        </div>
      </div>

      {/* Drawn card pile — stacked */}
      <div style={{minWidth:90,minHeight:110,flexShrink:0}}>
        <div style={{fontSize:10,color:'#888',fontWeight:700,textTransform:'uppercase',letterSpacing:.8,marginBottom:4}}>
          This Half-Inning ({drawnPile.length})
        </div>
        <div style={{position:'relative',width:60,height:90}}>
          ...
          {drawnPile.map((card,i)=>{
            const isTop=i===drawnPile.length-1;
            // First card always dead straight; subsequent cards get slight rotation
            const rot=i===0?0:((i*13+7)%11-5)*1.2;
            const ox=i===0?0:((i*11+3)%9-4)*0.6;
            const oy=Math.min(i*0.4,5);
            return(
              <div key={`p${i}`} style={{position:'absolute',top:oy,left:ox,transform:`rotate(${rot}deg)`,
                zIndex:i,opacity:isTop?1:.85,transition:'transform .2s'}}>
                <MiniCard card={card} style={{width:50,height:70,fontSize:9,padding:'2px 3px',
                  boxShadow:isTop?'0 2px 8px rgba(0,0,0,.2)':'0 1px 3px rgba(0,0,0,.1)'}}/>
              </div>
            );
          })}
        </div>
...
function BaseballCardGameInner(){
  ...
  const computeRunnerPaths=(oldBases, outcome)=>{
    const paths=[];
    const hp=F.HP, b1=F.B1, b2=F.B2, b3=F.B3;
    if(['single','error'].includes(outcome)){
      ...
      // Lead runner out, others may advance
      if(oldBases[2]) paths.push([b3,hp]);
  ...
  const doDrawCard=()=>{
    ...
    setTimeout(()=>{
      setPitchPhase('throw');
      const pitchPath=linePts(F.P,{x:F.HP.x,y:F.HP.y-5},16);
      animateBall(pitchPath,650,()=>{});
    },550);

    // 4) Ball arrives at plate ~t=1200, batter reacts
    setTimeout(()=>{
      if(swings){
        setBatterPhase('swing');
        if(sndRef.current&&!isPitchOnly)playSound('hit');
      }
    },1200);

    // 5) Show outcome + secondary animation at ~t=1400
    setTimeout(()=>{
      setPitchPhase('idle');
      setDrawnPile(prev=>[...prev,card]);
      setOutcome(oc);

      if(isPitchOnly||oc==='strikeout'){
        // Catcher catches → throw back to pitcher
        setBatterPhase('stance');
        setTimeout(()=>{
          try{
            const throwBack=linePts(F.C,F.P,14);
            animateBall(throwBack,600,()=>clearBall());
          }catch(e){clearBall();}
        },500);
      } else if(oc==='foulBall'){
        // Batter swings, foul ball arcs into foul territory
        try{
          const ba=genBallAnim(oc,'foul');
          animateBall(ba.path,ba.dur,()=>{clearBall();setBatterPhase('stance');});
        }catch(e){clearBall();setBatterPhase('stance');}
      } else if(oc==='foulOut'){
        // Pop-up caught in foul territory
        setBatterPhase('swing');
        try{
          const ba=genBallAnim(oc,'foul');
          animateBall(ba.path,ba.dur,()=>{
            // Hold ball at catch point
            setTimeout(()=>clearBall(),400);
            setBatterPhase('stance');
          });
        }catch(e){clearBall();setBatterPhase('stance');}
      } else if(['groundOut','doublePlay'].includes(oc)){
        // Ground ball to infielder — ball stops at fielder (caught/fielded)
        setBatterPhase('swing');
        try{
          const ba=genBallAnim(oc,'inf');
          animateBall(ba.path,ba.dur,()=>{
            setTimeout(()=>clearBall(),300);
            setBatterPhase('stance');
          });
        }catch(e){clearBall();setBatterPhase('stance');}
      } else if(oc==='flyOut'){
        // Fly ball caught by outfielder — ball stops at fielder
        setBatterPhase('swing');
        try{
          const nar=narrate(oc,teams[gs.current.half].players[gs.current.batterIdx[gs.current.half]],gs.current.bases,0,gs.current.outs);
          const ba=genBallAnim(oc,nar.dir);
          animateBall(ba.path,ba.dur,()=>{
            setTimeout(()=>clearBall(),400);
            setBatterPhase('stance');
          });
        ...
        // Line drive caught — ball stops at fielder
        setBatterPhase('swing');
        try{
          const nar=narrate(oc,teams[gs.current.half].players[gs.current.batterIdx[gs.current.half]],gs.current.bases,0,gs.current.outs);
          const ba=genBallAnim(oc,nar.dir);
        ...
        // Hits: single, double, triple, homeRun, error — batter runs
        setBatterPhase('gone');
        try{
          const nar=narrate(oc,teams[gs.current.half].players[gs.current.batterIdx[gs.current.half]],gs.current.bases,0,gs.current.outs);
          const ba=genBallAnim(oc,nar.dir);
    ...
    // 6) GAME LOGIC — runs on fixed timer, matched to ball animation duration
    // Base offset is ~1400 (outcome dispatch) + ball flight duration
    const logicDelays = {
      ball:2300, strike:2300, walk:2300, hbp:2300, strikeout:2300,
      foulBall:2800, foulOut:3200,
      groundOut:2500, doublePlay:2500,
      lineOut:2400, flyOut:3300,
      single:2800, error:2800, double:3000, triple:3100,
      homeRun:4200,
    };
    const logicDelay = logicDelays[oc] || 1500;
    setTimeout(()=>{
      try{
        clearBall();setPitchPhase('idle');
        // Reset batter for outs/counts — hits keep 'gone' until runner anim
        ...
        console.error('processOutcome error:',e);
        procRef.current=false;setBatterPhase('stance');rerender();
      }
    },logicDelay);
  };
  ...
  const processOutcome=(oc,card)=>{
    try {
    console.log('processOutcome:',oc);
    const g=gs.current;const bt=g.half;
    const batter=teams[bt].players[g.batterIdx[bt]];
    const stats=statsRef.current;
    const st=stats[bt]&&stats[bt][batter]?stats[bt][batter]:null;
    if(!st){console.warn('No stats for batter:',batter,'team:',bt);stats[bt]=stats[bt]||{};stats[bt][batter]={ab:0,h:0,d:0,tr:0,hr:0,rbi:0,r:0,bb:0,so:0}}
    ...
    const diff=bt===0?aR-hR:hR-aR;
    const crowdI=getCrowdIntensity(g.inning,diff,g.bases.filter(Boolean).length,g.outs);
    ...
    // Animate runners before updating bases
    ...
    // Check if batter is running (path starting from HP)
    ...
    } catch(e) { console.error('processOutcome error:',e);procRef.current=false;clearBall();setPitchPhase('idle');setBatterPhase('stance');rerender(); }
  };
  ...
  const transitionHalf=()=>{
    ...
    setHalfStartIdx(playHistory.length); // Mark start of new half-inning in history
    setAnnounceText(g.half===1?`Bottom of inning ${g.inning}`:`Top of inning ${g.inning}`);
    rerender();
    // Shuffle animation
    doShuffleAnimation(()=>{
      shuffleDeck();rerender();procRef.current=false;
    });
  };

  const doShuffleAnimation=(onComplete)=>{
    setShuffling(true);
    // Generate 16 random cards for a full-deck feel
    const sampleCards=[];
    for(let i=0;i<16;i++){
      const s=SUITS[Math.floor(Math.random()*4)];
      const v=VALUES[Math.floor(Math.random()*13)];
      sampleCards.push({value:v,suit:s,key:v+s});
    }
    ...
    setTimeout(()=>{
      setShuffling(false);setShuffleCards([]);
  ...
  const replayPlay=(idx)=>{
    ...
    procRef.current=true;
    const play=playHistory[idx];
    setReplayIdx(idx);
    const oc=play.outcome;
    const swings=doesSwing(oc);
    const isPitchOnly=['ball','strike','walk','hbp'].includes(oc);
    const isHit=['single','double','triple','homeRun','error','walk','hbp'].includes(oc);

    // 1) Card dispense
    setDispensedCard(play.card);setDispensing(false);
    setTimeout(()=>setDispensing(true),30);
    if(sndRef.current)playSound('cardFlip');
    setOutcome(null);setAnnounceText('');
    ...
    setTimeout(()=>{
      setPitchPhase('throw');
      try{animateBall(linePts(F.P,{x:F.HP.x,y:F.HP.y-5},16),650,()=>{})}catch(e){}
    },550);
    ...
    // 5) Outcome + ball animation at t=1400
    setTimeout(()=>{
      setPitchPhase('idle');
      setOutcome(oc);
      setAnnounceText(play.narration);

      if(isPitchOnly||oc==='strikeout'){
        setBatterPhase('stance');
        setTimeout(()=>{
          try{animateBall(linePts(F.C,F.P,14),600,()=>clearBall())}catch(e){clearBall();}
        },500);
      } else if(oc==='foulBall'){
        try{
          const ba=genBallAnim(oc,'foul');
        ...
        try{
          const ba=genBallAnim(oc,'foul');
          animateBall(ba.path,ba.dur,()=>{
            // Hold ball at catch point briefly
            setTimeout(()=>clearBall(),400);
            setBatterPhase('stance');
          });
        }catch(e){clearBall();setBatterPhase('stance');}
        ...
        setBatterPhase('swing');
        try{
          const ba=genBallAnim(oc,play.dir);
          animateBall(ba.path,ba.dur,()=>{
            setTimeout(()=>clearBall(),400);
            setBatterPhase('stance');
          });
    ...
    const logicDelay=logicDelayMap[oc]||2500;
    setTimeout(()=>{
      clearBall();setPitchPhase('idle');
      if(isHit){
        const bt=play.team!==undefined?play.team:gs.current.half;
        const runColor=bt===0?'#b91c1c':'#1e40af';
        const preBases=play.preBases||[null,null,null];
        const rPaths=computeRunnerPaths(preBases,oc);
        const runDur=oc==='homeRun'?2400:['walk','hbp'].includes(oc)?1300:1200;
        if(rPaths.length>0){
          animateRunners(rPaths,runColor,runDur);
          setTimeout(()=>{setBatterPhase('stance');procRef.current=false},runDur+100);
        } else {
          setBatterPhase('stance');procRef.current=false;
        }
      } else {
        setBatterPhase('stance');procRef.current=false;
      }
        ...
        <div style={{display:'flex',gap:20,flexWrap:'wrap',justifyContent:'center',maxWidth:800}}>
          {[0,1].map(t=>(
            <div key={t} style={{background:'#fff',border:'1px solid #ddd',borderRadius:12,padding:18,width:350,boxShadow:'0 2px 8px rgba(0,0,0,.06)'}}>
              ...
              <select value={Object.keys(PRESETS).includes(teams[t].name)?teams[t].name:'custom'}
                onChange={e=>{const n=e.target.value;const nt=[...teams];nt[t]=n==='custom'?{name:'',players:Array(9).fill('')}:{name:n,players:[...PRESETS[n]]};setTeams(nt)}}
                style={{width:'100%',padding:'10px 12px',borderRadius:8,border:'1px solid #ccc',background:'#fafafa',fontSize:14,marginBottom:10,cursor:'pointer'}}>
              ...
              {teams[t].players.map((p,i)=>(
                <div key={i} style={{display:'flex',alignItems:'center',gap:6,marginBottom:3}}>
                  <span style={{fontSize:12,color:'#999',width:18,textAlign:'right',fontWeight:600}}>{i+1}.</span>
                  <input value={p} onChange={e=>{const nt=[...teams];nt[t].players[i]=e.target.value;setTeams(nt)}} placeholder={`Player ${i+1}`}
                    style={{flex:1,padding:'5px 10px',borderRadius:6,border:'1px solid #ddd',fontSize:13,background:'#fafafa',boxSizing:'border-box'}}/>
                </div>
              ))}
            </div>
          ))}
        </div>
        <button onClick={startGame} disabled={!teams[0].name||!teams[1].name||teams[0].players.some(p=>!p)||teams[1].players.some(p=>!p)}
          style={{marginTop:24,padding:'14px 48px',borderRadius:10,border:'2px solid #1a5c2a',background:'linear-gradient(135deg,#2d8c1f,#1a6b12)',color:'#fff',fontSize:20,fontWeight:800,cursor:'pointer',letterSpacing:.5,opacity:(!teams[0].name||!teams[1].name||teams[0].players.some(p=>!p)||teams[1].players.some(p=>!p))?.4:1,boxShadow:'0 4px 12px rgba(26,92,18,.3)'}}>
          PLAY BALL!
        </button>
      </div>
    );
  ...
  // ===== GAME SCREEN =====
  const g=gs.current,bt=g.half;
  const batter=teams[bt].players[g.batterIdx[bt]];
  const aR=g.innings[0].reduce((s,r)=>s+r,0),hR=g.innings[1].reduce((s,r)=>s+r,0);
  const maxInn=Math.max(g.innings[0].length,g.innings[1].length,9);
  const innH=Array.from({length:maxInn},(_,i)=>i+1);
  ...
  return(
    <div style={{minHeight:'100vh',background:'linear-gradient(180deg,#e8e4d8 0%,#f0ede4 50%,#e8e4d8 100%)',color:'#1a1a1a',fontFamily:'system-ui,-apple-system,sans-serif',padding:'6px 8px'}}>
      <style>{`@keyframes drawPulse{0%,100%{opacity:1}50%{opacity:.4}}
@keyframes shuffleRiffle{0%{transform:translateX(0) translateY(0)}50%{transform:translateX(30px) translateY(-12px) rotate(5deg)}100%{transform:translateX(0) translateY(0)}}
@keyframes shuffleRiffleR{0%{transform:translateX(0) translateY(0)}50%{transform:translateX(-30px) translateY(-12px) rotate(-5deg)}100%{transform:translateX(0) translateY(0)}}`}</style>
      ...
      <div style={{maxWidth:750,margin:'0 auto 6px',background:'#fff',borderRadius:10,border:'1px solid #ddd',boxShadow:'0 2px 8px rgba(0,0,0,.08)',padding:'6px 10px',overflowX:'auto'}}>
        <table style={{width:'100%',borderCollapse:'collapse',fontSize:13,minWidth:420}}>
          <thead><tr style={{borderBottom:'2px solid #e0e0e0'}}>
            <th style={{textAlign:'left',padding:'3px 6px',width:140,fontWeight:700,color:'#333'}}></th>
            {innH.map(i=><th key={i} style={{textAlign:'center',padding:'3px 3px',width:26,fontWeight:600,color:i===g.inning?'#1a3a5c':'#999',fontSize:12,background:i===g.inning?'#eef2ff':'transparent',borderRadius:4}}>{i}</th>)}
            <th style={{textAlign:'center',padding:'3px 6px',fontWeight:800,color:'#c41e1e',fontSize:15,borderLeft:'2px solid #e0e0e0'}}>R</th>
            <th style={{textAlign:'center',padding:'3px 5px',fontWeight:700,color:'#333',fontSize:12}}>H</th>
            <th style={{textAlign:'center',padding:'3px 5px',fontWeight:700,color:'#333',fontSize:12}}>E</th>
          </tr></thead>
          <tbody>{[0,1].map(t=>(
            <tr key={t} style={{background:bt===t&&!g.gameOver?'#f8f9ff':'transparent'}}>
              <td style={{padding:'4px 6px',fontWeight:bt===t?700:500,color:bt===t&&!g.gameOver?'#1a3a5c':'#555',fontSize:13,whiteSpace:'nowrap',overflow:'hidden',textOverflow:'ellipsis',maxWidth:140}}>
                {bt===t&&!g.gameOver&&<span style={{color:'#c41e1e',marginRight:3}}>▸</span>}{teams[t].name}
              </td>
              {innH.map((_,i)=><td key={i} style={{textAlign:'center',padding:'3px',fontWeight:600,color:g.innings[t][i]!==undefined?(g.innings[t][i]>0?'#1a1a1a':'#888'):'transparent',fontSize:14,background:i+1===g.inning?'#eef2ff':'transparent',borderRadius:4,fontVariantNumeric:'tabular-nums'}}>{g.innings[t][i]!==undefined?g.innings[t][i]:''}</td>)}
              <td style={{textAlign:'center',padding:'3px 6px',fontWeight:800,color:'#c41e1e',fontSize:17,borderLeft:'2px solid #e0e0e0',fontVariantNumeric:'tabular-nums'}}>{t===0?aR:hR}</td>
              <td style={{textAlign:'center',padding:'3px 5px',fontWeight:600,fontSize:13,fontVariantNumeric:'tabular-nums'}}>{g.hits[t]}</td>
              <td style={{textAlign:'center',padding:'3px 5px',fontWeight:600,fontSize:13,fontVariantNumeric:'tabular-nums'}}>{g.errors[t]}</td>
            </tr>
          ))}</tbody>
        </table>
      </div>
      ...
      <div style={{maxWidth:750,margin:'0 auto 4px',display:'flex',alignItems:'center',gap:14,justifyContent:'center',flexWrap:'wrap'}}>
        <span style={{fontSize:15,fontWeight:700,color:'#1a3a5c'}}>{g.half===0?'▲':'▼'} {g.inning}{g.inning===1?'st':g.inning===2?'nd':g.inning===3?'rd':'th'}</span>
        <span style={{color:'#ccc'}}>│</span>
        <span style={{fontSize:15,letterSpacing:3}}><span style={{color:'#2563eb'}}>{'●'.repeat(Math.min(3,Math.max(0,g.count.balls)))}</span><span style={{color:'#ddd'}}>{'●'.repeat(Math.max(0,3-g.count.balls))}</span>{' '}<span style={{color:'#dc2626'}}>{'●'.repeat(Math.min(2,Math.max(0,g.count.strikes)))}</span><span style={{color:'#ddd'}}>{'●'.repeat(Math.max(0,2-g.count.strikes))}</span></span>
        <span style={{color:'#ccc'}}>│</span>
        <span style={{fontSize:15,letterSpacing:3}}><span style={{color:'#dc2626'}}>{'●'.repeat(Math.min(g.outs,3))}</span><span style={{color:'#ddd'}}>{'●'.repeat(Math.max(0,3-g.outs))}</span><span style={{fontSize:11,color:'#999',marginLeft:4,letterSpacing:0}}> Out</span></span>
        <span style={{color:'#ccc'}}>│</span>
        <span style={{fontSize:13,fontWeight:600,color:'#444'}}>AB: {batter}</span>
      </div>

      {/* FIELD */}
      <BaseballField bases={g.bases} defColor={bt===0?'#1e40af':'#b91c1c'} offColor={bt===0?'#b91c1c':'#1e40af'}
        ballPos={ballPos} pitchPhase={pitchPhase} batterPhase={batterPhase} showFW={showFW} fwKey={fwKey} movingRunners={movingRunners}/>
      ...
      {/* CARD AREA + CONTROLS */}
      <div style={{maxWidth:750,margin:'6px auto 0'}}>
        <CardArea drawnPile={drawnPile} dispensedCard={dispensedCard} dispensing={dispensing}
          onDraw={doDrawCard} canDraw={canDraw} autoPlay={autoPlay} gameOver={g.gameOver}
          shuffling={shuffling} shuffleCards={shuffleCards}/>
      </div>
      ...
      <div style={{maxWidth:750,margin:'8px auto',display:'flex',alignItems:'center',gap:8,justifyContent:'center',flexWrap:'wrap',position:'relative',zIndex:5}}>
        ...
        {autoPlay&&<div style={{display:'flex',gap:3}}>{[{l:'1×',v:4500},{l:'2×',v:3000},{l:'4×',v:1800},{l:'8×',v:1000}].map(s=><button key={s.l} onClick={()=>setAutoSpeed(s.v)} style={{background:autoSpeed===s.v?'#e0e7ff':'#fff',border:`1px solid ${autoSpeed===s.v?'#6366f1':'#ddd'}`,borderRadius:4,padding:'2px 6px',color:autoSpeed===s.v?'#4338ca':'#888',fontSize:10,fontWeight:600,cursor:'pointer'}}>{s.l}</button>)}</div>}
        <span style={{color:'#ddd'}}>│</span>
        <div style={{display:'flex',alignItems:'center',gap:3}}>
          <button onClick={()=>{
            const minIdx=halfStartIdx;
            const idx=replayIdx<0?playHistory.length-1:(replayIdx<=minIdx?minIdx:replayIdx-1);
            replayPlay(idx);
          }}
            disabled={playHistory.length<=halfStartIdx||procRef.current||(replayIdx>=0&&replayIdx<=halfStartIdx)}
            style={{background:'#fff',border:'1px solid #ddd',borderRadius:6,padding:'4px 8px',fontSize:11,
              cursor:'pointer',color:'#555',
              opacity:playHistory.length>halfStartIdx&&!procRef.current&&!(replayIdx>=0&&replayIdx<=halfStartIdx)?1:.4}}>◄</button>
          <span style={{fontSize:10,color:'#888',minWidth:55,textAlign:'center'}}>
            {replayIdx>=halfStartIdx?`${replayIdx-halfStartIdx+1}/${playHistory.length-halfStartIdx}`:`Replay (${playHistory.length-halfStartIdx})`}
          </span>
          <button onClick={()=>{
            const maxIdx=playHistory.length-1;
            const idx=replayIdx<0?halfStartIdx:Math.min(replayIdx+1,maxIdx);
            replayPlay(idx);
          }}
            disabled={playHistory.length<=halfStartIdx||procRef.current||(replayIdx>=0&&replayIdx>=playHistory.length-1)}
            style={{background:'#fff',border:'1px solid #ddd',borderRadius:6,padding:'4px 8px',fontSize:11,
              cursor:'pointer',color:'#555',
              opacity:playHistory.length>halfStartIdx&&!procRef.current&&!(replayIdx>=0&&replayIdx>=playHistory.length-1)?1:.4}}>►</button>
        </div>
      ...
      {confirmReset&&<div style={{maxWidth:750,margin:'6px auto',padding:'10px 16px',background:'#fef2f2',border:'1px solid #fecaca',borderRadius:8,display:'flex',gap:10,alignItems:'center',justifyContent:'center',flexWrap:'wrap'}}>
        <span style={{fontSize:13,color:'#b91c1c',fontWeight:600}}>Reset?</span>
        <button onClick={()=>{setConfirmReset(false);startGame()}} style={{padding:'4px 14px',borderRadius:6,border:'1px solid #ef4444',background:'#fef2f2',color:'#b91c1c',fontSize:12,fontWeight:700,cursor:'pointer'}}>Restart</button>
        <button onClick={()=>{setConfirmReset(false);setPhase('setup');gs.current=INIT_GS();rerender()}} style={{padding:'4px 14px',borderRadius:6,border:'1px solid #d97706',background:'#fffbeb',color:'#92400e',fontSize:12,fontWeight:700,cursor:'pointer'}}>New Teams</button>
        <button onClick={()=>setConfirmReset(false)} style={{padding:'4px 14px',borderRadius:6,border:'1px solid #ddd',background:'#fff',color:'#888',fontSize:12,cursor:'pointer'}}>Cancel</button>
      </div>}

      {g.gameOver&&g.message&&<div style={{maxWidth:750,margin:'8px auto',padding:'14px 28px',borderRadius:10,background:'linear-gradient(135deg,#fefce8,#fff)',border:'2px solid #ca8a04',textAlign:'center',boxShadow:'0 4px 15px rgba(0,0,0,.1)'}}>
        <div style={{fontSize:20,fontWeight:800,color:'#92400e'}}>🎉 {g.message}</div>
        <button onClick={()=>{setPhase('setup');gs.current=INIT_GS();rerender()}} style={{marginTop:10,padding:'8px 28px',borderRadius:8,border:'2px solid #1a5c2a',background:'linear-gradient(135deg,#2d8c1f,#1a6b12)',color:'#fff',fontSize:14,fontWeight:700,cursor:'pointer'}}>New Game</button>
      </div>}

      {/* STATS + LOG */}
      <div style={{maxWidth:750,margin:'8px auto 0',display:'flex',gap:8,flexWrap:'wrap'}}>
        <div ref={logRef} style={{flex:'1 1 300px',maxHeight:180,overflowY:'auto',background:'#fff',border:'1px solid #ddd',borderRadius:8,padding:8}}>
          <div style={{fontSize:10,color:'#999',marginBottom:4,fontWeight:700,textTransform:'uppercase',letterSpacing:1}}>Play-by-Play</div>
          {log.map((e,i)=><div key={i} style={{fontSize:12,padding:'2px 0',lineHeight:1.5,color:e.type==='info'?'#1a3a5c':e.type==='homer'?'#ea580c':e.type==='hit'?'#16a34a':e.type==='out'?'#888':'#555',fontWeight:e.type==='info'||e.type==='homer'?700:400,borderBottom:e.type==='info'?'1px solid #eee':'none',paddingBottom:e.type==='info'?4:0,marginBottom:e.type==='info'?4:0}}>{e.text}</div>)}
        </div>
        <div style={{flex:'1 1 300px',maxHeight:180,overflowY:'auto',background:'#fff',border:'1px solid #ddd',borderRadius:8,padding:8}}>
          <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:4}}>
            <span style={{fontSize:10,color:'#999',fontWeight:700,textTransform:'uppercase',letterSpacing:1}}>Player Stats</span>
            <button onClick={()=>setShowStats(s=>!s)} style={{background:'none',border:'none',fontSize:10,color:'#1a3a5c',fontWeight:700,cursor:'pointer'}}>{showStats?'Hide':'Show'}</button>
          </div>
          {showStats&&[0,1].map(t=><div key={t} style={{marginBottom:8}}>
            <div style={{fontSize:11,fontWeight:700,color:'#1a3a5c',marginBottom:2}}>{teams[t].name}</div>
            <table style={{width:'100%',fontSize:10,borderCollapse:'collapse'}}>
              <thead><tr style={{borderBottom:'1px solid #eee'}}>{['Player','AB','H','2B','3B','HR','RBI','R','BB','SO','AVG'].map(h=><th key={h} style={{padding:'2px 2px',textAlign:h==='Player'?'left':'center',color:'#999',fontWeight:600}}>{h}</th>)}</tr></thead>
              <tbody>{teams[t].players.map((p,i)=>{const s=statsRef.current[t][p]||{};return <tr key={i} style={{borderBottom:'1px solid #f5f5f5',background:bt===t&&g.batterIdx[t]===i?'#eef2ff':'transparent'}}>
                <td style={{padding:'2px 2px',fontWeight:bt===t&&g.batterIdx[t]===i?700:400,maxWidth:80,overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}}>{p}</td>
                {[s.ab||0,s.h||0,s.d||0,s.tr||0,s.hr||0,s.rbi||0,s.r||0,s.bb||0,s.so||0].map((v,j)=><td key={j} style={{textAlign:'center',padding:'2px',fontVariantNumeric:'tabular-nums'}}>{v}</td>)}
                <td style={{textAlign:'center',padding:'2px',fontWeight:600,fontVariantNumeric:'tabular-nums'}}>{ba(p,t)}</td>
              </tr>})}</tbody>
            </table>
          </div>)}
        </div>
