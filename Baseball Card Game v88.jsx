...
function BaseballCardGameInner() {
  ...
  useEffect(() => { return () => { if (animRef.current) cancelAnimationFrame(animRef.current); if (runnerAnimRef.current) cancelAnimationFrame(runnerAnimRef.current); if (fielderAnimRef.current) cancelAnimationFrame(fielderAnimRef.current); if (plateAnimRef.current) cancelAnimationFrame(plateAnimRef.current); if (moundVisitRef.current) cancelAnimationFrame(moundVisitRef.current); if (pitcherCoverAnimRef.current) cancelAnimationFrame(pitcherCoverAnimRef.current); if (ofBackupAnimRef.current) cancelAnimationFrame(ofBackupAnimRef.current); if (pitcherHomeBackupAnimRef.current) cancelAnimationFrame(pitcherHomeBackupAnimRef.current); }; }, []);
  ...
  function doDrawCard() {
    ...
    if (doesStepOut) {
      ...
      setBatterPhase('stance');
      setTimeout(() => setPitcherRubberKick(false), 700);
      setTimeout(() => setRubberKickBadge(null), 1600);
      setTimeout(() => { setPitchPhase('windup'); setCatcherSigns(signCount); setCoachSignKey(k => k + 1); }, rubberKickDelay);
    } else {
    ...
    setTimeout(() => {
      ...
      if (oc === 'ball' && nar.isPitchout) {
        ...
        animateBall(linePts(F.P, { x: wideX, y: wideY }, 16), 520, () => {
          ...
          setTimeout(() => setPitchoutBadge(null), 3000);
          setTimeout(() => {
    ...
    if (nar.isCheckSwing) {
      ...
      setTimeout(() => {
        ...
        setTimeout(() => setUmpire1BPhase('idle'), 1500);
        setTimeout(() => setCheckSwingBadge(null), 3200);
      }, csCueDelay);
    }
    ...
    if (doesSwing(oc)) {
      ...
      setTimeout(() => {
        ...
        if (nar.isOffFoot && oc === 'foulBall') {
          ...
          setTimeout(() => setBatterPhase('offFoot'), 60);
          setTimeout(() => setBatterPhase('stance'), 60 + 1000);
          ...
          setTimeout(() => setOffFootBadge(null), 2500);
          if (sndRef.current) setTimeout(() => playSoundLocal('fouledOffFoot'), 140);
        }
        ...
        if (nar.isWindAided || nar.isWindBlownBack) {
          ...
          setTimeout(() => setWindBadge(null), 3500);
          if (sndRef.current) setTimeout(() => playSoundLocal('windGust'), Math.max(0, (ba.dur || 1600) * 0.35));
        }
        ...
        if (nar.isStretchedDouble && oc === 'double') {
          ...
          setTimeout(() => setStretchedDoubleBadge(null), 3500);
          if (sndRef.current) setTimeout(() => playSoundLocal('stretchDouble'), 80);
          ...
          setTimeout(() => triggerCrowdReaction('hit', bt), 1500);
        }
        ...
        if (nar.isLineOutDP && oc === 'lineOut') {
          ...
          setTimeout(() => setLineOutDPBadge(null), 3500);
          ...
          setTimeout(() => triggerCrowdReaction('doublePlay', bt), 700);
        }
        ...
        if (['flyOut', 'lineOut', 'foulOut'].includes(oc)) {
          ...
          if (nar.isFoulSacFly && oc === 'foulOut') {
            ...
            setTimeout(() => { if (playIdRef.current === thisPlayId) { setFoulSacFlyBadge({ batter, runner: fsfRunnerLast, fielder: nar.foulSacFlyFielder || 'the fielder', team: bt }); setTimeout(() => setFoulSacFlyBadge(null), 3500); } }, ba.dur);
            if (sndRef.current) setTimeout(() => playSoundLocal('glovePop'), ba.dur);
            setTimeout(() => { if (playIdRef.current === thisPlayId) triggerCrowdReaction('hit', bt); }, ba.dur + 200);
          }
          ...
          if (nar.isCapOff && ba.target) {
            ...
            setTimeout(() => {
              ...
              setTimeout(() => setCapFlyOff(null), 1400);
            }, ba.dur);
            setTimeout(() => setCapOffBadge({ team: 1 - bt }), ba.dur + 50);
            setTimeout(() => setCapOffBadge(null), ba.dur + 1700);
            if (sndRef.current) setTimeout(() => playSoundLocal('capFly'), ba.dur + 60);
          }
          ...
          if (isRobbedHR && ba.target) {
            ...
            if (nar.isCapOff) {
              ...
              setTimeout(() => setCapOffBadge({ team: 1 - bt }), ba.dur + 100);
              setTimeout(() => setCapOffBadge(null), ba.dur + 1700);
              if (sndRef.current) setTimeout(() => playSoundLocal('capFly'), ba.dur - 40);
            }
          }
        } else if (isG) {
        ...
        animateBall(ba.path, ba.dur, () => {
          if (isGRD) {
            ...
            setTimeout(() => {
              ...
              setTimeout(() => { if (playIdRef.current === thisPlayId) setPitcherHomeBackupBadge(null); }, 3200);
            }, 80);
            setTimeout(() => {
    ...
    } else setTimeout(() => setAnnounceText(nar.text), 1100 + stepOutDelay + shakeOffDelay + cleatKnockDelay + browWipeDelay + checkRunnerDelay + signNodDelay + rubberKickDelay);
  }
  ...
  function transitionHalf() {
    ...
    setTimeout(() => {
      ...
      if (is7thStretch) {
        ...
        if (sndRef.current) setTimeout(() => playSoundLocal('fanCheer'), 200);
        setTimeout(() => { setBeachBallActive(false); doTransition(); }, 3400);
        ...
        if (sndRef.current) setTimeout(() => playSoundLocal('mascotDance'), 250);
        setTimeout(() => { setMascotAct(null); doTransition(); }, 3400);
      } else {
        doTransition();
      }
    }, 2500);
  ...
  function replayPlay(idx) {
    ...
    setTimeout(() => {
      ...
      if (p.isFoulSacFly && p.outcome === 'foulOut') {
        ...
        setTimeout(() => setFoulSacFlyBadge(null), 3500);
        if (sndRef.current) setTimeout(() => playSoundLocal('glovePop'), ba.dur);
        setTimeout(() => triggerCrowdReaction('hit', bt), ba.dur + 200);
      }
      ...
      if (p.isOutfieldAssist && p.outcome === 'single') {
        ...
        setTimeout(() => {
          ...
          setPitcherHomeBackupBadge({ team: 1 - bt });
          if (sndRef.current) playSoundLocal('pitcherBackup');
          setTimeout(() => setPitcherHomeBackupBadge(null), 3200);
          setTimeout(() => setPitcherHomeBackup(null), ba.dur + 2200);
        }, 80);
  ...
  if (phase === 'setup') {
    return (
      <div className="min-h-screen bg-gray-100 flex flex-col items-center p-10 font-sans text-gray-900">
        <h1 className="text-4xl font-black text-blue-900 mb-2 tracking-tight uppercase">⚾ Card Baseball v88</h1>
