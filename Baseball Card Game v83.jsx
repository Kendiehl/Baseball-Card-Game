...
function BaseballCardGameInner() {
  ...
  function doDrawCard() {
    ...
    if (nar.isCheckSwing) {
      const csCueDelay = 1000 + stepOutDelay + shakeOffDelay + cleatKnockDelay + browWipeDelay + checkRunnerDelay + signNodDelay;
      setTimeout(() => {
        if (playIdRef.current !== thisPlayId) return;
  ...
  function replayPlay(idx) {
    ...
    setTimeout(() => {
      ...
      if (['flyOut', 'lineOut', 'foulOut'].includes(p.outcome)) {
        const fk = (p.outcome === 'foulOut') ? 'C' : (OF_MAP[p.sub] || INF_MAP[p.sub]);
        if (fk) animateFielder(fk, F[fk], ba.target, ba.dur);
  ...
  if (phase === 'setup') {
    return (
      <div className="min-h-screen bg-gray-100 flex flex-col items-center p-10 font-sans text-gray-900">
        <h1 className="text-4xl font-black text-blue-900 mb-2 tracking-tight uppercase">⚾ Card Baseball v83</h1>
