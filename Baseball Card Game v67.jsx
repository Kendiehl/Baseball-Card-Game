  ...
  return (
    <div className="relative w-full max-w-[750px] mx-auto overflow-hidden rounded-xl shadow-2xl">
      ...
      <svg viewBox="0 0 620 500" className="w-full h-auto block bg-blue-100 font-sans">
        ...
        {!rosinAct && <SVGPitcher x={F.P.x} y={F.P.y - 14} color={defColor} phase={pitcherTipCap ? 'tipCap' : (pitcherBrowWipe ? 'wipeBrow' : (pitcherCheckRunner ? 'checkRunner' : pitchPhase))} fromStretch={fromStretch} headShake={pitcherShakeOff} />}
        ...
        {/* Pickoff dive — runner briefly slides back to 1B or 2B (Backlog #64, #120).
            Ball is rendered by ballPos. The `base` field on pickoffDive selects which
            bag the slide-back renders at: 'B1' (default) or 'B2'. The lateral
            direction flips for 2B so the slide reads naturally — at 1B the runner
            slides back from the lead-off direction (toward 2B) so the body trails
            right-to-left; at 2B the runner slides back from the lead toward 3B so
            the body trails left-to-right (mirrored horizontally). */}
        {pickoffDive && (() => {
          const isB2 = pickoffDive.base === 'B2';
          const bag = isB2 ? F.B2 : F.B1;
          const ox = isB2 ? 0 : -6;
          const oy = isB2 ? 4 : 2;
          // Horizontal flip multiplier — 1B keeps the original orientation, 2B mirrors.
          const f = isB2 ? -1 : 1;
          return (
...
function BaseballCardGameInner() {
  ...
  function replayPlay(idx) {
    ...
    // Pickoff replay (Backlog #64, #120) — no card, no pitch. Windup → throw to
    // 1B or 2B → dive-back → (out or safe). The `pickoffBase` field on the play
    // history selects which base the throw goes to. Legacy plays without the
    // field default to 'B1' so older saves replay correctly.
    if (p.isPickoff) {
      setDrawnPile(p.pileSnapshot || []);
      setOutcome(null); setAnnounceText(p.narration); setPitchPhase('windup'); setBatterPhase('stance'); setActiveBatter(p.batter);
      const offColor = bt === 0 ? '#b91c1c' : '#1e40af';
      const poBase = p.pickoffBase === 'B2' ? 'B2' : 'B1';
      const poBag = poBase === 'B2' ? F.B2 : F.B1;
      const poBallOutDur = poBase === 'B2' ? 420 : 380;
      const poBallBackDur = poBase === 'B2' ? 380 : 360;
      setTimeout(() => {
        setPitchPhase('idle');
