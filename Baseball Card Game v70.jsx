...
function simulateGameInstant(team1Players, team2Players) {
  ...
  while (safety-- > 0) {
    ...
    // First-to-third on a single (Backlog #123) — sim mirror. ~18% of singles
    // with a runner on 1B only and < 2 outs convert the runner-from-1B's
    // advancement from 2B to 3B. No AB/H/RBI change, but the runner is now
    // in scoring position which affects subsequent run-scoring probability.
    // Keeps season stats aligned with live-game behavior.
    if (oc === 'single' && gs.bases[0] && !gs.bases[1] && !gs.bases[2] && gs.outs < 2 && Math.random() < 0.18) {
      res.nextBases = [...res.nextBases];
...
function SVGBatter({ x, y, color = '#dc2626', phase = 'stance', side = 'R', chokeUp = false }) {
  ...
  // component.
  const applyChokeUp = chokeUp && !isBatFlip && !isStepOut && !isHbp && !isBrushback && !isCleatKnock;
...
function SVGCatcher({ x, y, color = '#1e40af', isMoving = false, signs = 0, lookingUp = false, blocking = false }) {
  ...
  return (
    <div className="relative w-full max-w-[750px] mx-auto overflow-hidden rounded-xl shadow-2xl">
      ...
      <svg viewBox="0 0 620 500" className="w-full h-auto block bg-blue-100 font-sans">
        ...
        {movingFielder?.id !== 'C' && !moundVisit && <SVGCatcher x={F.C.x} y={F.C.y - 8} color={defColor} signs={catcherSigns} lookingUp={catcherLookingUp} blocking={catcherBlocking} />}
        ...
        {!rosinAct && <SVGPitcher key={fistPump ? `fp-${fistPump.ts}` : 'p-std'} x={F.P.x} y={F.P.y - 14} color={fistPump?.color || defColor} phase={fistPump ? 'fistPump' : (pitcherTipCap ? 'tipCap' : (pitcherBrowWipe ? 'wipeBrow' : (pitcherCheckRunner ? 'checkRunner' : pitchPhase)))} fromStretch={fromStretch} headShake={pitcherShakeOff} />}
...
function BaseballCardGameInner() {
  ...
  function doDrawCard() {
    ...
    if (gs.current.bases[1] && !gs.current.bases[0] && !gs.current.gameOver && Math.random() < 0.012) {
      ...
      const pickoffOut = Math.random() < 0.18;
      const offColor = bt === 0 ? '#b91c1c' : '#1e40af';
      const poTexts = pickoffOut
        ? [
    ...
    if (gs.current.bases[2] && !gs.current.bases[0] && !gs.current.bases[1] && !gs.current.gameOver && Math.random() < 0.004) {
      ...
      const pickoffOut = Math.random() < 0.25;
      const offColor = bt === 0 ? '#b91c1c' : '#1e40af';
      const poTexts = pickoffOut
        ? [
    ...
    const isComebacker = oc === 'groundOut' && !nar.isBangBang && !nar.isDivingStop && !nar.isProductiveOut && !nar.isChallenge && Math.random() < 0.05;
    if (isComebacker) {
    ...
    const doesBrowWipe = !doesStepOut && !doesCleatKnock && !doesShakeOff
      && !stealHomeEvent && !stolenBaseEvent && oc !== 'hbp'
    ...
    }, 550 + stepOutDelay + shakeOffDelay + cleatKnockDelay + browWipeDelay + checkRunnerDelay);
    ...
    if (doesSwing(oc)) {
      setTimeout(() => {
        ...
        if (['single', 'double', 'triple', 'homeRun', 'error', 'groundOut', 'doublePlay', 'bunt'].includes(oc) || (res.isSacFly && ['flyOut', 'lineOut'].includes(oc))) {
           ...
           if (nar.isFirstToThird && oc === 'single') {
           ...
           const ftTrotDur = (nar.isFirstToThird && oc === 'single') ? 3200 : troDur;
      ...
      }, 1050 + stepOutDelay + shakeOffDelay + cleatKnockDelay + browWipeDelay + checkRunnerDelay);
      setTimeout(() => {
      ...
      }, 1100 + stepOutDelay + shakeOffDelay + cleatKnockDelay + browWipeDelay + checkRunnerDelay);
    } else setTimeout(() => setAnnounceText(nar.text), 1100 + stepOutDelay + shakeOffDelay + cleatKnockDelay + browWipeDelay + checkRunnerDelay);
  }
  ...
  function replayPlay(idx) {
    ...
    // Pickoff replay (Backlog #64, #120, #124) — no card, no pitch. Windup →
    // throw to 1B, 2B, or 3B → dive-back → (out or safe). The `pickoffBase`
    // field on the play history selects which base the throw goes to. Legacy
    // plays without the field default to 'B1' so older saves replay correctly.
    if (p.isPickoff) {
      setDrawnPile(p.pileSnapshot || []);
      setOutcome(null); setAnnounceText(p.narration); setPitchPhase('windup'); setBatterPhase('stance'); setActiveBatter(p.batter);
      const offColor = bt === 0 ? '#b91c1c' : '#1e40af';
      ...
      // Ball-flight durations vary slightly per base — 2B is farthest from the
      // mound, 3B is roughly the same diagonal as 1B (matched to live timings).
      const poBallOutDur = poBase === 'B2' ? 420 : (poBase === 'B3' ? 400 : 380);
      const poBallBackDur = poBase === 'B2' ? 380 : (poBase === 'B3' ? 380 : 360);
      setTimeout(() => {
        setPitchPhase('idle');
        animateBall(linePts(F.P, poBag, 14), poBallOutDur, () => {
          setBallPos(null);
          if (sndRef.current) playSoundLocal('glovePop');
          setPickoffDive({ color: offColor, base: poBase });
          setTimeout(() => setPickoffDive(null), 900);
          if (p.pickoffOut) {
            setPickoffBadge({ outcome: 'out', team: 1 - bt, base: poBase });
            setTimeout(() => setPickoffBadge(null), 2600);
            setTimeout(() => { procRef.current = false; }, 950);
          } else {
            setPickoffBadge({ outcome: 'safe', team: 1 - bt, base: poBase });
            setTimeout(() => setPickoffBadge(null), 2200);
            setTimeout(() => {
              animateBall(linePts(poBag, F.P, 12), poBallBackDur, () => { setBallPos(null); procRef.current = false; });
            }, 500);
          }
        });
      }, 450);
    ...
    setTimeout(() => {
      if (doesSwing(p.outcome)) {
        ...
        if (p.isDropped3rd) {
          ...
          const finalTrotDur = (p.isFirstToThird && p.outcome === 'single') ? 3200 : trotDur;
