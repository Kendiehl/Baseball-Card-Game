...
function INIT_GS() {
  return {
    ...
    hits: [0, 0],
    errors: [0, 0],
    // Left on base (LOB) — runners stranded at the end of each half-inning,
    // credited to the batting team. Tallied at the half-inning transition and
    // at the standard 9th-inning game-ending third out. Shown in the post-game
    // box score. (A walk-off ending leaves its runners untallied — the game
    // ended on a hit, not stranded runners — a minor, intentional omission.)
    lob: [0, 0],
    rallyCount: 0,
...
function simulateGameInstant(team1Players, team2Players) {
  ...
  const stats = [{}, {}];
  // bb (free passes: BB + HBP) and pa (plate appearances) added with Backlog
  // #167 so the season engine can support OBP leaders later. ab now follows
  // official scoring (no AB on walks/HBP, sac flies, or sac bunts).
  for (let t = 0; t < 2; t++) for (const p of teams[t]) stats[t][p] = { ab: 0, h: 0, hr: 0, rbi: 0, bb: 0, pa: 0 };
  ...
  // Wind conditions (Backlog #152): roll a per-game wind exactly like the live
  // game's startGame — 18% blowing out, 15% blowing in, otherwise calm — so
  // season HR totals stay aligned with interactive expectations.
  const simWindRoll = Math.random();
  const simWind = simWindRoll < 0.18 ? 'out' : simWindRoll < 0.33 ? 'in' : null;
  ...
  while (safety-- > 0) {
    ...
    if (ibbSim) {
      ...
      // Backlog #167: an IBB is a plate appearance and a walk (no AB).
      stats[bt][batter].pa++; stats[bt][batter].bb++;
    ...
    // Mirror the live-game rate so season stats match interactive games. The
    // simRobbed flag keeps the wind (Backlog #152) from carrying a robbed ball
    // back out of the park, mirroring the live !isRobbedHR guard.
    let simRobbed = false;
    if (oc === 'homeRun' && Math.random() < 0.03) { oc = 'flyOut'; simRobbed = true; }
    ...
    // games. The simITP flag keeps the wind (Backlog #152) from knocking down
    // a ball that never left the park, mirroring the live !isInsideThePark guard.
    let simITP = false;
    if (oc === 'triple' && Math.random() < 0.03) { oc = 'homeRun'; simITP = true; }
    ...
    // Wind conditions (Backlog #152): mirror the live-game re-maps so season
    // HR totals stay aligned. Blowing out → ~8% of flyOuts carry over the
    // wall; blowing in → ~14% of homeRuns die at the track (a runner on 3B
    // with < 2 outs still scores via the engine's sac-fly path). Same guards
    // as live play: no compounding with the robbed HR, the inside-the-park
    // homer, or position-player-pitching conversions.
    if (simWind && !posPP[1 - bt]) {
      if (simWind === 'out' && oc === 'flyOut' && !simRobbed && Math.random() < 0.08) oc = 'homeRun';
      else if (simWind === 'in' && oc === 'homeRun' && !simITP && Math.random() < 0.14) oc = 'flyOut';
    }
    ...
    // Check-swing appeal (Backlog #150): ~11% of 2-strike `ball` outcomes get
    // appealed to the base umpire as a possible check swing; ~45% of those are
    // ruled "he went around!" and become strikeouts. The remaining ~55% are
    // ruled "no swing" and the ball stands (no re-map). Mirrors the live-game
    // rate so season K totals stay close to interactive expectations.
    if (oc === 'ball' && gs.count.strikes === 2 && Math.random() < 0.11 && Math.random() < 0.45) {
      oc = 'strikeout';
    }
    ...
    if (Math.random() < 0.0015) {
      ...
      // Backlog #167: CI is a plate appearance (no AB, no BB — reach on error).
      stats[bt][batter].pa++;
    ...
    // Stretched single → double (Backlog #159) — sim mirror. ~5% of singles
    // with bases empty (clean line-drive single situation) convert to a
    // double via the batter rounding first hard and stretching it. Re-mapped
    // BEFORE calculatePlayResult so all downstream stat logic (hit credit,
    // double increment, etc.) flows naturally. Mirrors the live-game rate so
    // season doubles totals stay close to interactive expectations.
    if (oc === 'single' && !gs.bases[0] && !gs.bases[1] && !gs.bases[2] && Math.random() < 0.05) {
      oc = 'double';
    }
    ...
    // Line-drive double play (Backlog #161) — sim mirror. ~6% of lineOut
    // outcomes with a runner on 1B or 2B and < 2 outs re-mapped to a double
    // play: the runner most off the bag is doubled off. R2 takes priority
    // over R1 (the longer lead is the more dangerous one to be caught off).
    // Mutates res: +1 out (becomes 2 total), runner cleared from nextBases.
    // Mutually exclusive with the tag-up block above (a doubled-off runner
    // can't also tag up), enforced via the simIsLineOutDP gate. Same
    // probability as the live game so season HR / hit / out distributions
    // stay close.
    let simIsLineOutDP = false;
    if (oc === 'lineOut' && !res.isSacFly && gs.outs < 2
        && (gs.bases[0] || gs.bases[1])
        && Math.random() < 0.06) {
      if (gs.bases[1]) {
        simIsLineOutDP = true;
        res.outsAdded = 2;
        res.nextBases = [res.nextBases[0], null, res.nextBases[2]];
      } else if (gs.bases[0]) {
        simIsLineOutDP = true;
        res.outsAdded = 2;
        res.nextBases = [null, res.nextBases[1], res.nextBases[2]];
      }
    }
    if (['flyOut', 'lineOut'].includes(oc) && !res.isSacFly && !simIsLineOutDP && gs.bases[1]
    ...
    let nO = gs.outs + res.outsAdded, bD = false, simIsFreePass = false;
    if (oc === 'ball') { gs.count.balls++; if (gs.count.balls >= 4) { bD = true; simIsFreePass = true; } }
    ...
    else if (doesSwing(oc) || ['walk', 'hbp'].includes(oc)) { bD = true; if (oc === 'walk' || oc === 'hbp') simIsFreePass = true; }
    if (bD) {
      const S = stats[bt][batter];
      // Official-scoring AB rules (Backlog #167): no AB charged on walks
      // (ball four or a walk/HBP card), sacrifice flies (fair or foul), or
      // sacrifice bunts (batter retired while a runner advances). The IBB and
      // catcher's-interference paths above already skip the AB the same way.
      // Errors/ROE still charge the AB per standard scoring. Note gs.bases is
      // still the PRE-play state here (reassigned from res.nextBases below),
      // so the sac-bunt check correctly reads the runners the bunt advanced.
      const simIsSacBunt = oc === 'bunt' && res.outsAdded > 0 && (gs.bases[0] || gs.bases[1] || gs.bases[2]);
      S.pa++;
      if (simIsFreePass) S.bb++;
      else if (!res.isSacFly && !res.isFoulSacFly && !simIsSacBunt) S.ab++;
      ...
      if (!simDroppedFly && !simIsThrowError && ['single', 'double', 'triple', 'homeRun'].includes(oc)) { S.h++; if (oc === 'homeRun') S.hr++; gs.hits[bt]++; }
      if (oc === 'bunt' && res.outsAdded === 0) { S.h++; gs.hits[bt]++; }
      // RBI rules aligned with the live game (Backlog #167): no RBI on errors
      // (including the ROE re-maps) or double plays, per standard scoring.
      if (!simDroppedFly && !simIsThrowError && oc !== 'error' && oc !== 'doublePlay' && res.runs > 0) S.rbi += res.runs;
...
function simulateSeason(teamNames) {
  ...
  for (const name of teamNames) {
    ...
    playerStats[name] = {};
    for (const p of PRESETS[name]) playerStats[name][p] = { ab: 0, h: 0, hr: 0, rbi: 0, bb: 0, pa: 0 };
  }
  for (let i = 0; i < n; i++) {
    for (let j = i + 1; j < n; j++) {
      ...
      for (let g = 0; g < gamesPerMatchup; g++) {
        ...
        let result = simulateGameInstant(PRESETS[away], PRESETS[home]);
        // Backlog #167: a safety-exhaustion tie (winner: -1, vanishingly rare)
        // was previously mis-counted as a home win. Re-sim the game until it
        // produces a decision; if it somehow never does, skip it entirely
        // rather than miscredit either team.
        let tieGuard = 8;
        while (result.winner === -1 && tieGuard-- > 0) result = simulateGameInstant(PRESETS[away], PRESETS[home]);
        ...
        for (const p of awayPlayers) {
          ...
          if (s) { playerStats[away][p].ab += s.ab; playerStats[away][p].h += s.h; playerStats[away][p].hr += s.hr; playerStats[away][p].rbi += s.rbi; playerStats[away][p].bb += s.bb; playerStats[away][p].pa += s.pa; }
        }
        for (const p of homePlayers) {
          const s = result.stats[1][p];
          if (s) { playerStats[home][p].ab += s.ab; playerStats[home][p].h += s.h; playerStats[home][p].hr += s.hr; playerStats[home][p].rbi += s.rbi; playerStats[home][p].bb += s.bb; playerStats[home][p].pa += s.pa; }
        }
      }
    }
  }
  ...
  for (const team of teamNames) {
    for (const p of PRESETS[team]) {
      ...
      // OBP (Backlog #180): on-base percentage from the BB/PA tracked since
      // #167. obp = (hits + walks) / plate appearances. `bb` here folds BB+HBP
      // together and `pa` counts every plate appearance, so this is a close
      // approximation of official OBP — it includes sac flies in the
      // denominator (correct) and sac bunts (a minor over-count, negligible at
      // season scale). Guard pa > 0 to avoid NaN.
      if (s.ab > 0) allPlayers.push({ name: p, team, ...s, avg: s.h / s.ab, obp: s.pa > 0 ? (s.h + s.bb) / s.pa : 0 });
    }
  }
  ...
  // OBP leaders (Backlog #180): same >=30-PA qualifier idea as AVG leaders,
  // keyed on plate appearances rather than at-bats so walk-heavy hitters
  // qualify on the same footing.
  const obpLeaders = [...allPlayers].filter(p => p.pa >= 30).sort((a, b) => b.obp - a.obp).slice(0, 10);
  return { standings: sorted, hrLeaders, avgLeaders, rbiLeaders, obpLeaders, gamesPerMatchup };
}
...
function calculatePlayResult(currentgs, type, batterName) {
  ...
  // Force-chain resolver (Backlog #164) — shared correctness fix for bunt,
  // groundOut, and doublePlay. The old branches silently ERASED baserunners
  // in common states (groundOut with R1+R3 dropped R3; bases-loaded groundOut
  // dropped R2; DP with R1+R2 dropped R2) and scored squeeze runs that the
  // official rules nullify. The rule applied throughout: a runner is FORCED
  // only if every base behind him (down to 1B) is occupied by a trailing
  // runner + the batter becoming a runner; forced runners advance, non-forced
  // runners hold, and NO run scores on a play where the 3rd out is a force
  // out or the batter is retired before reaching 1B (Official Rule 5.08(a)).
  // Used by both the live engine and simulateGameInstant (shared function).

  if (type === 'bunt') {
    // Sacrifice bunt: all runners advance one base (the defense concedes the
    // out at 1B). ~25% of the time the batter beats the throw and is safe.
    let nb = [null, null, null], r = 0;
    if (bases[1]) nb[2] = bases[1];
    if (bases[0]) nb[1] = bases[0];
    const batterSafe = Math.random() < 0.25;
    // R3 scores on the squeeze — but if the batter's out at 1B is the 3rd
    // out (a before-reaching-first out), the run is nullified per 5.08(a).
    if (bases[2] && (batterSafe || currentgs.outs < 2)) r++;
    if (batterSafe) { nb[0] = batterName; return { nextBases: nb, runs: r, outsAdded: 0 }; }
    return { nextBases: nb, runs: r, outsAdded: 1 };
  }

  if (type === 'groundOut') {
    if (bases[0]) {
      // Fielder's choice: the force at 2B retires R1; the batter reaches 1B.
      // Remaining FORCED runners advance one base; non-forced runners hold.
      nextBases[0] = batterName;
      if (bases[1]) {
        nextBases[2] = bases[1]; // R2 forced to 3B (R1 + batter behind him)
        // Bases loaded: R3 is forced home. The run counts unless the force
        // at 2B is the 3rd out (no run can score on an inning-ending force).
        if (bases[2] && currentgs.outs < 2) runs = 1;
      } else if (bases[2]) {
        nextBases[2] = bases[2]; // R1+R3: R3 is NOT forced — he holds at 3B
      }
      return { nextBases, runs, outsAdded: 1 };
    } else {
      // No force anywhere: batter is thrown out at 1B. R3 scores on the
      // contact play with < 2 outs (existing behavior); R2 holds.
  ...
  if (type === 'doublePlay') {
    if (bases[0]) {
      // Conventional 6-4-3/4-6-3 turn: R1 forced at 2B + batter out at 1B.
      // Other forced runners advance during the turn; non-forced hold.
      if (bases[1]) {
        nextBases[2] = bases[1]; // R2 forced to 3B
        // Bases loaded: R3 crosses on the turn. With 0 outs the run counts
        // (the DP makes only the 2nd out before the batter is retired —
        // but the batter's out at 1B IS the 3rd-out force when outs === 1,
        // so the run is nullified in that case per 5.08(a)).
        if (bases[2] && currentgs.outs === 0) runs = 1;
      } else if (bases[2]) {
        nextBases[2] = bases[2]; // R1+R3: R3 is NOT forced — he holds at 3B
      }
      return { nextBases, runs, outsAdded: 2 };
    }
  ...
  // Foul-territory sacrifice fly (Backlog #156) — on foulOut outcomes with a
  // runner on 3B and < 2 outs, ~25% chance the catch is deep enough down the
  // line that the runner tags up and scores. Real-MLB rule applies to any
  // fair-or-foul caught fly: a runner may tag from any base after the catch.
  // Mirrors the existing flyOut sac-fly pathway (isFoulSacFly flag, runs: 1,
  // nb[2] cleared, outsAdded: 1). Used by both live + simulateGameInstant.
  if (type === 'foulOut') {
    if (bases[2] && currentgs.outs < 2 && Math.random() < 0.25) {
      const nb = [...bases];
      nb[2] = null;
      return { nextBases: nb, runs: 1, outsAdded: 1, isFoulSacFly: true };
    }
    return { nextBases: [...bases], runs: 0, outsAdded: 1 };
  }
...
// Canonical team-color identity (Backlog #169). The gameplay engine keys every
// runner/fielder color off the team INDEX: index 0 = away team = red, index 1 =
// home team = blue (away bats in the top, half === 0; home bats in the bottom,
// half === 1). Keep all home/away/relief/walk-off color derivations pointed at
// this single source of truth so they never invert relative to the field.
const TEAM_COLORS = ['#b91c1c', '#1e40af']; // [away=red, home=blue]
...
function playSoundLocal(type) {
  try {
    ...
    if (type === 'cardFlip') {
      ...
      // Cap flies off (Backlog #158) — soft whoosh of fabric flipping through
      // the air. Two-stage layer over ~200ms: (1) a tonal-ish whoosh (~180→
      // 280 Hz rising sweep with filtered noise, 130ms — the cap separating
      // from the head and spinning through the air), and (2) a soft cotton
      // landing thud (~110Hz body, 90ms in — the fabric hitting the grass).
      // Intentionally very quiet (peak gain 0.10) so it reads as a subtle
      // visual emphasizer — matching the existing tipCap / freshBall
      // subtle-gesture sound register.
      const cfDur = 0.13, cfB = c.createBuffer(1, Math.ceil(c.sampleRate * cfDur), c.sampleRate), cfD = cfB.getChannelData(0);
      for (let i = 0; i < cfD.length; i++) {
        const t = i / c.sampleRate;
        const env = Math.sin((t / cfDur) * Math.PI); // bell-shaped envelope
        const freq = 180 + 100 * (t / cfDur);
        cfD[i] = ((Math.random() * 2 - 1) * 0.18 + Math.sin(t * freq) * 0.08) * env;
      }
      const cfS = c.createBufferSource(); cfS.buffer = cfB;
      const cfG = c.createGain();
      cfG.gain.setValueAtTime(0.0001, n); cfG.gain.linearRampToValueAtTime(0.10, n + 0.02);
      cfG.gain.exponentialRampToValueAtTime(0.001, n + cfDur);
      cfS.connect(cfG); cfG.connect(c.destination); cfS.start(n);
      // Stage 2: soft cotton landing thud at +130ms (the fabric touching grass)
      const ctStart = n + 0.13;
      const ctDur = 0.07, ctB = c.createBuffer(1, Math.ceil(c.sampleRate * ctDur), c.sampleRate), ctD = ctB.getChannelData(0);
      for (let i = 0; i < ctD.length; i++) {
        const t = i / c.sampleRate;
        ctD[i] = (Math.random() * 2 - 1) * Math.exp(-t / 0.015) * 0.14
              + Math.sin(t * 110) * Math.exp(-t / 0.03) * 0.10;
      }
      const ctS = c.createBufferSource(); ctS.buffer = ctB;
      const ctG = c.createGain();
      ctG.gain.setValueAtTime(0.10, ctStart); ctG.gain.exponentialRampToValueAtTime(0.001, ctStart + ctDur);
      ctS.connect(ctG); ctG.connect(c.destination); ctS.start(ctStart);
    } else if (type === 'stretchDouble') {
      // Stretched single → double (Backlog #159) — the batter rounds first
      // hard, sprints to second, and slides in. Three-stage layer over ~700ms:
      // (1) a sharp wood-on-ball crack at t=0 (~620Hz transient, 60ms — the
      // line-drive contact), (2) pounding footfalls 80ms in (~155Hz body
      // pulsed by a 6Hz envelope for distinct stomps, 380ms — the batter
      // sprinting to second), and (3) a final low slide-thump at +520ms
      // (~135Hz body, 120ms — the headfirst dive into the bag). Distinct
      // from `infieldSingle` (similar footfalls but with a late glove-pop)
      // and from a regular `hit` (sharper bat-crack alone, no footfalls).
      // Stage 1: sharp bat-on-ball crack
      const scDur = 0.06, scB = c.createBuffer(1, Math.ceil(c.sampleRate * scDur), c.sampleRate), scD = scB.getChannelData(0);
      for (let i = 0; i < scD.length; i++) {
        const t = i / c.sampleRate;
        scD[i] = (Math.random() * 2 - 1) * Math.exp(-t / 0.012) * 0.24
              + Math.sin(t * 620) * Math.exp(-t / 0.024) * 0.22;
      }
      const scS = c.createBufferSource(); scS.buffer = scB; const scG = c.createGain();
      scG.gain.setValueAtTime(0.36, n); scG.gain.exponentialRampToValueAtTime(0.001, n + scDur);
      scS.connect(scG); scG.connect(c.destination); scS.start(n);
      // Stage 2: pounding footfalls 80ms in (sprinting to 2B)
      const ffStart = n + 0.08;
      const ffDur = 0.38, ffB = c.createBuffer(1, Math.ceil(c.sampleRate * ffDur), c.sampleRate), ffD = ffB.getChannelData(0);
      for (let i = 0; i < ffD.length; i++) {
        const t = i / c.sampleRate;
        const stomp = Math.max(0, Math.sin(t * 38)); // pulse at ~6Hz for distinct footfalls
        ffD[i] = ((Math.random() * 2 - 1) * 0.18 + Math.sin(t * 155) * 0.14) * stomp;
      }
      const ffS = c.createBufferSource(); ffS.buffer = ffB; const ffG = c.createGain();
      ffG.gain.setValueAtTime(0.0001, ffStart); ffG.gain.linearRampToValueAtTime(0.24, ffStart + 0.04);
      ffG.gain.exponentialRampToValueAtTime(0.001, ffStart + ffDur);
      ffS.connect(ffG); ffG.connect(c.destination); ffS.start(ffStart);
      // Stage 3: low slide-thump at +520ms (the headfirst dive into 2B)
      const stStart = n + 0.52;
      const stDur = 0.12, stB = c.createBuffer(1, Math.ceil(c.sampleRate * stDur), c.sampleRate), stD = stB.getChannelData(0);
      for (let i = 0; i < stD.length; i++) {
        const t = i / c.sampleRate;
        stD[i] = (Math.random() * 2 - 1) * Math.exp(-t / 0.04) * 0.22
              + Math.sin(t * 135) * Math.exp(-t / 0.07) * 0.18;
      }
      const stS = c.createBufferSource(); stS.buffer = stB; const stG = c.createGain();
      stG.gain.setValueAtTime(0.30, stStart); stG.gain.exponentialRampToValueAtTime(0.001, stStart + stDur);
      stS.connect(stG); stG.connect(c.destination); stS.start(stStart);
    } else if (type === 'ofWarmup') {
      // Outfielders warm-up tosses between innings (Backlog #160) — three
      // crisp leather mitt-pops spaced ~950ms apart, capturing the back-and-
      // forth toss between two outfielders during the inning summary card.
      // Each pop is ~110ms with a sharp tonal transient at ~520Hz (the
      // ball into the leather) layered with a brief noise tick. Cap gain
      // 0.10 — intentionally quiet so it reads as ambient between-innings
      // ritual without competing with the inning-summary card or organ
      // CHARGE riff that may also fire at the same transition.
      [0, 0.95, 1.9].forEach((start) => {
        const wuDur = 0.11, wuB = c.createBuffer(1, Math.ceil(c.sampleRate * wuDur), c.sampleRate), wuD = wuB.getChannelData(0);
        for (let i = 0; i < wuD.length; i++) {
          const t = i / c.sampleRate;
          wuD[i] = (Math.random() * 2 - 1) * Math.exp(-t / 0.018) * 0.14
                 + Math.sin(t * 520) * Math.exp(-t / 0.04) * 0.20;
        }
        const wuS = c.createBufferSource(); wuS.buffer = wuB;
        const wuG = c.createGain();
        wuG.gain.setValueAtTime(0.0001, n + start);
        wuG.gain.linearRampToValueAtTime(0.10, n + start + 0.012);
        wuG.gain.exponentialRampToValueAtTime(0.001, n + start + wuDur);
        wuS.connect(wuG); wuG.connect(c.destination); wuS.start(n + start);
      });
    } else if (type === 'lineOutDP') {
      // Doubled off on a line drive (Backlog #161) — the unmistakable sound of
      // a line-drive double play. Three-stage layer over ~700ms: (1) a sharp
      // wood-on-ball crack at t=0 (~580Hz transient + noise burst, 70ms — the
      // line-drive contact); (2) a crisp mitt-pop at +260ms (~660Hz tonal pop
      // with a brief noise tick, ~110ms — the fielder snares the rope); (3) a
      // second crisp glove-pop at +560ms at the bag (~720Hz pop, ~110ms — the
      // throw arriving to double off the runner). Distinct from `doublePlayTurn`
      // (which is mid-frequency mitt pops for a ground-ball turn) — this one
      // leads with the sharp wood crack to convey the line-drive nature.
      // Stage 1: bat-on-ball crack
      const lcDur = 0.07, lcB = c.createBuffer(1, Math.ceil(c.sampleRate * lcDur), c.sampleRate), lcD = lcB.getChannelData(0);
      for (let i = 0; i < lcD.length; i++) {
        const t = i / c.sampleRate;
        lcD[i] = (Math.random() * 2 - 1) * Math.exp(-t / 0.012) * 0.26
              + Math.sin(t * 580) * Math.exp(-t / 0.028) * 0.22;
      }
      const lcS = c.createBufferSource(); lcS.buffer = lcB; const lcG = c.createGain();
      lcG.gain.setValueAtTime(0.36, n); lcG.gain.exponentialRampToValueAtTime(0.001, n + lcDur);
      lcS.connect(lcG); lcG.connect(c.destination); lcS.start(n);
      // Stage 2: mitt-pop catch at +260ms
      const mpStart = n + 0.26, mpDur = 0.11;
      const mpB = c.createBuffer(1, Math.ceil(c.sampleRate * mpDur), c.sampleRate), mpD = mpB.getChannelData(0);
      for (let i = 0; i < mpD.length; i++) {
        const t = i / c.sampleRate;
        mpD[i] = (Math.random() * 2 - 1) * Math.exp(-t / 0.02) * 0.18
              + Math.sin(t * 660) * Math.exp(-t / 0.04) * 0.22;
      }
      const mpS = c.createBufferSource(); mpS.buffer = mpB; const mpG = c.createGain();
      mpG.gain.setValueAtTime(0.30, mpStart); mpG.gain.exponentialRampToValueAtTime(0.001, mpStart + mpDur);
      mpS.connect(mpG); mpG.connect(c.destination); mpS.start(mpStart);
      // Stage 3: throw-to-bag glove-pop at +560ms
      const dgStart = n + 0.56, dgDur = 0.11;
      const dgB = c.createBuffer(1, Math.ceil(c.sampleRate * dgDur), c.sampleRate), dgD = dgB.getChannelData(0);
      for (let i = 0; i < dgD.length; i++) {
        const t = i / c.sampleRate;
        dgD[i] = (Math.random() * 2 - 1) * Math.exp(-t / 0.02) * 0.20
              + Math.sin(t * 720) * Math.exp(-t / 0.038) * 0.24;
      }
      const dgS = c.createBufferSource(); dgS.buffer = dgB; const dgG = c.createGain();
      dgG.gain.setValueAtTime(0.32, dgStart); dgG.gain.exponentialRampToValueAtTime(0.001, dgStart + dgDur);
      dgS.connect(dgG); dgG.connect(c.destination); dgS.start(dgStart);
    } else if (type === 'fouledOffFoot') {
      // Foul ball off the batter's foot (Backlog #162) — two-stage layer over
      // ~280ms capturing the sharp tick + dull body-thud of a foul that
      // ricochets directly off the batter's shoe/shin. Stage 1: sharp high-
      // frequency tick (~1100Hz, the bat tip making contact with the ball
      // — 40ms, similar to screenHit's lead transient). Stage 2: dull low-
      // frequency thud (~140Hz body + noise burst, the ball hitting the
      // foot/leg — 220ms, starts +60ms after the tick). Distinct from
      // screenHit (tick + metallic clatter) and hbpThud (low body + tonal
      // thump at 310Hz).
      // Stage 1: sharp tick (40ms)
      const offTickDur = 0.04;
      const offTickB = c.createBuffer(1, Math.ceil(c.sampleRate * offTickDur), c.sampleRate);
      const offTickD = offTickB.getChannelData(0);
      for (let i = 0; i < offTickD.length; i++) {
        const t = i / c.sampleRate;
        offTickD[i] = (Math.random() * 2 - 1) * Math.exp(-t / 0.005) * 0.30
                    + Math.sin(t * 1100) * Math.exp(-t / 0.02) * 0.26;
      }
      const offTickS = c.createBufferSource(); offTickS.buffer = offTickB; const offTickG = c.createGain();
      offTickG.gain.setValueAtTime(0.30, n); offTickG.gain.exponentialRampToValueAtTime(0.001, n + offTickDur);
      offTickS.connect(offTickG); offTickG.connect(c.destination); offTickS.start(n);
      // Stage 2: low body thud (220ms, starts +60ms after the tick)
      const offThudStart = n + 0.06;
      const offThudDur = 0.22;
      const offThudB = c.createBuffer(1, Math.ceil(c.sampleRate * offThudDur), c.sampleRate);
      const offThudD = offThudB.getChannelData(0);
      for (let i = 0; i < offThudD.length; i++) {
        const t = i / c.sampleRate;
        // Low body tone for the foot impact + a quick noise burst layered on top
        const env = Math.exp(-t / 0.14);
        offThudD[i] = (Math.sin(t * 140 * 2 * Math.PI) * 0.40
                    + (Math.random() * 2 - 1) * 0.16) * env;
      }
      const offThudS = c.createBufferSource(); offThudS.buffer = offThudB; const offThudG = c.createGain();
      offThudG.gain.setValueAtTime(0.0001, offThudStart); offThudG.gain.linearRampToValueAtTime(0.26, offThudStart + 0.02);
      offThudG.gain.exponentialRampToValueAtTime(0.001, offThudStart + offThudDur);
      offThudS.connect(offThudG); offThudG.connect(c.destination); offThudS.start(offThudStart);
    } else if (type === 'pitcherBackup') {
      // Pitcher backs up home plate (Backlog #163) — soft pounding-footfall
      // cue for the pitcher hustling from the mound to back up the catcher
      // on outfield-assist gun-downs at home. Single-stage layer over ~420ms:
      // a low-frequency body tone (~155Hz) pulsed by a 5Hz envelope produces
      // 2-3 distinct stomps over the run, with a quiet noise wash for the
      // cleat-on-grass texture. Intentionally muted (cap gain 0.10) so it
      // reads as a foreground ritual matching the existing tipCap / freshBall
      // subtle-gesture register — the focus stays on the OF throw / catcher
      // tag, not the backup hustle.
      const pbDur = 0.42;
      const pbB = c.createBuffer(1, Math.ceil(c.sampleRate * pbDur), c.sampleRate);
      const pbD = pbB.getChannelData(0);
      for (let i = 0; i < pbD.length; i++) {
        const t = i / c.sampleRate;
        // 5Hz pulse envelope produces ~2 distinct stomps over 420ms
        const pulse = Math.max(0, Math.sin(t * 5 * 2 * Math.PI));
        const env = Math.sin((t / pbDur) * Math.PI); // overall bell-shaped wrapper
        pbD[i] = (Math.sin(t * 155 * 2 * Math.PI) * 0.28 * pulse
               + (Math.random() * 2 - 1) * 0.07) * env;
      }
      const pbS = c.createBufferSource(); pbS.buffer = pbB; const pbG = c.createGain();
      pbG.gain.setValueAtTime(0.0001, n); pbG.gain.linearRampToValueAtTime(0.10, n + 0.05);
      pbG.gain.exponentialRampToValueAtTime(0.001, n + pbDur);
      pbS.connect(pbG); pbG.connect(c.destination); pbS.start(n);
    } else if (type === 'rubberKick') {
      // Pitcher kicks the rubber (Backlog #157) — two soft brushy dirt-scrapes
      // captured as filtered-noise scuffs ~260ms apart, matching the two-pass
      // foot-scrape animation. Each scrape is ~140ms with a slight rising
      // tonal mid-frequency component (~160→220Hz) to suggest the cleats
      // dragging across the dirt and the rubber's hard surface. Intentionally
      // very quiet (peak gain 0.11) so it reads as quiet foreground ritual,
      // matching the existing browWipe / tipCap / freshBall subtle-gesture
      // sound register.
      const rkDur = 0.14;
      [0, 0.26].forEach((start) => {
        const rkB = c.createBuffer(1, Math.ceil(c.sampleRate * rkDur), c.sampleRate);
        const rkD = rkB.getChannelData(0);
        for (let i = 0; i < rkD.length; i++) {
          const t = i / c.sampleRate;
          const env = Math.sin((t / rkDur) * Math.PI); // bell-shaped envelope
          const sweep = (160 + 60 * (t / rkDur));
          rkD[i] = ((Math.random() * 2 - 1) * 0.22
                 + Math.sin(t * sweep) * 0.08) * env;
        }
        const rkS = c.createBufferSource(); rkS.buffer = rkB;
        const rkG = c.createGain();
        rkG.gain.setValueAtTime(0.0001, n + start);
        rkG.gain.linearRampToValueAtTime(0.11, n + start + 0.03);
        rkG.gain.exponentialRampToValueAtTime(0.001, n + start + rkDur);
        rkS.connect(rkG); rkG.connect(c.destination); rkS.start(n + start);
      });
    ...
    } else if (type === 'checkSwing') {
      // Check-swing appeal (Backlog #150) — a two-stage cue capturing the
      // moment a check swing goes to the base umpire. (1) A short, soft
      // "questioning" upward tone at ~0.0s (the catcher gesturing down the
      // line — "did he go?"), a 320→520Hz triangle sweep over 200ms, followed
      // by (2) a crisp verdict "pop" at +260ms (a bright filtered-noise +
      // tonal transient at ~600Hz — the base umpire's emphatic signal coming
      // down the line). Intentionally compact and percussive so it reads as a
      // quick officiating beat, distinct from the heavier crowd-reaction
      // bursts and the leather glove-pops.
      const qO = c.createOscillator(), qG = c.createGain();
      qO.type = 'triangle';
      qO.frequency.setValueAtTime(320, n);
      qO.frequency.linearRampToValueAtTime(520, n + 0.20);
      qG.gain.setValueAtTime(0.0001, n);
      qG.gain.linearRampToValueAtTime(0.10, n + 0.03);
      qG.gain.exponentialRampToValueAtTime(0.001, n + 0.22);
      qO.connect(qG); qG.connect(c.destination); qO.start(n); qO.stop(n + 0.26);
      // Verdict pop — sharp tonal transient layered with a brief noise tick.
      const vStart = n + 0.26;
      const vDur = 0.12, vB = c.createBuffer(1, Math.ceil(c.sampleRate * vDur), c.sampleRate), vD = vB.getChannelData(0);
      for (let i = 0; i < vD.length; i++) {
        const t = i / c.sampleRate;
        vD[i] = (Math.sin(t * 600 * Math.PI * 2) * Math.exp(-t / 0.04) * 0.5
              + (Math.random() * 2 - 1) * Math.exp(-t / 0.02) * 0.30);
      }
      const vS = c.createBufferSource(); vS.buffer = vB; const vG = c.createGain();
      vG.gain.setValueAtTime(0.20, vStart); vG.gain.exponentialRampToValueAtTime(0.001, vStart + vDur);
      vS.connect(vG); vG.connect(c.destination); vS.start(vStart);
    } else if (type === 'pitchout') {
      // Pitchout (Backlog #155) — a two-stage cue capturing the moment the
      // catcher stands up to receive an intentionally wide pitch. (1) A soft
      // rising whoosh (~280→480Hz triangle over 220ms — the pitch sailing
      // outside the strike zone toward the standing catcher), followed by
      // (2) a crisp tonal mitt-pop at +240ms (~620Hz with a brief noise tick
      // — the ball into the catcher's glove). Distinct from `brushback`
      // (chin-high whistle) and `frameMitt` (subtle leather creak) — this is
      // a clear "the pitch was thrown wide on purpose" cue, intentionally
      // crisp so it telegraphs the defensive intent.
      const poO = c.createOscillator(), poG = c.createGain();
      poO.type = 'triangle';
      poO.frequency.setValueAtTime(280, n);
      poO.frequency.linearRampToValueAtTime(480, n + 0.22);
      poG.gain.setValueAtTime(0.0001, n);
      poG.gain.linearRampToValueAtTime(0.12, n + 0.04);
      poG.gain.exponentialRampToValueAtTime(0.001, n + 0.24);
      poO.connect(poG); poG.connect(c.destination); poO.start(n); poO.stop(n + 0.26);
      // Mitt pop — sharp tonal transient layered with a brief noise tick.
      const mpStart = n + 0.24;
      const mpDur = 0.10, mpB = c.createBuffer(1, Math.ceil(c.sampleRate * mpDur), c.sampleRate), mpD = mpB.getChannelData(0);
      for (let i = 0; i < mpD.length; i++) {
        const t = i / c.sampleRate;
        mpD[i] = (Math.sin(t * 620 * Math.PI * 2) * Math.exp(-t / 0.035) * 0.55
              + (Math.random() * 2 - 1) * Math.exp(-t / 0.015) * 0.28);
      }
      const mpS = c.createBufferSource(); mpS.buffer = mpB; const mpG = c.createGain();
      mpG.gain.setValueAtTime(0.22, mpStart); mpG.gain.exponentialRampToValueAtTime(0.001, mpStart + mpDur);
      mpS.connect(mpG); mpG.connect(c.destination); mpS.start(mpStart);
    } else if (type === 'windGust') {
      // Wind gust (Backlog #152) — a soft whooshing swell that accompanies a
      // wind-affected ball flight (a fly ball carried out by the wind, or a
      // would-be homer knocked down at the track). Smoothed (one-pole low-
      // passed) noise with a slow rise-and-fall gain envelope reads as a gust
      // sweeping through the park — intentionally airy and unpitched so it
      // sits underneath the crowd reaction rather than competing with it.
      const wDur = 1.1, wB = c.createBuffer(1, Math.ceil(c.sampleRate * wDur), c.sampleRate), wD = wB.getChannelData(0);
      let lp = 0;
      for (let i = 0; i < wD.length; i++) {
        // One-pole lowpass over white noise → "whoosh" rather than "hiss".
        lp += ((Math.random() * 2 - 1) - lp) * 0.045;
        // Slow tremolo gives the gust a swirling character.
        const t = i / c.sampleRate;
        wD[i] = lp * (0.75 + 0.25 * Math.sin(t * 9));
      }
      const wS = c.createBufferSource(); wS.buffer = wB; const wG = c.createGain();
      wG.gain.setValueAtTime(0.0001, n);
      wG.gain.linearRampToValueAtTime(0.34, n + 0.38);
      wG.gain.linearRampToValueAtTime(0.18, n + 0.72);
      wG.gain.exponentialRampToValueAtTime(0.001, n + wDur);
      wS.connect(wG); wG.connect(c.destination); wS.start(n);
    } else if (type === 'mascotDance') {
      // Mascot dance (Backlog #153) — a goofy, bouncy organ-style riff for the
      // mascot popping up on the dugout roof: six quick square-wave notes
      // (a major-triad hop up and back down — circus-organ flavor) with a soft
      // crowd-whoop noise wash underneath. Kept light and short so it reads as
      // a playful between-innings beat, distinct from the organ CHARGE riff.
      const riff = [
        { f: 392.0, at: 0.00 },  // G4
        { f: 493.9, at: 0.16 },  // B4
        { f: 587.3, at: 0.32 },  // D5
      ...
      riff.forEach(({ f, at }) => {
        const o = c.createOscillator(), g = c.createGain();
        o.type = 'square';
        o.frequency.setValueAtTime(f, n + at);
        g.gain.setValueAtTime(0.0001, n + at);
        g.gain.linearRampToValueAtTime(0.055, n + at + 0.02);
        g.gain.exponentialRampToValueAtTime(0.001, n + at + 0.15);
        o.connect(g); g.connect(c.destination); o.start(n + at); o.stop(n + at + 0.18);
      });
      // Light crowd-whoop wash under the riff — the fans enjoying the bit.
      const mDur = 1.2, mB = c.createBuffer(1, Math.ceil(c.sampleRate * mDur), c.sampleRate), mD = mB.getChannelData(0);
      let mlp = 0;
      for (let i = 0; i < mD.length; i++) {
        mlp += ((Math.random() * 2 - 1) - mlp) * 0.06;
        mD[i] = mlp;
      }
      const mS = c.createBufferSource(); mS.buffer = mB; const mG = c.createGain();
      mG.gain.setValueAtTime(0.0001, n);
      mG.gain.linearRampToValueAtTime(0.16, n + 0.35);
      mG.gain.exponentialRampToValueAtTime(0.001, n + mDur);
      mS.connect(mG); mG.connect(c.destination); mS.start(n);
    }
  } catch (e) {}
    ...
    <div className="flex items-start gap-8 justify-center flex-wrap p-2 relative font-sans min-h-[140px]">
      ...
      <div className="flex flex-col items-center flex-shrink-0">
        <div className="relative w-28 h-44 bg-gradient-to-b from-gray-600 to-gray-800 rounded-t-2xl rounded-b-lg border-x-4 border-t-4 border-gray-400 shadow-2xl flex flex-col items-center overflow-hidden">
          ...
          {dispensedCard && ( <div style={{ transitionTimingFunction: 'cubic-bezier(0.17, 0.67, 0.83, 0.67)' }} className={`absolute left-4 z-10 transition-all duration-500 ${dispensing ? 'top-24 scale-100' : 'top-16 scale-75 opacity-0'}`}><MiniCard card={dispensedCard} /></div> )}
        </div>
      </div>
...
function SVGPitcher({ x, y, color = '#1e40af', phase = 'idle', fromStretch = false, headShake = false, headNod = false }) {
  ...
  // Rubber-kick pose (Backlog #157) — pre-pitch atmospheric flavor where the
  // pitcher scrapes his back foot along the rubber to groom the dirt before
  // setting up. Real-MLB pitchers do this constantly between pitches. The
  // body stays mostly upright with a slight forward lean, both arms hang at
  // the sides relaxed (the glove arm holds the ball), and the back leg
  // animates a back-and-forth scraping motion via <animate> on the foot's
  // x2 endpoint. A small tan dust-puff ellipse fades in and out under the
  // foot at each scrape pass to sell the groove. Pure cosmetic — no
  // mechanical effect on the pitch, no card drawn, no count change.
  if (phase === 'rubberKick') {
    return (
      <g key="rubberKick" transform={`translate(${x},${y})`}>
        <ellipse cx={0} cy={14} rx={6} ry={2.5} fill="rgba(0,0,0,.12)" />
        {/* Body — slight forward lean as the pitcher looks down at his foot */}
        <line x1={0} y1={0} x2={0.5} y2={8} stroke={color} strokeWidth={2.8} strokeLinecap="round" />
        {/* Front leg — planted, slightly bent */}
        <line x1={0.5} y1={8} x2={3} y2={14} stroke={color} strokeWidth={2.2} strokeLinecap="round" />
        {/* Back leg — animates back-and-forth scrape across the rubber.
            The foot endpoint sweeps from -5 to -1 (back-and-forth, two passes)
            on the x axis while the y stays near the ground. */}
        <line x1={0.5} y1={8} x2={-5} y2={14} stroke={color} strokeWidth={2.2} strokeLinecap="round">
          <animate attributeName="x2" values="-5;-1;-5;-1;-5" keyTimes="0;0.25;0.5;0.75;1" dur="0.7s" fill="freeze" />
          <animate attributeName="y2" values="14;13.4;14;13.4;14" keyTimes="0;0.25;0.5;0.75;1" dur="0.7s" fill="freeze" />
        </line>
        {/* Glove arm — hangs at side, holding the ball */}
        <line x1={0.5} y1={2} x2={-4} y2={6} stroke={color} strokeWidth={2} strokeLinecap="round" />
        <circle cx={-4.5} cy={6.5} r={2.2} fill="#7a3a12" stroke="#3a1a08" strokeWidth={0.5} />
        {/* Throwing arm — relaxed at the side */}
        <line x1={0.5} y1={2} x2={4} y2={6} stroke={color} strokeWidth={2} strokeLinecap="round" />
        {/* Head + cap — tilted slightly down looking at the rubber */}
        <g>
          <animateTransform attributeName="transform" type="translate"
            values="0,0; 0,0.5; 0,0; 0,0.5; 0,0" keyTimes="0;0.25;0.5;0.75;1" dur="0.7s" fill="freeze" />
          <circle cx={0} cy={-5} r={5} fill="#f5d0a0" />
          <path d={`M-5.5,-6 Q0,-13 5.5,-6`} fill={color} />
        </g>
        {/* Dust puff under the foot — fades in/out twice as the foot scrapes,
            with rx pulsing slightly larger on each pass. */}
        <ellipse cx={-3} cy={14.5} rx={2.4} ry={0.9} fill="#c9b59a" opacity="0">
          <animate attributeName="opacity" values="0;0.55;0;0.55;0" keyTimes="0;0.2;0.45;0.7;1" dur="0.7s" fill="freeze" />
          <animate attributeName="rx" values="1.8;2.8;1.8;2.8;1.8" keyTimes="0;0.2;0.45;0.7;1" dur="0.7s" fill="freeze" />
        </ellipse>
      </g>
    );
  }
...
function SVGBatter({ x, y, color = '#dc2626', phase = 'stance', side = 'R', chokeUp = false }) {
  ...
  // component.
  const applyChokeUp = chokeUp && !isBatFlip && !isStepOut && !isHbp && !isBrushback && !isCleatKnock && !isTipCapFan && !isOffFoot;
  ...
  // Fouled-off-foot pose (Backlog #162) — batter just fouled a pitch directly
  // off his foot/shin. He winces, lifts the affected (front) foot off the
  // ground in a brief hop, leans on the bat barrel for support, and shakes
  // it off. Pure cosmetic recovery — distinct from HBP (which is the body
  // recoiling away from a thrown ball, with arms curled protectively). The
  // hop is a two-stage cycle: front foot lifts ~3px, holds, comes back
  // down, all over ~1.0s. Animated via SMIL <animate> on the foot's y2
  // endpoint so the leg visibly hops without the whole body translating.
  if (isOffFoot) {
    return (
      <g key={`off-foot-${side}`} transform={`translate(${x},${y})`}>
        <ellipse cx={0} cy={14} rx={5} ry={2} fill="rgba(0,0,0,.12)" />
        {/* Body — slightly hunched forward in pain */}
        <line x1={0} y1={0} x2={1 * f} y2={9} stroke={color} strokeWidth={2.8} strokeLinecap="round" />
        {/* Back leg — bears all the weight, planted firmly */}
        <line x1={1 * f} y1={9} x2={-3 * f} y2={14} stroke={color} strokeWidth={2.2} strokeLinecap="round" />
        {/* Front leg — hopping in pain, two-stage lift cycle over 1.0s */}
        <line x1={1 * f} y1={9} x2={4 * f} y2={14} stroke={color} strokeWidth={2.2} strokeLinecap="round">
          <animate attributeName="y2" values="14;10;11;10;14" keyTimes="0;0.25;0.5;0.75;1" dur="1.0s" fill="freeze" />
        </line>
        {/* Both arms leaning forward and down — gripping the bat handle for support */}
        <line x1={0} y1={2} x2={3 * f} y2={6} stroke={color} strokeWidth={2} strokeLinecap="round" />
        <line x1={0} y1={2} x2={5 * f} y2={5} stroke={color} strokeWidth={2} strokeLinecap="round" />
        {/* Bat planted barrel-down on the ground, used as a crutch for support */}
        <line x1={5 * f} y1={5} x2={6 * f} y2={14} stroke="#b8860b" strokeWidth={2.5} strokeLinecap="round" />
        {/* Head — tilted down looking at the hurt foot, in a grimace */}
        <circle cx={0} cy={-3} r={5} fill="#f5d0a0" />
        <path d={`M${-5 * f},-4 Q${0 * f},-11 ${5 * f},-4`} fill={color} />
        {/* Small "ouch" grimace mouth line */}
        <line x1={-1.5 * f} y1={-2} x2={1.5 * f} y2={-2} stroke="#000" strokeWidth={0.6} strokeLinecap="round" opacity={0.55} />
        {/* Pain-stars puff above the head — three small dots flashing then fading */}
        <circle cx={-3 * f} cy={-11} r={0.8} fill="#fbbf24" opacity="0">
          <animate attributeName="opacity" values="0;0.8;0.5;0" keyTimes="0;0.18;0.55;1" dur="1.0s" fill="freeze" />
        </circle>
        <circle cx={2 * f} cy={-12} r={0.9} fill="#fbbf24" opacity="0">
          <animate attributeName="opacity" values="0;0.8;0.4;0" keyTimes="0;0.22;0.55;1" dur="1.0s" fill="freeze" />
        </circle>
        <circle cx={5 * f} cy={-10} r={0.7} fill="#fbbf24" opacity="0">
          <animate attributeName="opacity" values="0;0.7;0.4;0" keyTimes="0;0.28;0.6;1" dur="1.0s" fill="freeze" />
        </circle>
      </g>
    );
  }
...
function SVGCatcher({ x, y, color = '#1e40af', isMoving = false, signs = 0, lookingUp = false, blocking = false, framing = false, framingKey = 0 }) {
  ...
  return (
    <g key={`rosin-${rosinKey}`} transform={`translate(${x},${y})`}>
      ...
      {/* Chalk puff — three expanding circles that bloom at the clap moment.
          Offset down by +12 to sit at the rosin bag (translate 0,12), where the
          pitcher is bent over clapping at 0.40–0.72 of the cycle; without the
          offset the puff bloomed up at the rubber, ~12px above the hands. */}
  ...
  return (
    <g transform={`translate(${x},${y})`}>
      ...
      <g>
        ...
        {/* Weighted training donut (Backlog #179) — the classic warm-up ring
            slid onto the barrel. Sits ~65% up the bat (between the hands at
            (6f,1) and the barrel tip at (18f,-6)) and, because it lives inside
            the rotating arm-and-bat group, rides through the whole swing arc
            with the bat. Drawn as a dark ring the bat passes through:
            translucent fill for heft plus a solid outline for the donut hole. */}
        <circle cx={13.8 * f} cy={-4} r={2.5} fill="#3a3a3a" opacity={0.3} />
        <circle cx={13.8 * f} cy={-4} r={2.5} fill="none" stroke="#222" strokeWidth={1.6} />
  ...
  return (
    <g>
      {Array.from({ length: count }, (_, i) => {
        // K signs stack into rows of 5; col walks across the fence, kY steps up
        // a row every 5 K's. (Removed two unused locals — an `x` that duplicated
        // this `col` for i<5 and a `y` whose row offset `kY` already computes.)
        const col = startX + (i % 5) * dirX;
...
function RobbedHRLeap({ x, y, color = '#1e40af' }) {
  if (x == null || y == null) return null; // guard nulls, not a legit x=0/y=0 catch point
...
// Outfielder's cap flies off during a great catch (Backlog #158) — pure
// cosmetic SVG: a small color-coded cap (crown circle + brim arc) pops off
// the fielder's head at the catch point, tumbles through the air in a
// parabolic arc (rises ~8px, spins 540°), and lands ~16px away with a
// small dust puff. Fires alongside web gems and robbed HRs. One of
// baseball's most universally recognized "great catch" moments — every
// fan has seen the diving outfielder's cap go flying.
function SVGFlyingCap({ x, y, color = '#1e40af', side = 1 }) {
  if (x == null || y == null) return null;
  // Landing point offset to one side (depends on direction the fielder dove)
  const ldx = 14 * side;
  const ldy = 4;
  // Backlog #170 fix: the cap's shapes are drawn around the LOCAL origin
  // (0,0) and the static position lives in the wrapper g's translate. The
  // rotate animateTransform has no explicit center, so it spins about the
  // local origin — which is now the cap itself, not the SVG's top-left
  // corner (the old absolute-coordinate drawing made the 540° tumble sweep
  // the cap on a ~480px orbit around the canvas origin). Matches the
  // local-origin pattern used by FlyingHelmet / MaskToss / BrokenBat.
  return (
    <g transform={`translate(${x},${y})`}>
      {/* Cap silhouette — rises ~10px to peak, tumbles, lands at +ldx/+ldy
          from start. The transform chain composes the wrapper's static
          position, a translate (parabolic x + y motion), and a rotate
          (the 540° tumble about the cap's own center) over 850ms. */}
      <g>
        <animateTransform attributeName="transform" type="translate"
          values={`0,0; ${ldx * 0.3},-10; ${ldx * 0.7},-6; ${ldx},${ldy}`}
          keyTimes="0;0.35;0.7;1" dur="0.85s" fill="freeze" />
        <g>
          <animateTransform attributeName="transform" type="rotate"
            values="0;180;360;540" keyTimes="0;0.35;0.7;1" dur="0.85s" fill="freeze" />
          {/* Cap crown — small circle in team color */}
          <circle cx={0} cy={-2} r={2.6} fill={color} stroke="#000" strokeWidth={0.35} />
          {/* Cap brim — short arc extending forward */}
          <path d={`M${-2.6},${-1.5} Q0,1.5 3.4,${-0.5}`}
            fill={color} stroke="#000" strokeWidth={0.3} />
          {/* Tiny logo dot on the crown for visual contrast */}
          <circle cx={0} cy={-2.5} r={0.5} fill="#fff" opacity={0.85} />
        </g>
      </g>
      {/* Landing dust puff — small expanding ellipse at the touchdown point,
          delayed to appear when the cap lands at +0.7s */}
      <ellipse cx={ldx} cy={ldy + 1.5} rx={2} ry={0.8} fill="#c9b59a" opacity={0}>
        <animate attributeName="opacity" values="0;0.55;0" keyTimes="0;0.55;1" begin="0.7s" dur="0.55s" fill="freeze" />
        <animate attributeName="rx" values="1;3.5;4.5" keyTimes="0;0.5;1" begin="0.7s" dur="0.55s" fill="freeze" />
      </ellipse>
    </g>
  );
}
...
/**
 * OUTFIELDERS WARM-UP TOSSES BETWEEN INNINGS (Backlog #160)
 * Atmospheric between-innings flavor every real broadcast captures: two
 * outfielders play catch in shallow OF during the inning summary card to
 * stay loose before play resumes. A small white ball travels back and
 * forth via SMIL <animateMotion> along a 3-stop path (from → to → from →
 * to → from) over ~2.8s for three crisp tosses, with subtle parabolic
 * arcs (Q-curve control points lifted above the baseline so the ball
 * reads as airborne rather than rolling). Pure cosmetic — no gameplay
 * effect, no statistical impact. Dims slightly in night mode to read as
 * flood-lit. Fires alongside the inning summary card so it adds zero
 * extra delay — the warmup starts as the summary appears and ends well
 * before the next at-bat begins.
 */
function SVGOFWarmup({ visible, fromPos, toPos, color = '#1e40af', isNightGame = false }) {
  if (!visible || !fromPos || !toPos) return null;
  const totalDur = 2.85;
  const opacity = isNightGame ? 0.78 : 1;
  // Build a back-and-forth motion path with three lift arcs (one per toss).
  // Each leg arcs ~18px above the straight baseline for a natural lob.
  const liftY = Math.min(fromPos.y, toPos.y) - 18;
  const midX = (fromPos.x + toPos.x) / 2;
  const motionPath = [
    `M ${fromPos.x} ${fromPos.y}`,
    `Q ${midX} ${liftY} ${toPos.x} ${toPos.y}`,    // toss 1: from → to
    `Q ${midX} ${liftY} ${fromPos.x} ${fromPos.y}`, // toss 2: to → from
    `Q ${midX} ${liftY} ${toPos.x} ${toPos.y}`,    // toss 3: from → to
  ].join(' ');
  return (
    <g opacity={opacity}>
      {/* Animated baseball that arcs back and forth three times. Tiny
          white circle with a red stitch line to read as a real ball. */}
      <g>
        <animateMotion path={motionPath} dur={`${totalDur}s`} fill="freeze" />
        {/* Fade out at the end of the third toss so the ball disappears
            into the OF's glove rather than freezing mid-air. */}
        <animate attributeName="opacity" values="0;1;1;0" keyTimes="0;0.04;0.92;1" dur={`${totalDur}s`} fill="freeze" />
        <circle cx={0} cy={0} r={2.6} fill="#fff" stroke="#c00" strokeWidth={0.5} />
        {/* Faint red stitch line for the rotating-baseball look */}
        <path d="M -1.6 0 Q 0 -1.2 1.6 0" stroke="#c00" strokeWidth={0.35} fill="none" opacity={0.7} />
      </g>
    </g>
  );
}
...
function ErrorBobble({ x, y }) {
  if (x == null || y == null) return null; // guard nulls, not a legit x=0/y=0 fielder spot
  return (
    <g>
  ...
  return (
    <g>
      ...
      {/* Four sun rays — vertical/horizontal cross pattern stretching outward.
          Spaced 90° apart (0°/90°/180°/270°) so they form an even 4-spoke
          star/cross; the previous i*45 packed all four rays into a single
          quadrant (0/45/90/135), giving a lopsided fan instead of a star. */}
      {[0, 1, 2, 3].map(i => {
        const angle = i * 90;
...
// Wind conditions (Backlog #152): on windy days the cloth whips on a much faster
// ripple cycle, and when the wind is blowing IN the whole flag is mirrored to the
// other side of the pole — the classic broadcaster's cue ("check the flags!").
function StadiumFlag({ isNightGame = false, wind = null }) {
  ...
  // Windy games whip the cloth visibly faster than the gentle calm-day ripple.
  const flagDur = wind ? '1.7s' : '3.4s';
  ...
  return (
    <g opacity={opacityScale}>
      ...
      {/* Flag cloth — mirrored to the home-plate side of the pole when the wind
          is blowing IN (Backlog #152), so the flag itself tells the story. */}
      <g transform={wind === 'in' ? `translate(${px * 2},0) scale(-1,1)` : undefined}>
      {/* Red-and-white striped flag body (bottom half) */}
      <path d={wave1} fill="#c91d1d" stroke="#7a0f0f" strokeWidth={0.3}>
        <animate attributeName="d" values={`${wave1};${wave2};${wave3}`} dur={flagDur} repeatCount="indefinite" />
      </path>
      ...
      <path d={wave1} fill="none" stroke="#ffffff" strokeWidth={1.6} opacity={0.9}>
        <animate attributeName="d" values={`
          ...
          M ${px} ${fy + fh * 0.22} Q ${px + fw * 0.5} ${fy + fh * 0.22 - 1.5} ${px + fw} ${fy + fh * 0.22 + 1}
        `} dur={flagDur} repeatCount="indefinite" />
      </path>
      <path d={wave1} fill="none" stroke="#ffffff" strokeWidth={1.6} opacity={0.9}>
        <animate attributeName="d" values={`
      ...
      <rect x={px + 0.4} y={fy + 0.3} width={fw * 0.42} height={fh * 0.5} fill="#162c6b">
        <animate attributeName="y" values={`${fy + 0.3};${fy + 1.6};${fy + 0.3}`} dur={flagDur} repeatCount="indefinite" />
      </rect>
      ...
      {[0.12, 0.28].map((row, ri) =>
        [0.06, 0.14, 0.22, 0.30].map((col, ci) => (
          <circle key={`star-${ri}-${ci}`} cx={px + fw * col + (ri % 2 ? 2 : 0)} cy={fy + fh * row} r={0.45} fill="#ffffff" opacity={0.95}>
            <animate attributeName="cy" values={`${fy + fh * row};${fy + fh * row + 1.3};${fy + fh * row}`} dur={flagDur} repeatCount="indefinite" />
          </circle>
        ))
      )}
      </g>
...
/**
 * BEACH BALL IN THE STANDS (Backlog #151)
 * Atmospheric crowd moment every fan has seen: a colorful inflatable beach ball
 * appears in the seats and gets batted along the upper deck from fan to fan,
 * bouncing in little parabolic arcs, until an usher reaches up and snatches it
 * away (much to the crowd's mock-disappointment). Shown during some mid/late
 * half-inning transitions. A 6-panel multicolor ball travels left-to-right
 * across the stands via <animateMotion> while spinning in place, then fades as
 * the usher (dark-jacketed silhouette at the field rail) grabs it. A
 * "🏐 BEACH BALL!" banner fades in and out over the field. Lasts ~3.4s before
 * play resumes. Pure cosmetic — no gameplay effect. Dims slightly in night mode
 * to read as flood-lit. Companion to the crowd wave (#78) and fan-souvenir
 * catch (#81) in the stands-atmosphere family.
 */
function BeachBall({ visible, isNightGame = false }) {
  if (!visible) return null;
  const totalDur = 3.4;             // overall visibility window (seconds)
  const travelDur = totalDur * 0.82; // ball is in motion for most of the window
  const opacity = isNightGame ? 0.82 : 1;
  // Zig-zag path across the upper stands. The Q-curve control points lifted
  // above the baseline (~y36-42) create the parabolic "bat-up" arcs as each fan
  // punches the ball along to the next; endpoints sit at the stands baseline
  // (~y56-60). Tuned to the same upper-stands band the crowd wave uses.
  const path = "M 96,58 Q 150,38 206,56 Q 262,36 318,56 Q 374,38 430,56 Q 486,42 532,60";
  // Classic 6-panel multicolor beach ball.
  const wedges = ['#ef4444', '#f59e0b', '#facc15', '#22c55e', '#3b82f6', '#ffffff'];
  const R = 5.4;
  return (
    <g opacity={opacity}>
      {/* "BEACH BALL!" banner — fades in then out over the field */}
      <g transform="translate(310, 40)">
        <rect x={-42} y={-7} width={84} height={14} rx={3} fill="#0b2a4a" stroke="#fde047" strokeWidth={0.7} opacity={0.92}>
          <animate attributeName="opacity" values="0;0.92;0.92;0" keyTimes="0;0.12;0.8;1" dur={`${totalDur}s`} fill="freeze" />
        </rect>
        <text x={0} y={2.4} textAnchor="middle" fill="#fde047" fontSize="8" fontWeight="bold" fontFamily="sans-serif" letterSpacing="0.5">
          🏐 BEACH BALL!
          <animate attributeName="opacity" values="0;1;1;0" keyTimes="0;0.12;0.8;1" dur={`${totalDur}s`} fill="freeze" />
        </text>
      </g>
      {/* The beach ball — travels along the path (animateMotion) while spinning
          in place, then fades out at the end as the usher snatches it. */}
      <g>
        <animateMotion path={path} dur={`${travelDur}s`} fill="freeze" />
        <g>
          {/* Slow spin around the ball's own center as it travels */}
          <animateTransform attributeName="transform" type="rotate" from="0" to="320" dur={`${travelDur}s`} fill="freeze" />
          {/* Fade out near the end (the usher grabs it) */}
          <animate attributeName="opacity" values="1;1;0" keyTimes="0;0.86;1" dur={`${totalDur}s`} fill="freeze" />
          {/* White base disk */}
          <circle cx={0} cy={0} r={R} fill="#ffffff" stroke="#cbd5e1" strokeWidth={0.4} />
          {/* Six colored pie wedges */}
          {wedges.map((col, i) => {
            ...
            return <path key={`bb-w-${i}`} d={`M0,0 L${x0},${y0} A${R},${R} 0 0,1 ${x1},${y1} Z`} fill={col} opacity={0.92} />;
          })}
          {/* Glossy highlight */}
          <circle cx={-R * 0.35} cy={-R * 0.35} r={R * 0.28} fill="#ffffff" opacity={0.55} />
        </g>
      </g>
      {/* Usher / security at the field-side rail below the right end of the path
          — appears late, reaches both arms up, and snags the ball. */}
      <g transform="translate(534, 64)" opacity={0}>
        <animate attributeName="opacity" values="0;0;1;1;0" keyTimes="0;0.7;0.82;0.92;1" dur={`${totalDur}s`} fill="freeze" />
        {/* Shadow at feet */}
        <ellipse cx={0} cy={12} rx={4} ry={1.6} fill="rgba(0,0,0,.18)" />
        {/* Torso — dark usher jacket */}
        <rect x={-2.4} y={0} width={4.8} height={7} rx={1} fill="#1e293b" />
        {/* Head */}
        <circle cx={0} cy={-2.4} r={1.7} fill="#e6c79a" />
        {/* Cap */}
        <path d="M -1.9 -3.2 Q 0 -4.8 1.9 -3.2 L 2.2 -2.4 L -2.2 -2.4 Z" fill="#0f172a" />
        {/* Both arms reaching UP to snag the ball */}
        <line x1={-1.4} y1={1} x2={-3} y2={-5} stroke="#e6c79a" strokeWidth={1} strokeLinecap="round" />
        <line x1={1.4} y1={1} x2={3} y2={-5} stroke="#e6c79a" strokeWidth={1} strokeLinecap="round" />
        {/* Legs */}
        <line x1={-1} y1={7} x2={-1.6} y2={12} stroke="#1e293b" strokeWidth={1.3} strokeLinecap="round" />
        <line x1={1} y1={7} x2={1.6} y2={12} stroke="#1e293b" strokeWidth={1.3} strokeLinecap="round" />
      </g>
    </g>
  );
...
/**
 * MASCOT DANCE ON THE DUGOUT ROOF (Backlog #153)
 * Atmospheric between-innings moment: a big-headed fuzzy team mascot pops up
 * on top of a dugout, rocks side-to-side in a goofy dance with both arms
 * waving and music notes floating up, then ducks back down before play
 * resumes (~3.4s, matching the beach-ball window). Fires during some
 * half-inning transitions in innings 2-8, mutually exclusive with the crowd
 * wave and the beach ball (one stands/atmosphere bit per transition). Pure
 * cosmetic — no gameplay effect. Dims slightly in night mode.
 */
function SVGMascot({ visible, x = 112, isNightGame = false }) {
  if (!visible) return null;
  const totalDur = 3.4;             // overall visibility window (seconds)
  const opacity = isNightGame ? 0.85 : 1;
  const fur = '#16a34a';            // classic fuzzy-green mascot (Phanatic-style)
  const belly = '#86efac';
  const snout = '#f59e0b';
  return (
    <g opacity={opacity}>
      {/* "MASCOT TIME!" banner — fades in then out over the field */}
      <g transform="translate(310, 40)">
        <rect x={-46} y={-7} width={92} height={14} rx={3} fill="#0b2a4a" stroke="#fde047" strokeWidth={0.7} opacity={0.92}>
          <animate attributeName="opacity" values="0;0.92;0.92;0" keyTimes="0;0.12;0.8;1" dur={`${totalDur}s`} fill="freeze" />
        </rect>
        <text x={0} y={2.4} textAnchor="middle" fill="#fde047" fontSize="8" fontWeight="bold" fontFamily="sans-serif" letterSpacing="0.5">
          🎭 MASCOT TIME!
      ...
      {/* The mascot — anchored at the dugout roof line. Pops up from below the
          roof, dances, then ducks back down at the end of the window. */}
      <g transform={`translate(${x}, 410)`}>
        {/* Pop-up / duck-down (vertical slide) + fade at the very edges so the
            rise out of the dugout reads cleanly. */}
        <animateTransform attributeName="transform" type="translate" additive="sum" values="0,16; 0,0; 0,0; 0,16" keyTimes="0;0.14;0.86;1" dur={`${totalDur}s`} fill="freeze" />
        <animate attributeName="opacity" values="0;1;1;0" keyTimes="0;0.12;0.88;1" dur={`${totalDur}s`} fill="freeze" />
        {/* Rocking dance — the whole body rocks around the feet anchor. */}
        <g>
          <animateTransform attributeName="transform" type="rotate" values="-9;9;-9" dur="0.6s" repeatCount="indefinite" />
          {/* Shadow at feet */}
          <ellipse cx={0} cy={0.5} rx={6} ry={1.6} fill="rgba(0,0,0,.18)" />
          {/* Legs — little stomping kicks */}
          <line x1={-2.4} y1={-4} x2={-3.2} y2={0} stroke={fur} strokeWidth={2} strokeLinecap="round" />
          <line x1={2.4} y1={-4} x2={3.2} y2={0} stroke={fur} strokeWidth={2} strokeLinecap="round" />
          {/* Round fuzzy body */}
          <ellipse cx={0} cy={-9} rx={6.4} ry={6} fill={fur} />
          {/* Belly patch */}
          <ellipse cx={0} cy={-8} rx={3.6} ry={3.4} fill={belly} opacity={0.9} />
          {/* Arms — wave up and down out of phase (animated endpoint sweep) */}
          <line x1={-5} y1={-11} x2={-9.5} y2={-16} stroke={fur} strokeWidth={2} strokeLinecap="round">
            <animate attributeName="y2" values="-16;-8;-16" dur="0.6s" repeatCount="indefinite" />
          </line>
          <line x1={5} y1={-11} x2={9.5} y2={-8} stroke={fur} strokeWidth={2} strokeLinecap="round">
            <animate attributeName="y2" values="-8;-16;-8" dur="0.6s" repeatCount="indefinite" />
          </line>
          {/* Oversized head */}
          <circle cx={0} cy={-19} r={5.6} fill={fur} />
          {/* Snout / duckbill */}
          <ellipse cx={0} cy={-17.2} rx={3.2} ry={1.6} fill={snout} />
          {/* Googly eyes — wide whites with wandering pupils */}
          <circle cx={-2} cy={-20.6} r={1.7} fill="#ffffff" stroke="#0f172a" strokeWidth={0.25} />
          <circle cx={2} cy={-20.6} r={1.7} fill="#ffffff" stroke="#0f172a" strokeWidth={0.25} />
          <circle cx={-2} cy={-20.6} r={0.7} fill="#0f172a">
            <animate attributeName="cx" values="-2.6;-1.4;-2.6" dur="1.1s" repeatCount="indefinite" />
          </circle>
          <circle cx={2} cy={-20.6} r={0.7} fill="#0f172a">
            <animate attributeName="cx" values="1.4;2.6;1.4" dur="1.1s" repeatCount="indefinite" />
          </circle>
          {/* Goofy tuft of fur on top */}
          <path d="M -1.5 -24 Q 0 -27 1.5 -24 Q 0.4 -24.8 0 -24 Q -0.4 -24.8 -1.5 -24 Z" fill={fur} />
        </g>
        {/* Floating music notes — loop upward beside the dancing mascot */}
        <text x={-12} y={-24} fontSize="6" fill="#0b2a4a" fontFamily="sans-serif" opacity={0}>
          ♪
          <animate attributeName="opacity" values="0;0.9;0" dur="1.3s" repeatCount="indefinite" />
          <animate attributeName="y" values="-22;-32" dur="1.3s" repeatCount="indefinite" />
        </text>
        <text x={11} y={-26} fontSize="6" fill="#0b2a4a" fontFamily="sans-serif" opacity={0}>
          ♫
          <animate attributeName="opacity" values="0;0.9;0" dur="1.3s" begin="0.65s" repeatCount="indefinite" />
          <animate attributeName="y" values="-24;-34" dur="1.3s" begin="0.65s" repeatCount="indefinite" />
        </text>
      </g>
    </g>
  );
...
function BaseballField({ bases, outs = 0, defColor, offColor, ballPos, ballTrail = [], pitchPhase, batterPhase, showFW, fwKey, movingRunners, movingFielder, batterSide = 'R', platePlay = null, isPlayoff = false, pitchingChange = null, battingTeamIdx = 0, homeColor = '#1e40af', awayColor = '#b91c1c', homeKs = 0, awayKs = 0, homeKHistory = [], awayKHistory = [], isNightGame = false, webGem = null, errorBobble = null, umpirePhase = 'idle', crowdReactions = [], pitchLocations = [], inning = 1, catcherSigns = 0, bullpenActive = false, reliefColor = '#1e40af', moundVisit = null, brokenBat = null, pickoffDive = null, shiftType = null, leadCaution = 1, robbedHRLeap = null, bangBangPlay = null, sunFlare = null, crowdWaveActive = false, crowdWaveKey = 0, beachBallActive = false, beachBallKey = 0, challengeReview = null, fanCatch = null, coachSignKey = 0, rosinAct = null, rosinKey = 0, hbpImpact = null, groundsCrewActive = false, groundsCrewKey = 0, divingStop = null, ejectionAct = null, ejectionUmpireActive = false, flyingHelmet = null, maskToss = null, catcherLookingUp = false, pitcherTipCap = false, pitcherShakeOff = false, catcherBlocking = false, framingKey = 0, pitcherBrowWipe = false, pitcherCheckRunner = false, f1bStretch = null, umpire1BPhase = 'idle', batBoyAct = null, chokeUp = false, fistPump = null, catcherFraming = false, pitcherHeadNod = false, freshBallToss = null, infieldIn = false, curtainCall = null, pitcherCover = null, ofBackup = null, walkOffMob = null, highFiveLine = null, rallyCaps = null, wind = null, mascotAct = null, mascotKey = 0, pitcherRubberKick = false, capFlyOff = null, ofWarmup = null, ofWarmupKey = 0, pitcherHomeBackup = null }) {
  ...
  return (
    <div className="relative w-full max-w-[750px] mx-auto overflow-hidden rounded-xl shadow-2xl">
      ...
      <svg viewBox="0 0 620 500" className="w-full h-auto block bg-blue-100 font-sans">
        ...
        <StadiumFlag isNightGame={isNightGame} wind={wind} />
        ...
        {/* Beach ball in the stands (Backlog #151) — atmospheric crowd moment.
            Keyed so each trigger restarts the SVG animation cleanly. */}
        <g key={`beach-ball-${beachBallKey}`}>
          <BeachBall visible={beachBallActive} isNightGame={isNightGame} />
        </g>
        {/* Mascot dance on the dugout roof (Backlog #153) — atmospheric
            between-innings bit. Keyed so each trigger restarts the SVG
            animation cleanly. */}
        <g key={`mascot-${mascotKey}`}>
          <SVGMascot visible={!!mascotAct} x={mascotAct ? mascotAct.x : 112} isNightGame={isNightGame} />
        </g>
        ...
        {/* On-deck batter (Backlog #110) — the next-up hitter takes slow
            ...
            coaches, catcher signs, and runner leadoffs. The batting team is
            known directly from battingTeamIdx (1 = home) — no fragile hex
            comparison of offColor against homeColor (Backlog #169). */}
        {(() => {
          const homeBatting = battingTeamIdx === 1;
        ...
        {movingFielder?.id !== 'LF' && ofBackup?.id !== 'LF' && <SVGFielder x={lfPos.x} y={lfPos.y} color={defColor} glove="right" />}
        {movingFielder?.id !== 'CF' && ofBackup?.id !== 'CF' && <SVGFielder x={cfPos.x} y={cfPos.y} color={defColor} glove="right" />}
        {movingFielder?.id !== 'RF' && ofBackup?.id !== 'RF' && <SVGFielder x={rfPos.x} y={rfPos.y} color={defColor} glove="left" />}
        ...
        {movingFielder?.id !== 'C' && !moundVisit && <SVGCatcher x={F.C.x} y={F.C.y - 8} color={defColor} signs={catcherSigns} lookingUp={catcherLookingUp} blocking={catcherBlocking} framing={catcherFraming} framingKey={framingKey} />}
        ...
        ].filter(f => movingFielder?.id !== f.id && ofBackup?.id !== f.id).map(f => (
          <g key={`lbl-${f.id}`}>
        ...
        {/* Pitcher backs up home plate (Backlog #163) — on outfield-assist
            gun-downs at home, the pitcher sprints from the mound to a
            position just behind home plate to back up the catcher's tag
            in case the relay throw sails wide. Rendered as a hustling
            fielder sprite; the mound pitcher render below is suppressed
            while this is active so only one pitcher figure is on screen. */}
        {pitcherHomeBackup && <SVGFielder x={pitcherHomeBackup.x} y={pitcherHomeBackup.y} color={pitcherHomeBackup.color} isMoving={true} glove="left" />}
        {/* Outfielder backing up the play (Backlog #154) — second OF jogging
            diagonally to back up the lead fielder on contact plays to LF/CF/RF.
            Rendered as a moving fielder sprite; the static OF render at ofBackup.id
            is suppressed above so only one figure occupies that position. */}
        {ofBackup && <SVGFielder x={ofBackup.x} y={ofBackup.y} color={ofBackup.color || defColor} isMoving={true} glove={ofBackup.id === 'RF' ? 'left' : 'right'} />}
        ...
        {!rosinAct && !pitcherCover && !pitcherHomeBackup && <SVGPitcher key={fistPump ? `fp-${fistPump.ts}` : (pitcherHeadNod ? 'p-nod' : (pitcherRubberKick ? 'p-rk' : 'p-std'))} x={F.P.x} y={F.P.y - 14} color={fistPump?.color || defColor} phase={fistPump ? 'fistPump' : (pitcherTipCap ? 'tipCap' : (pitcherBrowWipe ? 'wipeBrow' : (pitcherCheckRunner ? 'checkRunner' : (pitcherRubberKick ? 'rubberKick' : pitchPhase))))} fromStretch={fromStretch} headShake={pitcherShakeOff} headNod={pitcherHeadNod} />}
        ...
        {/* Outfielder's cap flies off (Backlog #158) — pure cosmetic SVG that
            renders the team-colored cap tumbling away from the catch point.
            Keyed via `ts` so the SMIL animations remount cleanly on each
            trigger. */}
        {capFlyOff && <SVGFlyingCap key={`cfo-${capFlyOff.ts || 0}`} x={capFlyOff.x} y={capFlyOff.y} color={capFlyOff.color} side={capFlyOff.side || 1} />}
        {/* Outfielders warm-up tosses between innings (Backlog #160) — pure
            cosmetic ball arcing between two OFs during the inning summary
            card. Keyed via `ofWarmupKey` so the SMIL animation elements
            remount cleanly on each trigger and start from frame 0. The
            two OFs are already rendered statically (lfPos/cfPos/rfPos);
            this just adds the airborne ball that arcs back and forth. */}
        {ofWarmup && (() => {
          const _ofPosMap = { LF: lfPos, CF: cfPos, RF: rfPos };
          const fp = _ofPosMap[ofWarmup.fromKey] || lfPos;
          const tp = _ofPosMap[ofWarmup.toKey] || cfPos;
          return <SVGOFWarmup key={`ofw-${ofWarmupKey}`} visible={true} fromPos={fp} toPos={tp} color={ofWarmup.color} isNightGame={isNightGame} />;
        })()}
...
// ============================================================
// Game-generation token + play-scoped timer registry (Backlog #165)
// Every gameplay setTimeout inside the main component is routed through
// schedT(). Each timer is tagged with the game generation it was scheduled
// under and tracked in a registry; startGame() calls cancelGameTimers(),
// which bumps the generation and clears every pending timer, so no
// in-flight play from a previous game can mutate the new one (stale balk
// chains, ghost transitionHalf calls, deadlocked procRef, etc.). The
// generation token is also captured by the long-running rAF walkers
// (ejection manager, bat boy) so a mid-walk reset stops them cold.
let gameGen = 0;
const gameTimers = new Set();
function schedT(fn, ms) {
  const gen = gameGen;
  const id = setTimeout(() => {
    gameTimers.delete(id);
    if (gen !== gameGen) return; // scheduled under a previous game — drop it
    fn();
  }, ms);
  gameTimers.add(id);
  return id;
...
function BaseballCardGameInner() {
  ...
  // Synchronous mirror of the LIVE drawn pile (Backlog #168). Every live
  // mutation goes through pileRef first (cp = [...pileRef.current, ...];
  // pileRef.current = cp; setDrawnPile(cp);) so playHistory pileSnapshots are
  // captured synchronously instead of inside a setDrawnPile updater — the old
  // pattern was an updater-purity violation whose cross-updater write order
  // React doesn't guarantee, and could persist `undefined` snapshots (empty
  // pile on replay). Replays may point drawnPile at a historical snapshot for
  // display, but they never touch pileRef, so the live pile survives replays
  // intact and is restored on the next draw.
  ...
  // Cap flies off (Backlog #158) — pure cosmetic SVG that pops a fielder's cap
  // off during a web gem or robbed-HR catch (~35% chance on either). { x, y,
  // color, side } drive the position + team color + tumble direction. Auto-
  // clears ~1.3s after the catch.
  const [capFlyOff, setCapFlyOff] = useState(null);
  // "🧢 CAP OFF!" info-bar flash that accompanies the SVG animation. Pure
  // visual flair — fires alongside the cap-fly trigger.
  const [capOffBadge, setCapOffBadge] = useState(null); // { team: 0|1 }
  // Stretched single → double (Backlog #159) — batter rounds first hard on a
  // clean single with bases empty and reaches second standing up. Badge state.
  const [stretchedDoubleBadge, setStretchedDoubleBadge] = useState(null); // { batter, team: 0|1 }
  // Outfielders warm-up tosses between innings (Backlog #160) — atmospheric
  // pre-pitch flavor where two OFs play catch during the inning summary card.
  // `ofWarmup` carries { fromKey, toKey, color } selecting which two OFs
  // toss back and forth; rendered through `BaseballField` as a small ball
  // arcing between them. `ofWarmupKey` bumps on each trigger to remount the
  // SMIL animations cleanly.
  const [ofWarmup, setOfWarmup] = useState(null); // { fromKey: 'LF'|'CF'|'RF', toKey: 'LF'|'CF'|'RF', color, ts } or null
  const [ofWarmupKey, setOfWarmupKey] = useState(0);
  // Line-drive double play (Backlog #161) — one of baseball's most exciting
  // defensive plays. ~6% of lineOut outcomes with a runner on 1B or 2B (< 2
  // outs) re-mapped to a "doubled off" play: the fielder snares the line
  // drive and throws to the bag for the second out. Badge state.
  const [lineOutDPBadge, setLineOutDPBadge] = useState(null); // { batter, runner, base, team: 0|1 }
  ...
  // Beach ball in the stands (Backlog #151) — atmospheric crowd moment shown at
  // some mid/late half-inning transitions. beachBallActive controls visibility;
  // beachBallKey forces the SVG <animateMotion>/<animate> elements to remount
  // and restart cleanly on each trigger.
  const [beachBallActive, setBeachBallActive] = useState(false);
  const [beachBallKey, setBeachBallKey] = useState(0);
  // Wind conditions (Backlog #152) — rolled once per game in startGame:
  // null (calm, most games), 'out' (blowing out — some deep flies carry over
  // the wall), or 'in' (blowing in — some would-be homers die at the track).
  // Shown via a persistent info-bar chip and the CF flag, with a transient
  // badge + gust sound on wind-affected plays.
  const [wind, setWind] = useState(null);
  const [windBadge, setWindBadge] = useState(null); // { type: 'aided' | 'blown', team: 0|1 }
  // Mascot dance on the dugout roof (Backlog #153) — between-innings
  // atmosphere bit. mascotAct = { x } while dancing (null otherwise);
  // mascotKey remounts the SVG animations fresh on each trigger.
  const [mascotAct, setMascotAct] = useState(null);
  ...
  const [checkSwingBadge, setCheckSwingBadge] = useState(null); // Check-swing appeal (Backlog #150) — { batter, team: 0|1, ruledSwing: bool }
  ...
  // Foul-territory sacrifice fly (Backlog #156) — brief "🎯 FOUL SAC FLY" pill
  // when the catch in foul ground is deep enough for the runner on 3B to tag
  // up and score. { batter, runner, fielder, team: 0|1 }.
  const [foulSacFlyBadge, setFoulSacFlyBadge] = useState(null);
  // Pitcher kicks the rubber (Backlog #157) — pre-pitch atmospheric flavor:
  // the pitcher scrapes his back foot along the rubber to groom the dirt.
  // The boolean drives the SVGPitcher's 'rubberKick' pose; the badge is a
  // brief pulsing pill, { team: 0|1 }.
  const [pitcherRubberKick, setPitcherRubberKick] = useState(false);
  const [rubberKickBadge, setRubberKickBadge] = useState(null);
  ...
  // Outfielder backing up the play (Backlog #154) — when the ball is hit to LF/CF/RF
  // on a contact play, a second outfielder jogs to a backup position behind the
  // lead fielder. Pure cosmetic team-defense flavor. `ofBackup` payload:
  // { id: 'LF'|'CF'|'RF', x, y, color } where (x,y) is the interpolated current
  // position of the backup sprite. Standard OF render at that id is suppressed
  // while this is set so only one figure occupies the position. Modeled on the
  // pitcher-cover (Backlog #143) sprite pattern.
  const [ofBackup, setOfBackup] = useState(null);
  // Pitcher backs up home plate (Backlog #163) — companion to pitcher-cover-first
  // (#143) and OF-backup (#154). On outfield-assist gun-downs at home (#89), the
  // pitcher sprints from the mound to a backup spot behind the catcher in case
  // the relay throw sails wide. Drives a moving fielder sprite from F.P toward
  // a point just behind home plate. The mound SVGPitcher render is suppressed
  // while this is active so only one pitcher figure is on screen. `pitcherHomeBackup`
  // payload: { x, y, color } — interpolated current position of the backup sprite.
  // Pure cosmetic — no engine effect.
  const [pitcherHomeBackup, setPitcherHomeBackup] = useState(null);
  const [pitcherHomeBackupBadge, setPitcherHomeBackupBadge] = useState(null); // { team: 0|1 }
  // Fouled-off-foot recovery (Backlog #162) — ~6% of foul ball outcomes with
  // < 2 strikes get re-narrated as a foul ricocheted off the batter's foot or
  // shin. Pure visual + narration + pacing layer — still counts as a strike
  // per real MLB rules. The badge flashes during the batter's hopping recovery
  // animation; the count update is delayed ~1.0s so the recovery reads cleanly
  // before the next pitch.
  const [offFootBadge, setOffFootBadge] = useState(null); // { batter, team: 0|1 }
  // Pitchout (Backlog #155) — defensive counter to a suspected steal. ~3% of
  // non-contact pitches with a runner on 1B or 2B only (and < 2 outs, count.balls
  // < 3) get re-mapped to a `'ball'` and routed wide of the strike zone. The
  // catcher reaches outside the zone to receive. If a steal attempt was also
  // queued on this pitch, success rate drops by 50%. Pulsing red-to-amber
  // "🎯 PITCHOUT" pill in the info bar.
  const [pitchoutBadge, setPitchoutBadge] = useState(null); // { team: 0|1 }
  ...
  // Monotonic key bumped each time framing starts (Backlog #171). The framing
  // <g> was keyed with Date.now(), which changed every render and remounted the
  // group continuously — restarting the SMIL glove-nudge so it never played
  // through. Keying by this id remounts once per framing, like rosinKey does.
  ...
  const ofBackupAnimRef = useRef(null); // rAF id for the outfielder-backing-up sprite (Backlog #154)
  const pitcherHomeBackupAnimRef = useRef(null); // rAF id for the pitcher-backs-up-home sprite (Backlog #163)
  const playIdRef = useRef(0);
  const prevLeadRef = useRef(null); // 0 = away leads, 1 = home leads, null = tied
  // Backlog #166: latest-callback ref for auto-play. The interval effect only
  // re-runs on [autoPlay, autoSpeed, phase], so without this it called ONE
  // frozen doDrawCard closure for the whole session — posPitcherActive stayed
  // [false,false] (log spam every pitch in a blowout, mechanics never engaged),
  // batterSide froze at whoever batted when AUTO was toggled (mirror-flipped
  // HBP/brushback/pitchout/fan-grab visuals), and half-start bookkeeping wrote
  // stale play counts. The ref is refreshed after every render, so the interval
  // always invokes a closure that sees current state.
  const doDrawCardRef = useRef(null);
  useEffect(() => { doDrawCardRef.current = doDrawCard; });
  // Backlog #166: live mirror of playHistory.length — transitionHalf reads this
  // instead of a possibly-stale captured playHistory when recording the new
  // half-inning's replay back-boundary (also fixes a subtle off-by-one in
  // manual play: the captured array predates the half's final play).
  const playHistLenRef = useRef(0);
  useEffect(() => { playHistLenRef.current = playHistory.length; }, [playHistory]);

  useEffect(() => { sndRef.current = soundOn; spdRef.current = autoSpeed; }, [soundOn, autoSpeed]);
  useEffect(() => { return () => { cancelGameTimers(); if (animRef.current) cancelAnimationFrame(animRef.current); if (runnerAnimRef.current) cancelAnimationFrame(runnerAnimRef.current); if (fielderAnimRef.current) cancelAnimationFrame(fielderAnimRef.current); if (plateAnimRef.current) cancelAnimationFrame(plateAnimRef.current); if (moundVisitRef.current) cancelAnimationFrame(moundVisitRef.current); if (pitcherCoverAnimRef.current) cancelAnimationFrame(pitcherCoverAnimRef.current); if (ofBackupAnimRef.current) cancelAnimationFrame(ofBackupAnimRef.current); if (pitcherHomeBackupAnimRef.current) cancelAnimationFrame(pitcherHomeBackupAnimRef.current); }; }, []);
  useEffect(() => { if (!autoPlay || phase !== 'playing') return; const id = setInterval(() => { if (procRef.current || gs.current.gameOver) return; doDrawCardRef.current(); }, autoSpeed); /* Backlog #166: interval calls through the latest-callback ref */ return () => clearInterval(id); }, [autoPlay, autoSpeed, phase]);
  ...
  useEffect(() => {
    ...
    const timer = schedT(() => setWalkUpStats(null), 2800);
    const musicTimer = schedT(() => setWalkUpMusic(null), 3400);
    return () => { clearTimeout(timer); clearTimeout(musicTimer); };
  }, [activeBatter, phase]);

  function rerender() { setRenderTick(t => t + 1); }
  function triggerFireworks() { setShowFW(true); setFwKey(k => k + 1); schedT(() => setShowFW(false), 4500); }
  ...
  function triggerWalkOffMob() {
    // Walk-offs only happen for the home team (they win in the bottom half),
    // so the mob wears the home color (Backlog #169 — was hardcoded red).
    setWalkOffMob({ x: 310, y: 416, color: TEAM_COLORS[1] });
    if (sndRef.current) schedT(() => playSoundLocal('dogPile'), 500);
    schedT(() => setWalkOffMob(null), 3200);
  }
  ...
  // Pitcher-backs-up-home sprite animator (Backlog #163) — drives a single
  // fielder figure from the mound (F.P) to a backup spot just behind home
  // plate on outfield-assist gun-downs. Dedicated ref + state cell so it
  // never collides with the receiver `movingFielder` (which carries the
  // catcher who's about to make the tag). The mound SVGPitcher render is
  // suppressed while `pitcherHomeBackup` is set so only one pitcher figure
  // is ever on screen. Modeled exactly on `animatePitcherCover`.
  function animatePitcherHomeBackup(sPos, ePos, dur, color) {
    const s = performance.now();
    function tick(now) {
      const t = Math.min(1, (now - s) / dur);
      setPitcherHomeBackup({ x: lerp(sPos.x, ePos.x, t), y: lerp(sPos.y, ePos.y, t), color });
      if (t < 1) pitcherHomeBackupAnimRef.current = requestAnimationFrame(tick);
    }
    pitcherHomeBackupAnimRef.current = requestAnimationFrame(tick);
  }

  // Outfielder-backing-up sprite animator (Backlog #154) — drives a second
  // outfielder figure from his standard position toward a backup spot behind
  // the lead fielder. Dedicated state cell + ref so it never collides with
  // the lead `movingFielder` (whose id will be a different OF). The static OF
  // render at the backup's id is suppressed in BaseballField while ofBackup
  // is set, so only one figure occupies that position. Pure cosmetic — purely
  // a render-position swap, no engine effect.
  function animateOFBackup(id, sPos, ePos, dur, color) {
    const s = performance.now();
    function tick(now) {
      const t = Math.min(1, (now - s) / dur);
      setOfBackup({ id, x: lerp(sPos.x, ePos.x, t), y: lerp(sPos.y, ePos.y, t), color });
      if (t < 1) ofBackupAnimRef.current = requestAnimationFrame(tick);
    }
    ofBackupAnimRef.current = requestAnimationFrame(tick);
  }

  // Higher-level helper that picks a backup OF, fires the sprite animation,
  // and auto-clears the state cell after the play resolves. Called from the
  // OF chase trigger sites (flyOut/lineOut to OF, singles/doubles/triples to
  // OF). Guarded so we never collide with a pitcher-cover or moving-fielder
  // pose, and gated at ~75% chance per qualifying play to leave some variety
  // (every-play backups would feel mechanical). Pure cosmetic.
  function triggerOFBackup(leadId, leadTarget, chaseDur, color, thisPlayId) {
    if (!leadId || !leadTarget) return;
    if (Math.random() >= 0.75) return; // ~75% trigger rate
    const pick = pickOFBackup(leadId, leadTarget);
    if (!pick) return;
    // Animate slightly slower than the lead chase so the backup arrives just
    // after the ball does (~+250ms) — reads as "trailing the play, ready to
    // recover an errant relay."
    animateOFBackup(pick.id, pick.startPos, pick.endPos, chaseDur + 250, color);
    // Auto-clear after the chase + a brief hold so the backup figure is
    // visible alongside the lead through the catch / pickup moment.
    schedT(() => {
      if (playIdRef.current !== thisPlayId) return;
      setOfBackup(null);
      if (ofBackupAnimRef.current) cancelAnimationFrame(ofBackupAnimRef.current);
    }, chaseDur + 1400);
  }

  // Helper to pick a backup OF given the lead OF id, and compute the backup
  // target position — a point ~30-40px deeper than the lead fielder's target,
  // in the direction the lead fielder ran. For LF/RF lead, CF backs up. For
  // CF lead, alternate between LF and RF (random). The backup target is
  // anchored slightly past the lead target (closer to the wall + offset
  // toward the backup's home position) so the two figures don't visually
  // overlap during the play.
  function pickOFBackup(leadId, leadTarget) {
    if (leadId === 'LF') {
      const startPos = F.CF;
      const tx = leadTarget.x + 14; // a bit toward CF from the lead position
      const ty = Math.max(FENCE_Y + 6, leadTarget.y - 6); // a bit deeper
      return { id: 'CF', startPos, endPos: { x: tx, y: ty } };
    } else if (leadId === 'RF') {
      const startPos = F.CF;
      const tx = leadTarget.x - 14; // a bit toward CF from the lead position
      const ty = Math.max(FENCE_Y + 6, leadTarget.y - 6);
      return { id: 'CF', startPos, endPos: { x: tx, y: ty } };
    } else if (leadId === 'CF') {
      const useLF = Math.random() < 0.5;
      const startPos = useLF ? F.LF : F.RF;
      const tx = leadTarget.x + (useLF ? -16 : 16); // backup comes from the side
      const ty = Math.max(FENCE_Y + 6, leadTarget.y - 6);
      return { id: useLF ? 'LF' : 'RF', startPos, endPos: { x: tx, y: ty } };
    }
    return null;
  }
  ...
  function animatePlatePlay(dur = 1800, onComplete) {
    ...
    function tick(now) {
      ...
      else { schedT(() => { setPlatePlay(null); if (onComplete) onComplete(); }, 600); }
    }
  ...
  function animateMoundVisit(onComplete) {
    ...
    function tick(now) {
      ...
      else { schedT(() => { setMoundVisit(null); if (onComplete) onComplete(); }, 100); }
    }
    moundVisitRef.current = requestAnimationFrame(tick);
  }
  ...
  function animateAroundTheHorn(onComplete) {
    ...
    function nextLeg() {
      ...
      animateBall(linePts(leg.from, leg.to, 16), leg.dur, () => { i++; schedT(nextLeg, 80); });
    }
    nextLeg();
  }
  ...
  function doShuffleAnimation(onComplete) {
    ...
    schedT(() => { setShuffling(false); setShuffleCards([]); if (onComplete) onComplete(); }, 2600);
  }
  ...
  function startGame() {
    ...
    // ---- Backlog #165: invalidate every in-flight play from the previous game ----
    cancelGameTimers();      // bump gameGen + clear all pending play-scoped timers
    playIdRef.current++;     // stale thisPlayId guards in any surviving callbacks now fail
    procRef.current = false; // a dead play can never deadlock the new game's DRAW button
    [animRef, fielderAnimRef, runnerAnimRef, plateAnimRef, moundVisitRef, pitcherCoverAnimRef, ofBackupAnimRef, pitcherHomeBackupAnimRef].forEach((r) => { if (r.current) { cancelAnimationFrame(r.current); r.current = null; } });
    ...
    // Wind conditions (Backlog #152): ~1 in 3 games is windy — 18% blowing out
    // (deep flies may carry over the wall), 15% blowing in (would-be homers can
    // get knocked down at the track). Announced in the game log, shown on the
    // CF flag and as a persistent info-bar chip for the whole game.
    const windRoll = Math.random();
    const gameWind = windRoll < 0.18 ? 'out' : windRoll < 0.33 ? 'in' : null;
    setWind(gameWind);
    setLog([
      { text: `⚾ Play Ball! ${teams[0].name} at ${teams[1].name}`, type: 'info' },
      { text: `📋 Attendance: ${attDisplay}`, type: 'info' },
      ...(gameWind ? [{ text: gameWind === 'out' ? `💨 The flags are pointing straight out to center — wind blowing OUT today!` : `💨 A stiff breeze is blowing IN from center field — deep drives will have to fight it.`, type: 'info' }] : []),
    ]);
    setPlayHistory([]); setReplayIdx(-1); setHalfStartIdx(0); pileRef.current = []; setDrawnPile(pileRef.current); setDispensedCard(null); setDispensing(false); setOutcome(null); setShowFW(false); setAutoPlay(false); setBallPos(null); setPitchPhase('idle'); setBatterPhase('stance'); setAnnounceText(''); setExitVelo(null); setHrDistance(null); setPopTime(null); setPlatePlay(null); setUmpirePhase('idle'); setScoringFlash(null); setPitchMilestone(null); setClutchBadge(null); setLeadChangeBadge(null); setCycleWatch(null); setWalkUpStats(null); setWalkUpMusic(null); setRobbedHRBadge(null); setRobbedHRLeap(null); setCrowdReactions([]); setGrooveBadge(null); setPitchLocations([]); setErrorBobble(null); setCatcherSigns(0); setFirstPitchStrikeBadge(null); setMoundVisit(null); setGrdBadge(null); setPickoffBadge(null); setPickoffDive(null); setLeadCaution(1); setBrokenBat(null); setPassedBallBadge(null); setPitchClockBadge(null); setBangBangPlay(null); setBangBangBadge(null); setCiBadge(null); setLostInSunBadge(null); setSunFlare(null); setDroppedFlyBadge(null); setChallengeReview(null); setChallengeBadge(null); setFanCatch(null); setCoachSignKey(0); setInsideTheParkBadge(null); setCrowdWaveActive(false); setCrowdWaveKey(0); setBeachBallActive(false); setBeachBallKey(0); setGroundsCrewActive(false); setGroundsCrewKey(0); setPosPitcherActive([false, false]); setRosinAct(null); setRosinKey(0); rosinTriggeredRef.current = false; setTripleplayBadge(null); setFoulTipBadge(null); setCheckSwingBadge(null); setHbpImpact(null); setHbpBadge(null); setOutfieldAssistBadge(null); setTexasLeaguerBadge(null); setDivingStop(null); setDivingStopBadge(null); setBrushbackBadge(null); setEjectionAct(null); setEjectionBadge(null); setEjectionUmpireActive(false); setFlyingHelmet(null); setSqueezeBadge(null); setMaskToss(null); setCatcherLookingUp(false); setMaskTossBadge(null); setInfieldSingleBadge(null); setPitcherTipCap(false); setTipCapBadge(null); setStealHomeBadge(null); setPitcherShakeOff(false); setShakeOffBadge(null); setThrowErrorBadge(null); setProductiveOutBadge(null); setTagUpBadge(null); if (pitcherCoverAnimRef.current) cancelAnimationFrame(pitcherCoverAnimRef.current); setPitcherCover(null); setPitcherCoverBadge(null); if (ofBackupAnimRef.current) cancelAnimationFrame(ofBackupAnimRef.current); setOfBackup(null); if (pitcherHomeBackupAnimRef.current) cancelAnimationFrame(pitcherHomeBackupAnimRef.current); setPitcherHomeBackup(null); setPitcherHomeBackupBadge(null); setOffFootBadge(null); setPitchoutBadge(null); setSacBuntBadge(null); setDragBuntBadge(null); setCatcherBlocking(false); setBlockedPitchBadge(null); setPitcherBrowWipe(false); setBrowWipeBadge(null); setPitcherCheckRunner(false); setCheckRunnerBadge(null); setF1bStretch(null); setLaunchAngle(null); setUmpire1BPhase('idle'); setBatBoyAct(null); setBatSpeed(null); setFirstTo3rdBadge(null); setOrganCharge(false); setComebackerBadge(null); setFieldersChoiceBadge(null); setHitAndRunBadge(null); setBackstopBadge(null); setChokeUpBadge(null); setFistPumpAct(null); setFistPumpBadge(null); setCatcherFraming(false); setFramingBadge(null); setPitcherHeadNod(false); setSignNodBadge(null); setCapTipFanBadge(null); setFreshBallToss(null); setFreshBallTossBadge(false); setInfieldFlyBadge(null); setCurtainCall(null); setCurtainCallBadge(null); setDoubleStealBadge(null); setWalkOffMob(null); setHighFiveLine(null); setOutfieldAssist3BBadge(null); setWindBadge(null); setMascotAct(null); setMascotKey(0); setFoulSacFlyBadge(null); setPitcherRubberKick(false); setRubberKickBadge(null); setCapFlyOff(null); setCapOffBadge(null); setStretchedDoubleBadge(null); setOfWarmup(null); setOfWarmupKey(0); setLineOutDPBadge(null); battleSoundFiredRef.current = false; prevLeadRef.current = null; setShowPostGame(false); setActiveBatter(teams[0].players[0]); shuffleDeck(); setPhase('playing'); rerender();
  }

  function doDrawCard() {
    if (procRef.current || gs.current.gameOver) return;
    procRef.current = true; setReplayIdx(-1);
    // Backlog #168: if the previous action was a replay, drawnPile still shows
    // the historical pileSnapshot — restore the live pile before this draw.
    // In normal play this is a same-reference set (React bails out, no render).
    setDrawnPile(pileRef.current);
      ...
      schedT(() => {
        ...
        schedT(() => {
          ...
          if (balkRuns > 0) {
            ...
            schedT(() => setScoringFlash(null), 2000);
            ...
            if (balkLead !== prevLeadRef.current) {
              if (balkLead === null) { setLeadChangeBadge({ type: 'tied', teamName: teams[bt].name }); schedT(() => setLeadChangeBadge(null), 3500); }
              else if (prevLeadRef.current !== null && balkLead !== prevLeadRef.current) { setLeadChangeBadge({ type: 'leadChange', teamName: teams[balkLead].name }); schedT(() => setLeadChangeBadge(null), 3500); }
            }
            prevLeadRef.current = balkLead;
          }
          const cp = [...pileRef.current]; pileRef.current = cp; setDrawnPile(cp); // #168: snapshot synchronously via pileRef
          ...
          if (balkRuns > 0 && bt === 1 && gs.current.inning >= 9) {
            ...
            if (homeTotal > awayTotal) {
              ...
              if (sndRef.current) schedT(() => playSoundLocal('walkOff'), 400);
              schedT(() => setAnnounceText(`WALK-OFF BALK! ${teams[1].name} win!`), 600);
              setAutoPlay(false);
              schedT(() => setShowPostGame(true), 3500);
            }
          }
      ...
      schedT(() => {
        ...
        if (pickoffWild) {
          ...
          animateBall([...linePts(F.P, F.B1, 8), ...linePts(F.B1, wildTarget, 8)], 700, () => {
            ...
            schedT(() => setErrorBobble(null), 1300);
            ...
            schedT(() => {
              ...
              schedT(() => {
                ...
                schedT(() => setPickoffBadge(null), 3000);
                const cp = [...pileRef.current]; pileRef.current = cp; setDrawnPile(cp); // #168: snapshot synchronously via pileRef
        ...
        animateBall(linePts(F.P, F.B1, 14), 380, () => {
          ...
          schedT(() => setPickoffDive(null), 900);
          schedT(() => {
            if (playIdRef.current !== thisPlayId) return;
            if (pickoffOut) {
              gs.current.bases[0] = null;
              ...
              schedT(() => setPickoffBadge(null), 2600);
              const cp = [...pileRef.current]; pileRef.current = cp; setDrawnPile(cp); // #168: snapshot synchronously via pileRef
              ...
              if (gs.current.outs >= 3) {
                ...
                if ((isTopOf9thPlus && homeTotal > awayTotal) || (isBotOf9thPlus && awayTotal > homeTotal)) {
                  ...
                  setAutoPlay(false);
                  procRef.current = false;
                  schedT(() => setShowPostGame(true), 2000);
                  rerender();
                  return;
                }
                procRef.current = true; // keep locked; transitionHalf releases
                schedT(() => transitionHalf(), 900);
                rerender();
                return;
              }
              procRef.current = false;
              ...
              schedT(() => setPickoffBadge(null), 2200);
              // Runner plays the next pitch cautiously — smaller lead, then decays back to full.
              setLeadCaution(0.35);
              schedT(() => setLeadCaution(0.65), 2400);
              schedT(() => setLeadCaution(1), 4800);
              const cp = [...pileRef.current]; pileRef.current = cp; setDrawnPile(cp); // #168: snapshot synchronously via pileRef
            ...
            schedT(() => {
              ...
              schedT(() => {
                ...
                if (wpLead !== prevLeadRef.current) {
                  if (wpLead === null) { setLeadChangeBadge({ type: 'tied', teamName: teams[bt].name }); schedT(() => setLeadChangeBadge(null), 3500); }
                  else if (prevLeadRef.current !== null && wpLead !== prevLeadRef.current) { setLeadChangeBadge({ type: 'leadChange', teamName: teams[wpLead].name }); schedT(() => setLeadChangeBadge(null), 3500); }
                }
                ...
                if (bt === 1 && gs.current.inning >= 9) {
                  ...
                  if (homeTotal > awayTotal) {
                    ...
                    triggerFireworks(); triggerFireworks();
                    if (sndRef.current) schedT(() => playSoundLocal('walkOff'), 400);
                    schedT(() => setAnnounceText(`WALK-OFF! Wild pickoff throw lets ${runnerName} score! ${teams[1].name} win!`), 600);
                    setAutoPlay(false);
                    procRef.current = false;
                    schedT(() => setShowPostGame(true), 3500);
                    rerender();
    ...
    if (!rosinTriggeredRef.current && !gs.current.gameOver && !hasRunnerOnBase
      ...
      if (sndRef.current) schedT(() => playSoundLocal('rosinClap'), 720);
      ...
      schedT(() => {
        ...
        schedT(() => { rosinTriggeredRef.current = false; }, 800);
      }, 1600);
      return;
    }
    ...
    if (!gs.current.gameOver && Math.random() < 0.003) {
      ...
      if (pitcherEligible || batterEligible) {
        ...
        schedT(() => setPitchClockBadge(null), 2600);
          ...
          schedT(() => {
            ...
            if (ciRes.runs > 0) {
              ...
              if (ciLead !== prevLeadRef.current) {
                if (ciLead === null) { setLeadChangeBadge({ type: 'tied', teamName: teams[bt].name }); schedT(() => setLeadChangeBadge(null), 3500); }
                else if (prevLeadRef.current !== null && ciLead !== prevLeadRef.current) { setLeadChangeBadge({ type: 'leadChange', teamName: teams[ciLead].name }); schedT(() => setLeadChangeBadge(null), 3500); }
              }
            ...
            if (ciRes.runs > 0 && bt === 1 && gs.current.inning >= 9) {
              ...
              if (homeTotal > awayTotal) {
                ...
                triggerFireworks(); triggerFireworks();
                if (sndRef.current) schedT(() => playSoundLocal('walkOff'), 400);
                schedT(() => setAnnounceText(`WALK-OFF! ${teams[1].name} win on catcher's interference!`), 600);
                setAutoPlay(false);
                schedT(() => setShowPostGame(true), 3500);
              }
            }
        ...
        animateBall(linePts(F.P, catcherWide, 12), 450, () => {
          ...
          if (ibbRes.runs > 0) {
            ...
            if (ibbLead !== prevLeadRef.current) {
              if (ibbLead === null) { setLeadChangeBadge({ type: 'tied', teamName: teams[bt].name }); schedT(() => setLeadChangeBadge(null), 3500); }
              else if (prevLeadRef.current !== null && ibbLead !== prevLeadRef.current) { setLeadChangeBadge({ type: 'leadChange', teamName: teams[ibbLead].name }); schedT(() => setLeadChangeBadge(null), 3500); }
            }
    ...
    // Check-swing appeal (Backlog #150) — on 2-strike counts, ~11% of `ball`
    // outcomes get appealed to the base umpire as a possible check swing. The
    // home-plate umpire calls it a ball, the catcher appeals, and the base ump
    // rules: ~45% are "he went around!" → strike three (re-mapped oc 'ball' →
    // 'strikeout' BEFORE calculatePlayResult / narrateLocal so the standard
    // strikeout pathway — out increment, K credit, count reset, batter advance,
    // HP-umpire punch-out, strikeout crowd reaction — flows with no special
    // engine plumbing). The other ~55% are ruled "no swing" → the pitch stands
    // as a ball (oc unchanged, count gets the ball, a 3-2 take walks normally).
    // The isCheckSwing / csRuledSwing flags are threaded through the narration
    // override, the appeal-cue (base-ump signal + badge + sound) block below,
    // and play history for replay parity.
    let isCheckSwing = false, csRuledSwing = false;
    if (oc === 'ball' && gs.current.count.strikes === 2 && !isFoulTip && Math.random() < 0.11) {
      isCheckSwing = true;
      if (Math.random() < 0.45) { csRuledSwing = true; oc = 'strikeout'; }
    }
    // Pitchout (Backlog #155) — defensive counter to a suspected steal. ~3% of
    // pitches with a runner on 1B only OR a runner on 2B only (no force
    // ambiguity), < 2 outs, count.balls < 3 (can't pitchout into a walk),
    // count.strikes < 2 (no surprise strikeouts from a re-mapped strike),
    // get re-mapped from a ball/strike to a 'ball'. The pitch is thrown wide
    // of the strike zone so the batter can't reach it but the catcher has a
    // clear shot at the runner if a steal is on. The standard ball pathway
    // handles the count increment downstream. If a stolen base event also
    // queued on this pitch, its success rate is halved post-hoc (the catcher
    // is standing and ready to throw — much harder to steal on a pitchout).
    // Suppressed when the engine has already re-mapped oc upstream into a
    // strikeout / single / homerun / hit-by-pitch outcome — the pitchout only
    // makes sense on a true pre-contact pitch.
    let isPitchout = false;
    {
      const _runnerStealThreat = (gs.current.bases[0] && !gs.current.bases[1])
        || (gs.current.bases[1] && !gs.current.bases[2]);
      if ((oc === 'ball' || oc === 'strike') && !csRuledSwing && !isFoulTip
        && _runnerStealThreat && gs.current.outs < 2
    ...
    // Wind conditions (Backlog #152) — on windy days (rolled per game in
    // startGame), the wind occasionally decides a deep ball's fate:
    //   - Blowing OUT: ~8% of flyOuts get carried over the wall → re-mapped to
    //     'homeRun' BEFORE calculatePlayResult / narrateLocal so the entire HR
    //     pathway (runs, trot, fireworks, HR stat) flows with no special
    //     engine plumbing.
    //   - Blowing IN: ~14% of homeRuns die at the warning track → re-mapped to
    //     'flyOut' so the standard out pathway flows (a runner on 3B with < 2
    //     outs still tags and scores via the engine's sac-fly path — deep
    //     enough to bring him home).
    // Guards: !isRobbedHR (robbery already converted that HR to a flyOut and
    // owns the wall-leap flavor), !isInsideThePark (an ITP homer never leaves
    // the park, so the wind can't knock it down), and !posPitcherActive (the
    // position-player conversions already skew fly balls; compounding would
    // over-inflate totals). isLostInSun / isDroppedFly re-mapped oc away from
    // 'flyOut' upstream, so they're excluded naturally. The isWindAided /
    // isWindBlownBack flags are threaded through the narration override,
    // badge + gust-sound cues, play history, and the season-sim mirror.
    let isWindAided = false, isWindBlownBack = false;
    if (wind && !posPitcherActive[1 - bt]) {
      if (wind === 'out' && oc === 'flyOut' && !isRobbedHR && Math.random() < 0.08) {
        isWindAided = true;
        oc = 'homeRun';
      } else if (wind === 'in' && oc === 'homeRun' && !isInsideThePark && Math.random() < 0.14) {
        isWindBlownBack = true;
        oc = 'flyOut';
      }
    }
    // Throwing error on routine groundouts (Backlog #105) — ~1.2% of routine
    ...
    if (isFirstPitch) {
      ...
      if (oc !== 'ball' && oc !== 'hbp') {
        ...
        schedT(() => setFirstPitchStrikeBadge(null), 1800);
      }
    }
    if (pc > 100) {
    ...
    if ([50, 75, 100].includes(pc)) {
      ...
      schedT(() => setPitchMilestone(null), 3000);
    }
    // Stretched single into a double (Backlog #159) — aggressive batter
    // baserunning play. With bases EMPTY (no runners — clean line-drive
    // single situation), ~5% chance the batter rounds first hard and takes
    // second standing up. Re-mapped from `single` → `double` BEFORE
    // calculatePlayResult so all downstream logic (batter to 2B, doubles
    // stat increment, hit credit, all runner advancement) flows naturally
    // through the standard double pathway. Companion to first-to-third
    // (#123, runner stretches) and OF-assist-at-3rd (#146, runner gunned
    // down) — this is the batter-side aggressive baserunning play.
    // Suppressed when the single is already a special-flavor case:
    //   - isLostInSun: sun-flavor case, no momentum to round
    //   - isThrowError: error-cause single, already special
    //   - isInfieldSingle: batter just barely beat it out, no stretch
    // We don't need a Texas-leaguer guard here because Texas leaguer is
    // decided AFTER calculatePlayResult — we'll suppress TL on stretched
    // doubles in the TL gate downstream.
    let isStretchedDouble = false;
    if (oc === 'single' && !isLostInSun && !isThrowError && !isInfieldSingle
        && !gs.current.bases[0] && !gs.current.bases[1] && !gs.current.bases[2]
        && Math.random() < 0.05) {
      isStretchedDouble = true;
      oc = 'double';
    }
    const res = calculatePlayResult(gs.current, oc, batter);
    // Line-drive double play / doubled-off the bag (Backlog #161) — ~6% of
    // lineOut outcomes with a runner on 1B or 2B and < 2 outs get re-mapped
    // to a double play: the fielder snares the line drive and the most-
    // advanced runner (off the bag) is doubled off. Mutates `res` in place:
    // outsAdded becomes 2, and the doubled-off runner is cleared from
    // res.nextBases. The runner with the longer lead is doubled off — prefer
    // R2 → caught off second; otherwise R1 → caught off first. Gated:
    //   - oc === 'lineOut': line drives only (sharp contact, runner can't
    //     hold up like on a fly ball — classic doubled-off scenario)
    //   - !res.isSacFly: if the engine already scored a run via the sac-fly
    //     path, the play is already resolved and shouldn't double up
    //   - gs.current.outs < 2: the +2 outs can never be the 3rd-and-4th
    //     out — outs goes from 0→2 or 1→3, half-inning ends naturally
    //   - (bases[0] || bases[1]): a runner must be off the bag to be
    //     doubled off (a runner on 3B has no place to go off the bag toward
    //     home — a runner on 3B with < 2 outs already converted to sac fly
    //     above via isSacFly gate)
    // The ~6% rate matches real-MLB frequency of line-drive double plays —
    // they're memorable enough that fans recognize them, but rare enough to
    // remain a genuine highlight when they happen. Mirrored in
    // simulateGameInstant so season stats stay close to interactive
    // expectations.
    let isLineOutDP = false;
    let lodpRunnerName = null;
    let lodpBase = null; // 'B1' or 'B2'
    if (oc === 'lineOut' && !res.isSacFly && gs.current.outs < 2
        && (gs.current.bases[0] || gs.current.bases[1])
        && Math.random() < 0.06) {
      if (gs.current.bases[1]) {
        // Runner on 2B doubled off — most common variant (R2 has the longer
        // lead and the longer trip back). Even if bases[0] is also set,
        // R2 has the bigger lead so he's the one caught off the bag.
        isLineOutDP = true;
        lodpRunnerName = gs.current.bases[1];
        lodpBase = 'B2';
        res.outsAdded = 2;
        res.nextBases = [res.nextBases[0], null, res.nextBases[2]];
      } else if (gs.current.bases[0]) {
        // Runner on 1B only — doubled off first.
        isLineOutDP = true;
        lodpRunnerName = gs.current.bases[0];
        lodpBase = 'B1';
        res.outsAdded = 2;
        res.nextBases = [null, res.nextBases[1], res.nextBases[2]];
      }
    ...
    const isGRD = oc === 'double' && !isDroppedFly && !isStretchedDouble && Math.random() < 0.06;
    ...
    // Check-swing appeal narration override (Backlog #150) — distinct variants
    // for the two verdicts. When ruled a swing, the engine already re-mapped
    // oc → 'strikeout' (above), so this swaps in the "he went around!" flavor
    // on top of the standard strikeout. When ruled no swing, oc is still a ball
    // and the at-bat continues. The base-umpire signal + badge + sound fire
    // from the dedicated appeal-cue block in the swing handler.
    if (isCheckSwing) {
      if (csRuledSwing) {
        const csOutTexts = [
          `Checked swing — the catcher appeals... and the base umpire rings him up! ${batter} went around — STRIKE THREE!${gs.current.outs >= 2 ? ' That retires the side!' : ''}`,
          `${batter} tries to hold up — appeal to the base ump... HE GOES AROUND! Strike three, the call comes down the line!${gs.current.outs >= 2 ? ' Inning over!' : ''}`,
          `Did he go? The catcher asks for help — and the base umpire punches him out! ${batter} caught swinging — that's the K!${gs.current.outs >= 2 ? ' Three up, three down!' : ''}`
        ];
        nar.text = pick(csOutTexts);
      } else {
        // Ruled "no swing" — the pitch stands as a ball. On a 3-ball count this
        // is ball four (a walk), so word the call accordingly.
        const isBallFour = gs.current.count.balls >= 3;
        const tail = isBallFour ? `that's ball four — ${batter} takes his base!` : `that's a ball.`;
        const csBallTexts = [
          `Checked swing — the catcher appeals down the line... NO, he held up! The base umpire says no swing — ${tail}`,
          `${batter} starts and stops — appeal to the base ump... and he says NO SWING! ${isBallFour ? 'Ball four — he walks!' : 'Ball.'}`,
          `Did he go around? The appeal goes to the base umpire... HE DID NOT! The check-swing holds — ${tail}`
        ];
        nar.text = pick(csBallTexts);
      }
      nar.isCheckSwing = true;
      nar.csRuledSwing = csRuledSwing;
    }
    // Pitchout narration override (Backlog #155) — three "catcher stood up
    // for a pitchout" variants. The runner reference picks the lead steal
    // threat (runner on 2B preferred over runner on 1B since that's the more
    // likely steal target). Always called as ball-X-Y so the count is clear
    // even when the pitchout doesn't deter a steal that wasn't on.
    if (isPitchout) {
      const _poBalls = gs.current.count.balls + 1;
      const _poStrikes = gs.current.count.strikes;
      const _poRunnerName = gs.current.bases[1] || gs.current.bases[0] || 'the runner';
      const _poRunnerLast = _poRunnerName.split(' ').slice(-1)[0] || 'the runner';
      const _poBase = gs.current.bases[1] ? 'second' : 'first';
      const poTexts = [
        `Pitchout! The catcher stands up — pitch is way outside, ${_poRunnerLast} stayed put at ${_poBase}. Ball ${_poBalls}-${_poStrikes}.`,
        `The defense calls a pitchout! Catcher reaches out to corral it — ${_poRunnerLast} didn't run. Ball ${_poBalls}-${_poStrikes}.`,
        `Wide one — a pitchout! The catcher was ready in case ${_poRunnerLast} went, but he held the bag. Ball ${_poBalls}-${_poStrikes}.`,
      ];
    ...
    // own dedicated narration and animation flavor — and when the wind knocked a
    // would-be homer down at the track (Backlog #152), which owns its own narration.
    const isWebGem = !isRobbedHR && !isWindBlownBack && ['flyOut', 'lineOut'].includes(oc) && !res.isSacFly && Math.random() < 0.15;
    if (isWebGem) {
      ...
      nar.isWebGem = true; // persist for replay parity (Backlog #171) — the
      // replay OF-backup guard (!p.isWebGem) was dead because this never persisted.
    }
    ...
    if (oc === 'flyOut' && !res.isSacFly && !isRobbedHR && !isWebGem && !isWindBlownBack
        && gs.current.bases[0] && gs.current.bases[1]
        && gs.current.outs < 2
        && Math.random() < 0.40) {
      isInfieldFly = true;
    ...
    // Cap flies off (Backlog #158) — ~35% of web gems and robbed HRs pop the
    // fielder's cap off in the diving / leaping catch. Pure cosmetic — no
    // gameplay effect, but one of baseball's most universally recognized
    // "great catch" moments. The visual trigger fires at the catch point in
    // the swing handler below (alongside the existing webGem/robbedHR
    // animation cues). Persisted to play history via nar.isCapOff for
    // replay parity. Mutually exclusive in practice with the infield fly
    // (an infielder catching a popup with ordinary effort would never lose
    // his cap), since isCapOff is gated to webGem OR robbedHR.
    if ((isWebGem || nar.isRobbedHR) && Math.random() < 0.35) {
      nar.isCapOff = true;
    }
    // Wind-aided home run narration override (Backlog #152) — the flyOut was
    // re-mapped to oc === 'homeRun' above, so the entire standard HR pathway
    // (runs, trot, fireworks, stats) already flows. Here we swap in narration
    // that credits the gust, and tag the play for the badge / gust-sound cue
    // and replay parity.
    if (isWindAided) {
      const ofChoices = ['lf', 'cf', 'rf'];
      if (!ofChoices.includes(nar.sub)) nar.sub = pick(ofChoices);
      const fieldName = nar.sub === 'lf' ? 'left' : nar.sub === 'cf' ? 'center' : 'right';
      const windAidedTexts = [
        `${batter} lifts a high fly ball to ${fieldName} — and the wind GRABS it! It carries... carries... GONE! A wind-aided home run!`,
        `Deep fly ball off the bat of ${batter} — the ${fieldName} fielder drifts back, but the wind keeps pushing it... it's OUTTA HERE! Thank the flags for that one!`,
        `${batter} gets under one to ${fieldName} field — on a calm day that's a routine out, but TODAY the gale carries it over the wall! HOME RUN!`
      ];
      nar.text = pick(windAidedTexts);
      nar.isWindAided = true;
    }
    // Wind-blown-back narration override (Backlog #152) — the homeRun was
    // re-mapped to oc === 'flyOut' above. Force an outfield direction and the
    // deepest catch variant so the ball animation dies at the wall (same
    // treatment as the robbed HR). If a runner tagged from 3B (the engine's
    // sac-fly path already credited the run), append the scoring line so the
    // call matches the play.
    if (isWindBlownBack) {
      const ofChoices = ['lf', 'cf', 'rf'];
      if (!ofChoices.includes(nar.sub)) nar.sub = pick(ofChoices);
      nar.dir = 'out';
      nar.variant = 2; // deepest catch — the ball dies at the warning track
      const fielderName = nar.sub === 'lf' ? 'left fielder' : nar.sub === 'cf' ? 'center fielder' : 'right fielder';
      const tagSuffix = res.runs > 0 ? ' Deep enough to bring the runner home from third!' : '';
      const windBlownTexts = [
        `${batter} CRUSHES one — that ball is GONE on any other day... but the wind knocks it down! The ${fielderName} hauls it in at the warning track!${tagSuffix}`,
        `Oh, he got ALL of that one! ${batter}'s deep drive battles the wind... and LOSES! Caught at the track by the ${fielderName}!${tagSuffix}`,
        `A towering blast from ${batter} — the crowd is up — but the wind blowing in flat KILLS it! The ${fielderName} squeezes it at the wall!${tagSuffix}`
      ];
      nar.text = pick(windBlownTexts);
      nar.isWindBlownBack = true;
    }
    ...
    if (['flyOut', 'lineOut'].includes(oc) && !res.isSacFly && !nar.isInfieldFly
        && !isWebGem && !isRobbedHR && !isLineOutDP
    ...
    // Foul-territory sacrifice fly (Backlog #156) — when the engine flagged
    // res.isFoulSacFly on a foulOut (runner on 3B + < 2 outs, ~25% chance),
    // override the default "pops out in foul territory" narration with three
    // sac-fly variants and force the catch to a corner infielder (1B or 3B
    // along the foul line) so the popup reads as deep enough down the line
    // for the runner to plausibly tag up and score. The runner-from-3B path
    // is animated via runnerOc='sacFly' downstream (computeRunnerPaths
    // already produces the [B3, HP] path for sacFly).
    if (oc === 'foulOut' && res.isFoulSacFly) {
      const fsfSub = pick(['the first baseman', 'the third baseman']);
      nar.sub = fsfSub;
      const fsfRunner = gs.current.bases[2];
      const fsfRunnerLast = (fsfRunner || '').split(' ').slice(-1)[0] || 'the runner';
      const fsfTexts = [
        `${batter} pops one up in foul territory — caught by ${fsfSub}! ${fsfRunnerLast} tags from third and scores on the sac fly!`,
        `Foul popup gloved by ${fsfSub} — but it's deep enough! ${fsfRunnerLast} tags up and crosses the plate! Sacrifice fly!`,
        `${fsfSub} ranges over into foul ground for the catch — and ${fsfRunnerLast} tags from third! A run scores on the foul sac fly!`,
      ];
      nar.text = pick(fsfTexts);
      nar.isFoulSacFly = true;
      nar.foulSacFlyRunner = fsfRunner;
      nar.foulSacFlyFielder = fsfSub;
    }
    if (isTexasLeaguer) {
      const ofChoices = ['lf', 'cf', 'rf'];
      if (!ofChoices.includes(nar.sub)) nar.sub = pick(ofChoices);
    ...
    // Stretched single → double narration override (Backlog #159) — the batter
    // rounded first hard on a clean single with bases empty and reached
    // second standing up. The engine has already re-mapped oc to 'double'
    // pre-CPR, so all stat logic flows through the standard double pathway
    // (AB charged, double credit, hit credit). This is purely the narration
    // / badge / sound layer that makes the play visibly different from a
    // regular card-mapped double. Force the catch direction to an outfield
    // position since stretched-single hits go where line-drive singles go
    // (into the outfield gaps). Pulsing emerald-to-amber badge with hover
    // tooltip naming the batter.
    if (isStretchedDouble) {
      const ofChoices = ['lf', 'cf', 'rf'];
      if (!ofChoices.includes(nar.sub)) nar.sub = pick(ofChoices);
      nar.dir = 'out';
      const fielderName = nar.sub === 'lf' ? 'left fielder' : nar.sub === 'cf' ? 'center fielder' : 'right fielder';
      const sdTexts = [
        `${batter} smacks one to the ${fielderName} — and he's NOT stopping at first! Rounds the bag hard, sliding into second — STRETCHED DOUBLE!`,
        `Sharp single to the ${fielderName} by ${batter} — but he reads the OF playing it casually and DIGS for second! SAFE! Stretched single into a double!`,
        `${batter} lines one toward the ${fielderName} — turns on the jets out of the box and STRETCHES it into a double! Hustle play!`,
      ];
      nar.text = pick(sdTexts);
      nar.isStretchedDouble = true;
    }
    // Line-drive double play narration override (Backlog #161) — three
    // variants that name the fielder, the doubled-off runner, and the bag.
    // The engine has already mutated `res` to clear the doubled-off runner
    // and add the second out; this is the narration / badge / sound layer.
    // Picks an OF or IF for the catch (lineOuts can be at either) — we use
    // the existing nar.sub if it's already a real fielder, otherwise default
    // to the middle infielders (where line drives are most often caught for
    // a doubled-off play in real MLB).
    if (isLineOutDP) {
      const lodpRunnerLast = (lodpRunnerName || '').split(' ').slice(-1)[0] || 'the runner';
      const lodpBagName = lodpBase === 'B1' ? 'first base' : 'second base';
      // Use the existing narration's fielder slot if it's already set to a
      // sensible position; otherwise force the catch to a middle infielder
      // (line drives doubled off the bag are most often snared by SS / 2B
      // who are nearest the basepaths). Avoid forcing the catcher / pitcher.
      const validFielders = ['lf', 'cf', 'rf', 'the shortstop', 'the second baseman', 'the third baseman', 'the first baseman'];
      if (!validFielders.includes(nar.sub)) {
        nar.sub = pick(['the shortstop', 'the second baseman']);
      }
      const fielderDisplay = nar.sub === 'lf' ? 'the left fielder'
        : nar.sub === 'cf' ? 'the center fielder'
        : nar.sub === 'rf' ? 'the right fielder'
        : nar.sub;
      const lodpTexts = [
        `LINE-DRIVE DOUBLE PLAY! ${batter} ropes one — ${fielderDisplay} snares it and fires to ${lodpBagName} to DOUBLE OFF ${lodpRunnerLast}!`,
        `Two on one swing! ${fielderDisplay} catches the liner and rifles to ${lodpBagName} — ${lodpRunnerLast} is DOUBLED OFF the bag!`,
        `${lodpRunnerLast} got caught napping! ${fielderDisplay} catches ${batter}'s rope and throws to ${lodpBagName} for the DOUBLE PLAY!`,
      ];
      nar.text = pick(lodpTexts);
      nar.isLineOutDP = true;
      nar.lodpRunner = lodpRunnerName;
      nar.lodpBase = lodpBase;
    }
    ...
    if (isDropped3rd) {
      ...
      schedT(() => setPassedBallBadge(null), 3200);
    }
    ...
    // Foul ball off the batter's foot (Backlog #162) — ~6% of plain foul-ball
    // strikes with fewer than 2 strikes get re-narrated as a foul that
    // ricochets directly off the batter's foot or shin. The batter winces,
    // hops on one leg, shakes it off, then resumes. Real MLB rule: still
    // counts as a strike (any foul ball with < 2 strikes increments the
    // count). Pure visual + narration + pacing layer — no statistical
    // change. Suppressed when the foul is already a special-flavor case:
    // - fan-grab souvenir (the ball is in the stands, not on the batter)
    // - foul-tip strikeout (already a 2-strike conversion path)
    // - backstop straight-back (the ball went up and away, not down at the foot)
    // The 2-strike guard avoids a weird "ouch... wait, foul is still just a
    // ball with 2 strikes so no count change" anti-climax. Mutually exclusive
    // with backstop because backstop fires first; both 30% + 6% rates compose
    // as independent rolls on different code paths and only one badge fires
    // per pitch (the if-block here gates on !nar.isBackstop).
    const isOffFoot = oc === 'foulBall' && !nar.isFanGrab && !nar.isFoulTip && !nar.isBackstop
      && gs.current.count.strikes < 2 && Math.random() < 0.06;
    if (isOffFoot) {
      const _ofStrikes = gs.current.count.strikes + 1; // < 2 enforces strike will not be 3
      const bodyPart = pick(['foot', 'shin', 'instep', 'ankle']);
      const ofTexts = [
        `OUCH! ${batter} fouls one off his ${bodyPart}! He hops away, shakes it off... back in the box. Strike ${_ofStrikes}.`,
        `${batter} fouls it straight down off his ${bodyPart}! He's hopping in pain — but he's okay. Strike ${_ofStrikes}.`,
        `Foul ball OFF THE ${bodyPart.toUpperCase()}! ${batter} winces, leans on the bat... shakes it off. Strike ${_ofStrikes}.`,
      ];
      nar.text = pick(ofTexts);
      nar.isOffFoot = true;
      nar.offFootBody = bodyPart;
    }
    ...
    // Suppress mask toss on a foul sac fly (Backlog #156) — the focal moment
    // is the runner scoring from third, not the catcher's mask. The catch is
    // also down the line (a corner infielder, not the catcher) so the mask
    // toss would be visually out of place.
    if (oc === 'foulOut' && !res.isFoulSacFly && Math.random() < 0.75) {
      nar.isMaskToss = true;
    }
    ...
    if (oc === 'ball' && !nar.isBrushback && !isCheckSwing
        && !gs.current.bases[0] && !gs.current.bases[1] && !gs.current.bases[2]
        && Math.random() < 0.04) {
    ...
    setDispensedCard(card); setDispensing(false); schedT(() => setDispensing(true), 30);
    ...
    // Pitchout halves steal success rates (Backlog #155) — the catcher is
    // standing in throwing position when the pitch arrives, so a runner who
    // broke for the next base is much more likely to be gunned down. If the
    // runner was originally going to be safe, re-roll at 50% — meaning the
    // safe call only stands ~50% of the time. The stealHomeEvent / doubleSteal
    // also pay the same penalty since the same logic applies.
    if (isPitchout && stolenBaseEvent && stolenBaseEvent.success) {
      stolenBaseEvent.success = Math.random() < 0.5;
    }
    if (isPitchout && stealHomeEvent && stealHomeEvent.success) {
      stealHomeEvent.success = Math.random() < 0.5;
    }
    if (isPitchout && doubleStealEvent && doubleStealEvent.trailSuccess) {
      doubleStealEvent.trailSuccess = Math.random() < 0.5;
    }
    ...
    // Pitcher kicks the rubber (Backlog #157) — pre-pitch atmospheric flavor:
    // the pitcher scrapes his back foot along the rubber to groom the dirt
    // before delivering. ~7% chance per pitch, mutually exclusive with all
    // other pre-pitch atmospheres (stepOut / cleatKnock / shakeOff / browWipe
    // / checkRunner / signNod) so only one ritual fires per pitch (pacing
    // brisk). Suppressed during stolen-base / steal-of-home events (those
    // have their own dedicated timing) and on HBP outcomes (special-trajectory
    // pitch). Adds ~700ms to the pitch-timing pipeline alongside the other
    // ritual delays. Completes the pre-pitch ritual library alongside rosin
    // bag (#84), brow wipe (#113), shake-off (#104), and check-the-runner
    // (#116) — one of the most universally recognized pitcher gestures.
    const doesRubberKick = !doesStepOut && !doesCleatKnock && !doesShakeOff && !doesBrowWipe && !doesCheckRunner && !doesSignNod
      && !stealHomeEvent && !stolenBaseEvent && !doubleStealEvent && oc !== 'hbp'
      && Math.random() < 0.07;
    const rubberKickDelay = doesRubberKick ? 700 : 0;
    if (doesStepOut) {
      setBatterPhase('stepOut');
      schedT(() => setBatterPhase('stance'), 800);
      ...
      schedT(() => { setPitchPhase('windup'); setCatcherSigns(signCount); setCoachSignKey(k => k + 1); }, stepOutDelay);
      ...
      schedT(() => setBatterPhase('stance'), 700);
      schedT(() => { setPitchPhase('windup'); setCatcherSigns(signCount); setCoachSignKey(k => k + 1); }, cleatKnockDelay);
      ...
      setBatterPhase('stance');
      schedT(() => setPitcherBrowWipe(false), 700);
      schedT(() => setBrowWipeBadge(null), 1600);
      schedT(() => { setPitchPhase('windup'); setCatcherSigns(signCount); setCoachSignKey(k => k + 1); }, browWipeDelay);
      ...
      setBatterPhase('stance');
      schedT(() => setPitcherCheckRunner(false), 700);
      schedT(() => setCheckRunnerBadge(null), 1600);
      schedT(() => { setPitchPhase('windup'); setCatcherSigns(signCount); setCoachSignKey(k => k + 1); }, checkRunnerDelay);
    } else if (doesSignNod) {
      ...
      schedT(() => setPitcherHeadNod(false), 100 + 520);
      schedT(() => setSignNodBadge(null), 1600);
      schedT(() => { setPitchPhase('windup'); }, signNodDelay);
    } else if (doesRubberKick) {
      // Rubber-kick pre-pitch routine (Backlog #157) — show the pitcher
      // scraping his back foot along the rubber for ~700ms to groom the dirt,
      // then begin the windup. The rubberKick pose takes priority over the
      // standard pitchPhase render (see BaseballField), so we set the flag
      // here and clear it after the gesture completes. The batter stays in
      // his standard stance. A soft brushy scuff sound plays as the foot
      // scrapes — intentionally quiet so it reads as background ritual.
      setPitcherRubberKick(true);
      setRubberKickBadge({ team: defTeam });
      if (sndRef.current) playSoundLocal('rubberKick');
      setBatterPhase('stance');
      schedT(() => setPitcherRubberKick(false), 700);
      schedT(() => setRubberKickBadge(null), 1600);
      schedT(() => { setPitchPhase('windup'); setCatcherSigns(signCount); setCoachSignKey(k => k + 1); }, rubberKickDelay);
    } else {
    ...
    if (doesShakeOff) {
      ...
      schedT(() => setShakeOffBadge(null), stepOutDelay + 2400);
    }
    schedT(() => {
      ...
      if (oc === 'hbp') {
        ...
        animateBall(linePts(F.P, { x: batterTargetX, y: batterTargetY }, 16), 600, () => {
          ...
          schedT(() => setHbpBadge(null), 3200);
          schedT(() => setHbpImpact(null), 1300);
          schedT(() => {
      ...
      if (oc === 'ball' && nar.isBrushback) {
        ...
        animateBall(linePts(F.P, { x: chinX, y: chinY }, 16), 500, () => {
          ...
          schedT(() => setBrushbackBadge(null), 3200);
          ...
          animateBall(linePts({ x: chinX, y: chinY }, F.C, 14), 280, () => {
            ...
            schedT(() => setBatterPhase('stance'), 250);
            schedT(() => {
      ...
      // Pitchout (Backlog #155) — re-route the pitch wide of the strike zone
      // (toward the catcher's standing pitchout position, away from the batter)
      // and fire the pitchout badge + sound. The pitch carries on back to the
      // pitcher for the standard ball-back-to-pitcher flow. The trajectory
      // deviation is intentional — the catcher is standing in throwing position
      // to nail any runner who breaks for the next base.
      if (oc === 'ball' && nar.isPitchout) {
        // Wide of the plate — toward the catcher's standing pitchout position.
        // For a right-handed batter the catcher stands to the OUTSIDE (1B side,
        // toward +x); for a lefty the catcher stands toward 3B (-x). The wide
        // target sits clearly outside the strike zone.
        const wideX = F.HP.x + (batterSide === 'L' ? -22 : 22);
        const wideY = F.HP.y - 10; // slightly above the plate (catcher's chest height while standing)
        animateBall(linePts(F.P, { x: wideX, y: wideY }, 16), 520, () => {
          if (sndRef.current) playSoundLocal('pitchout');
          setPitchoutBadge({ team: defTeam });
          schedT(() => setPitchoutBadge(null), 3000);
          schedT(() => {
            animateBall(linePts({ x: wideX, y: wideY }, F.P, 16), 480, () => {
              setBallPos(null);
              processOutcome(oc, card, nar, false, res);
            });
          }, 300);
      ...
      if (oc === 'ball' && nar.isBlocked) {
        ...
        animateBall(linePts(F.P, { x: dirtX, y: dirtY }, 16), 540, () => {
          ...
          schedT(() => setBlockedPitchBadge(null), 3000);
      ...
      animateBall(linePts(F.P, { x: F.HP.x, y: F.HP.y - 5 }, 16), 550, () => {
        if (!doesSwing(oc)) {
          if (stealHomeEvent) {
            ...
            schedT(() => setStealHomeBadge(null), 3500);
            if (she.success) {
              if (sndRef.current) schedT(() => playSoundLocal('stealHomeSafe'), 100);
              schedT(() => {
                if (playIdRef.current !== thisPlayId) return;
                ...
                if (lead !== prevLeadRef.current) {
                  if (lead === null) { setLeadChangeBadge({ type: 'tied', teamName: teams[bt].name }); schedT(() => setLeadChangeBadge(null), 3500); }
                  else if (prevLeadRef.current !== null && lead !== prevLeadRef.current) { setLeadChangeBadge({ type: 'leadChange', teamName: teams[lead].name }); schedT(() => setLeadChangeBadge(null), 3500); }
                }
                ...
                if (bt === 1 && gs.current.inning >= 9 && homeTotal > awayTotal) {
                  ...
                  triggerFireworks(); triggerFireworks();
                  if (sndRef.current) schedT(() => playSoundLocal('walkOff'), 400);
                  schedT(() => setAnnounceText(`WALK-OFF STEAL OF HOME! ${teams[1].name} win!`), 600);
                  setAutoPlay(false);
                  schedT(() => setShowPostGame(true), 3500);
                }
                rerender();
              ...
              if (sndRef.current) schedT(() => playSoundLocal('stealHomeOut'), 100);
              schedT(() => {
                if (playIdRef.current !== thisPlayId) return;
            ...
            schedT(() => setPopTime(null), 2800);
            animateRunners([[startPos, endPos]], bt === 0 ? '#b91c1c' : '#1e40af', 800);
            if (sbe.success) {
              if (sndRef.current) schedT(() => playSoundLocal('stolenBase'), 600);
              schedT(() => {
              ...
              if (sndRef.current) schedT(() => playSoundLocal('caughtStealing'), 600);
              schedT(() => {
            ...
            schedT(() => setDoubleStealBadge(null), 3500);
            if (sndRef.current) schedT(() => playSoundLocal('doubleSteal'), 600);
            schedT(() => {
              if (playIdRef.current !== thisPlayId) return;
            ...
            if (nar.isFraming) {
              setFramingKey(k => k + 1); // remount the framing SVG (Backlog #171)
              setCatcherFraming(true);
              if (sndRef.current) playSoundLocal('frameMitt');
              schedT(() => setCatcherFraming(false), 800);
              if (nar.isFramed) {
                setFramingBadge({ team: defTeam });
                schedT(() => setFramingBadge(null), 2400);
              }
            }
            schedT(() => { animateBall(linePts(F.C, F.P, 16), 500, () => { setBallPos(null); processOutcome(oc, card, nar, false, res); }); }, 300);
          }
        }
      });
    }, 550 + stepOutDelay + shakeOffDelay + cleatKnockDelay + browWipeDelay + checkRunnerDelay + signNodDelay + rubberKickDelay);
    // Check-swing appeal cue (Backlog #150) — fires for BOTH verdicts, timed to
    // land just after the pitch reaches the plate. The home-plate umpire's ball
    // call has happened; the catcher appeals; the base umpire (the existing 1B
    // umpire SVG) renders the verdict — an OUT punch when the batter went around
    // (strike three; the HP-umpire punch-out also fires inside processOutcome),
    // or the SAFE spread-arms "no swing" signal when he held up (the pitch
    // stands as a ball). A slate-to-amber info-bar badge flashes and the new
    // `checkSwing` sound plays. The mechanical outcome was already resolved at
    // the re-map, so this is purely the cosmetic appeal layer — it runs
    // independently of the swing/non-swing ball-flight dispatch.
    if (nar.isCheckSwing) {
      const csCueDelay = 1000 + stepOutDelay + shakeOffDelay + cleatKnockDelay + browWipeDelay + checkRunnerDelay + signNodDelay + rubberKickDelay;
      schedT(() => {
        if (playIdRef.current !== thisPlayId) return;
        setUmpire1BPhase(nar.csRuledSwing ? 'outCall' : 'safeCall');
        setCheckSwingBadge({ batter, team: bt, ruledSwing: !!nar.csRuledSwing });
        if (sndRef.current) playSoundLocal('checkSwing');
        schedT(() => setUmpire1BPhase('idle'), 1500);
        schedT(() => setCheckSwingBadge(null), 3200);
      }, csCueDelay);
    }
    ...
    if (doesSwing(oc)) {
      schedT(() => {
        ...
        if (nar.isFoulTip) {
          ...
          schedT(() => setFoulTipBadge(null), 3000);
        } else if (nar.isCheckSwing && nar.csRuledSwing) {
          // Check-swing ruled a swing (Backlog #150) — the batter went around
          // but made NO contact, so suppress the standard 'hit' sound. The
          // appeal sound (checkSwing) + badge + base-ump signal fire from the
          // dedicated appeal-cue block below.
        ...
        if (isBrokenBat && !nar.isFoulTip) {
          ...
          if (sndRef.current) schedT(() => playSoundLocal('batCrack'), 20);
          schedT(() => setBrokenBat(null), 950);
        }
        ...
        if (nar.isSqueeze) {
          ...
          schedT(() => setSqueezeBadge(null), 3500);
          if (sndRef.current) schedT(() => playSoundLocal('squeezePlay'), 80);
        }
        ...
        if (nar.isProductiveOut) {
          ...
          schedT(() => setProductiveOutBadge(null), 3500);
        }
        ...
        if (nar.isSacBunt) {
          ...
          schedT(() => setSacBuntBadge(null), 3500);
        ...
        if (nar.isDragBunt) {
          ...
          schedT(() => setDragBuntBadge(null), 3500);
          if (sndRef.current) schedT(() => playSoundLocal('dragBunt'), 80);
          // Trigger a brief offensive-crowd reaction for the hustle hit
          // (similar to first-to-third and infield-single moments).
          if (sndRef.current) schedT(() => triggerCrowdReaction('hit', bt), 1500);
        }
        ...
        if (nar.isComebacker) {
          ...
          schedT(() => setComebackerBadge(null), 3500);
          ...
          if (sndRef.current) schedT(() => playSoundLocal('comebacker'), 60);
        }
        ...
        if (nar.isHitAndRun) {
          ...
          schedT(() => setHitAndRunBadge(null), 3500);
          if (sndRef.current) schedT(() => playSoundLocal('hitAndRun'), 60);
          if (sndRef.current) schedT(() => triggerCrowdReaction('hit', bt), 1500);
        }
        setAnnounceText(nar.text);
        // Exit velocity display (Statcast-style)
        const ev = generateExitVelo(oc);
        if (ev) { setExitVelo(ev); schedT(() => setExitVelo(null), 2500); }
        ...
        if (la !== null) { setLaunchAngle(la); schedT(() => setLaunchAngle(null), 2500); }
        ...
        if (bs !== null) { setBatSpeed(bs); schedT(() => setBatSpeed(null), 2500); }
        // HR distance readout (Statcast-style) — show alongside exit velo on home runs
        if (oc === 'homeRun') {
          const dist = generateHRDistance(ev);
          if (dist) { setHrDistance(dist); schedT(() => setHrDistance(null), 3500); }
        }
        if (['single', 'double', 'triple', 'homeRun', 'error', 'groundOut', 'doublePlay', 'bunt'].includes(oc) || ((res.isSacFly || nar.isTagUp) && ['flyOut', 'lineOut'].includes(oc)) || (res.isFoulSacFly && oc === 'foulOut')) {
           const runnerOc = oc === 'bunt' ? 'single' : ((res.isSacFly || res.isFoulSacFly) ? 'sacFly' : oc);
           ...
           // Force-chain runner paths (Backlog #164) — rebuild the groundOut /
           // doublePlay paths from the same force logic as calculatePlayResult
           // so sprites only animate runners who actually move: forced runners
           // advance, non-forced runners stay planted (no more R3 sprinting
           // home and snapping back to 3B), and R3 only crosses the plate when
           // the engine actually counted the run. Hit-and-run (#148) keeps the
           // stock paths — its engine mutation already matches them.
           if (['groundOut', 'doublePlay'].includes(oc) && !nar.isHitAndRun && !nar.isTriplePlay) {
             const pb = gs.current.bases; // pre-play bases (write happens at trot end)
             rps = [];
             if (pb[0]) {
               // Force in effect: R1 runs into the out at 2B, batter up the line.
               if (pb[1] && pb[2] && res.runs > 0) rps.push([F.B3, F.HP]); // forced run counted
               if (pb[1]) rps.push([F.B2, F.B3]); // R2 forced to 3B
               rps.push([F.B1, F.B2]);
               rps.push([F.HP, F.B1]);
             } else {
               // No force: only a scoring R3 (contact play), a productive-out
               // R2 → 3B advance (#108), and the batter move.
               if (oc === 'groundOut' && res.runs > 0) rps.push([F.B3, F.HP]);
               if (nar.isProductiveOut) rps.push([F.B2, F.B3]);
               rps.push([F.HP, F.B1]);
             }
           }
           ...
           if (rps.some(p => p[0].x === F.HP.x)) {
             if (oc === 'homeRun' && !nar.isInsideThePark) {
               ...
               schedT(() => setBatterPhase('batFlip'), 400);
               schedT(() => setBatterPhase('gone'), 1200);
             } else {
               // ITPHR: no bat flip — batter drops the bat and sprints out of the box.
               schedT(() => setBatterPhase('gone'), 500);
             }
           }
           schedT(() => { if (playIdRef.current === thisPlayId) { gs.current.bases = res.nextBases; rerender(); } }, ftTrotDur);
           // Fresh ball toss from HP umpire to pitcher (Backlog #135) — fires
           ...
           if (oc === 'homeRun' && !nar.isInsideThePark) {
             schedT(() => {
               ...
               schedT(() => setFreshBallToss(null), 900);
               schedT(() => setFreshBallTossBadge(false), 1600);
             }, ftTrotDur + 700);
           }
           ...
           if (oc === 'homeRun') {
             schedT(() => {
               ...
               schedT(() => setCurtainCall(null), 2800);
               schedT(() => setCurtainCallBadge(null), 3400);
             }, ftTrotDur + 1100);
           }
           ...
           if (oc === 'homeRun') {
             schedT(() => {
               ...
               schedT(() => setHighFiveLine(null), 2600);
             }, ftTrotDur + 900);
           }
           ...
           schedT(() => setBatterPhase('gone'), 500);
           schedT(() => { if (playIdRef.current === thisPlayId) { gs.current.bases = res.nextBases; rerender(); } }, troDur);
        }
      }, 1050 + stepOutDelay + shakeOffDelay + cleatKnockDelay + browWipeDelay + checkRunnerDelay + signNodDelay + rubberKickDelay);
      schedT(() => {
        setPitchPhase('idle'); setOutcome(oc);
        const ba = genBallAnimLocal(oc, nar.dir, nar.sub, nar.variant, isGRD);
        // Foul-territory sacrifice fly (Backlog #156) — override the foul-ball
        // target so the popup lands near the corner infielder (1B or 3B side)
        // instead of the random ±90 default. This ensures the fielder doesn't
        // sprint across the diamond to a ball on the opposite foul line.
        // F.F1B and F.F3B are the standard fielding positions; we land a few
        // px outside them along the line into foul territory.
        if (nar.isFoulSacFly && oc === 'foulOut') {
          const fkey = INF_MAP[nar.sub];
          if (fkey && F[fkey]) {
            const cornerTarget = {
              x: F[fkey].x + (nar.sub === 'the first baseman' ? 14 : -14),
              y: F[fkey].y - 6,
            };
            ba.path = arcPts(F.HP, cornerTarget, 130, 28);
            ba.target = cornerTarget;
            ba.dur = 2200;
          }
        }
        ...
        if (nar.isBackstop && oc === 'foulBall') {
          ...
          schedT(() => setBackstopBadge(null), 2400);
          if (sndRef.current) schedT(() => playSoundLocal('screenHit'), 280);
        }
        // Foul ball off the batter's foot (Backlog #162) — flash the recovery
        // pose on the batter, fire the badge, play the dual-stage tick+thud
        // sound, and override the ball trajectory so the foul ricochets sharply
        // down at the batter's feet and skitters off into foul territory. The
        // batter holds the 'offFoot' pose (hop animation + grimace + bat-as-
        // crutch) for ~1.0s before returning to the standard stance. The pose
        // and pacing all complete before the next pitch arrives because the
        // foul-ball outcome doesn't trigger downstream play resolution — it
        // just increments the strike count.
        if (nar.isOffFoot && oc === 'foulBall') {
          // Ball trajectory: short sharp foul ricochet straight down at the
          // batter's foot (just in front + below where the bat tip would be)
          // then skipping a short distance away into foul territory. The
          // batter side (L/R) determines which way the ball skips foul.
          const ofSide = batterSide === 'L' ? -1 : 1;
          const ofFootPt = { x: F.HP.x + ofSide * 6, y: F.HP.y + 4 }; // at the foot
          const ofSkipPt = { x: F.HP.x + ofSide * 22, y: F.HP.y + 16 }; // skipping foul
          ba.path = [...arcPts(F.HP, ofFootPt, 14, 6), ...linePts(ofFootPt, ofSkipPt, 8)];
          ba.target = ofSkipPt;
          ba.dur = 700;
          // Fire the recovery pose on the batter immediately on contact — he
          // pops out of his swing into the hop-and-grimace pose, holds 1.0s,
          // and returns to stance. The setBatterPhase('stance') reset is
          // queued at the end of the pose duration so the next pitch's
          // windup begins from a clean idle.
          schedT(() => setBatterPhase('offFoot'), 60);
          schedT(() => setBatterPhase('stance'), 60 + 1000);
          // Flash the "OFF THE FOOT!" badge and queue the new fouledOffFoot
          // sound timed to land just as the ball ricochets off the foot.
          setOffFootBadge({ batter, team: bt });
          schedT(() => setOffFootBadge(null), 2500);
          if (sndRef.current) schedT(() => playSoundLocal('fouledOffFoot'), 140);
        }
        ...
        if (nar.isInsideThePark && oc === 'homeRun') {
          ...
          schedT(() => setInsideTheParkBadge(null), 4500);
          if (sndRef.current) schedT(() => playSoundLocal('insideTheParkHR'), ba.dur * 0.4);
        }
        // Ground rule double: flash a badge and play a fence-hit sound at the bounce moment.
        if (isGRD) {
          setGrdBadge({ team: bt });
          schedT(() => setGrdBadge(null), 3200);
          if (sndRef.current) schedT(() => playSoundLocal('fenceHit'), ba.dur * 0.5);
        }
        // Wind-affected ball flight (Backlog #152) — flash the wind badge and
        // play the gust sound mid-flight (when the wind visibly takes over the
        // ball), for both the carried-out homer and the knocked-down drive.
        if (nar.isWindAided || nar.isWindBlownBack) {
          setWindBadge({ type: nar.isWindAided ? 'aided' : 'blown', team: bt });
          schedT(() => setWindBadge(null), 3500);
          if (sndRef.current) schedT(() => playSoundLocal('windGust'), Math.max(0, (ba.dur || 1600) * 0.35));
        }
        ...
        if (nar.isTexasLeaguer && oc === 'single') {
          ...
          schedT(() => setTexasLeaguerBadge(null), 3200);
          if (sndRef.current) playSoundLocal('texasLeaguer');
        }
        // Stretched single → double badge + sound (Backlog #159) — the batter
        // rounded first hard and reached second standing up. Pure flavor /
        // badge layer on top of the standard double pathway (engine already
        // re-mapped oc to 'double' pre-CPR). Fires alongside the OF chase
        // animation (which the standard hit branch already routes through
        // OF_MAP[nar.sub]). Pulsing emerald-to-amber badge for 3.5s with
        // hover tooltip naming the batter. The bespoke `stretchDouble`
        // sound layers a sharp bat-on-ball crack with pounding footfalls
        // and a final slide-thump into 2B.
        if (nar.isStretchedDouble && oc === 'double') {
          setStretchedDoubleBadge({ batter, team: bt });
          schedT(() => setStretchedDoubleBadge(null), 3500);
          if (sndRef.current) schedT(() => playSoundLocal('stretchDouble'), 80);
          // Brief offensive-crowd reaction at +1500ms for the hustle hit
          // (matches the first-to-third and infield-single broadcast moments).
          schedT(() => triggerCrowdReaction('hit', bt), 1500);
        }
        // Line-drive double play badge + sound (Backlog #161) — the fielder
        // snared the line drive and threw to the bag for the second out.
        // Fires alongside the standard lineOut catch animation (the ball
        // already routes to the fielder via the lineOut pathway). Pulsing
        // emerald-to-rose badge with a hover tooltip naming the runner and
        // the bag. The bespoke `lineOutDP` sound layers a sharp bat-crack
        // (the line drive) with two glove-pops (catch + throw to the bag).
        // A defensive web-gem crowd reaction fires for the heroics, since
        // doubling off a runner is one of the most-celebrated defensive
        // plays in baseball. Persistence to play history happens in the
        // standard processOutcome / playHistory append below.
        if (nar.isLineOutDP && oc === 'lineOut') {
          setLineOutDPBadge({
            batter,
            runner: lodpRunnerName,
            base: lodpBase,
            team: bt,
          });
          schedT(() => setLineOutDPBadge(null), 3500);
          if (sndRef.current) playSoundLocal('lineOutDP');
          // Defensive crowd reaction — the home crowd cheers when their team
          // turns a line-drive double play (matches webGem/doublePlay behavior).
          schedT(() => triggerCrowdReaction('doublePlay', bt), 700);
        }
        ...
        if (nar.isInfieldSingle && oc === 'single') {
          ...
          schedT(() => setInfieldSingleBadge(null), 3200);
        ...
        if (nar.isThrowError && oc === 'single') {
          ...
          schedT(() => setThrowErrorBadge(null), 3200);
        ...
        if (sndRef.current && ba.carom && !isGRD) { schedT(() => playSoundLocal('fenceHit'), ba.dur * 0.65); }
        if (sndRef.current && oc === 'homeRun' && (nar.sub === 'lf' || nar.sub === 'rf') && Math.random() < 0.35) { schedT(() => playSoundLocal('foulPole'), ba.dur * 0.5); }
        ...
        if (['flyOut', 'lineOut', 'foulOut'].includes(oc)) {
          // For a foul sac fly (Backlog #156), nar.sub was overridden to a
          // corner infielder ("the first baseman" or "the third baseman") so
          // the catch reads as deep down the line. Route the ball there via
          // INF_MAP rather than defaulting to the catcher.
          const fk = (oc === 'foulOut')
            ? ((nar.isFoulSacFly && INF_MAP[nar.sub]) ? INF_MAP[nar.sub] : 'C')
            : (OF_MAP[nar.sub] || INF_MAP[nar.sub]);
          if (fk) animateFielder(fk, F[fk], ba.target, ba.dur);
          // Outfielder backing up the play (Backlog #154) — when a fly/line
          // out goes to an OF, a second OF jogs to a backup spot behind the
          // lead fielder. Suppressed when the play is already a special-
          // flavor case that owns the visual beat (HR robbery / web gem /
          // wind-blown-back — those already have their own focal animations
          // at the catch point). Pure cosmetic — no engine effect.
          if (fk && OF_MAP[nar.sub] && !isRobbedHR && !isWebGem && !nar.isWindBlownBack) {
            const ofbColor = (1 - bt) === 0 ? '#1e40af' : '#b91c1c'; // defensive team color
            triggerOFBackup(fk, ba.target, ba.dur, ofbColor, thisPlayId);
          }
          ...
          if (nar.isInfieldFly) {
            schedT(() => { if (playIdRef.current === thisPlayId) setUmpirePhase('infieldFly'); }, 500);
            schedT(() => { if (playIdRef.current === thisPlayId) setUmpirePhase('idle'); }, 500 + 1900);
            setInfieldFlyBadge({ batter, fielder: nar.sub, team: 1 - bt });
            schedT(() => setInfieldFlyBadge(null), 3500);
            if (sndRef.current) schedT(() => playSoundLocal('glovePop'), ba.dur);
          }
          ...
          if (nar.isTagUp) {
            ...
            schedT(() => { if (playIdRef.current === thisPlayId) { setTagUpBadge({ runner: tuLast, team: bt }); schedT(() => setTagUpBadge(null), 3500); } }, ba.dur);
            if (sndRef.current) schedT(() => playSoundLocal('glovePop'), ba.dur);
          }
          // Foul-territory sacrifice fly badge (Backlog #156) — flash the
          // amber-to-emerald "🎯 FOUL SAC FLY" pill when the runner on 3B
          // tags up and scores after the foulOut catch down the line. Fired
          // at ball-arrival (the catch — the moment the runner may legally
          // leave the bag), with a routine glove pop and offensive crowd
          // reaction. Runner advancement to home is baked into res.nextBases
          // and the runner-path animation set up via runnerOc='sacFly'.
          if (nar.isFoulSacFly && oc === 'foulOut') {
            const fsfRunnerLast = (nar.foulSacFlyRunner || '').split(' ').slice(-1)[0] || 'the runner';
            schedT(() => { if (playIdRef.current === thisPlayId) { setFoulSacFlyBadge({ batter, runner: fsfRunnerLast, fielder: nar.foulSacFlyFielder || 'the fielder', team: bt }); schedT(() => setFoulSacFlyBadge(null), 3500); } }, ba.dur);
            if (sndRef.current) schedT(() => playSoundLocal('glovePop'), ba.dur);
            schedT(() => { if (playIdRef.current === thisPlayId) triggerCrowdReaction('hit', bt); }, ba.dur + 200);
          }
          ...
          if (oc === 'foulOut' && nar.isMaskToss && ba.target) {
            ...
            schedT(() => { if (playIdRef.current === thisPlayId) setMaskToss(null); }, tossStart + 1100);
            schedT(() => { if (playIdRef.current === thisPlayId) setCatcherLookingUp(false); }, ba.dur + 400);
            schedT(() => { if (playIdRef.current === thisPlayId) setMaskTossBadge(null); }, ba.dur + 2800);
          }
          // Web gem sparkle effect at catch point
          if (isWebGem && ba.target) {
            schedT(() => { setWebGem({ x: ba.target.x, y: ba.target.y }); schedT(() => setWebGem(null), 1200); }, ba.dur);
            schedT(() => triggerCrowdReaction('webGem', bt), ba.dur + 200);
          }
          // Cap flies off (Backlog #158) — fire alongside web gem at catch
          // point. The cap pops off the diving fielder, tumbles ~14px to
          // the side, lands with a tiny dust puff. Color is the defensive
          // team's color so the cap visually reads as belonging to the
          // fielder. The `side` controls tumble direction — random ±1
          // for visual variety. Sound is a soft quiet swoosh.
          if (nar.isCapOff && ba.target) {
            const capColor = (1 - bt) === 0 ? '#1e40af' : '#b91c1c';
            const capSide = Math.random() < 0.5 ? -1 : 1;
            const capX = ba.target.x;
            const capY = Math.max(F.HP.y - 80, ba.target.y - 8); // perch above catch point so cap reads as "from the head"
            schedT(() => {
              setCapFlyOff({ x: capX, y: capY, color: capColor, side: capSide, ts: Date.now() });
              schedT(() => setCapFlyOff(null), 1400);
            }, ba.dur);
            schedT(() => setCapOffBadge({ team: 1 - bt }), ba.dur + 50);
            schedT(() => setCapOffBadge(null), ba.dur + 1700);
            if (sndRef.current) schedT(() => playSoundLocal('capFly'), ba.dur + 60);
          }
          ...
          if (isRobbedHR && ba.target) {
            ...
            const fielderName = nar.sub === 'lf' ? 'left fielder' : nar.sub === 'cf' ? 'center fielder' : 'right fielder';
            schedT(() => { setRobbedHRLeap({ x: ba.target.x, y: leapY, color: leapColor }); schedT(() => setRobbedHRLeap(null), 1500); }, ba.dur - 300);
            schedT(() => setRobbedHRBadge({ batter, fielder: fielderName, team: bt }), ba.dur - 100);
            schedT(() => setRobbedHRBadge(null), ba.dur + 3200);
            if (sndRef.current) schedT(() => playSoundLocal('hrRobbed'), ba.dur - 320);
            schedT(() => triggerCrowdReaction('webGem', bt), ba.dur + 200);
            // Cap flies off (Backlog #158) — also fires on robbed HRs.
            // Anchored slightly above the leap so the cap reads as separating
            // from the leaping fielder's head at the snare moment.
            if (nar.isCapOff) {
              const capSide = Math.random() < 0.5 ? -1 : 1;
              schedT(() => {
                setCapFlyOff({ x: ba.target.x, y: leapY - 12, color: leapColor, side: capSide, ts: Date.now() });
                schedT(() => setCapFlyOff(null), 1400);
              }, ba.dur - 100);
              schedT(() => setCapOffBadge({ team: 1 - bt }), ba.dur + 100);
              schedT(() => setCapOffBadge(null), ba.dur + 1700);
              if (sndRef.current) schedT(() => playSoundLocal('capFly'), ba.dur - 40);
            }
          }
        } else if (isG) {
          ...
          if ((nar.isInfieldSingle || nar.isThrowError) && oc === 'single') {
            ...
            // Outfielder backing up the play (Backlog #154) — second OF jogs to
            // a backup spot behind the lead fielder. Suppressed on special
            // flavor cases that own the visual beat: Texas leaguer (the ball
            // drops in shallow — no backup needed), outfield assist (the relay
            // throw is the focal moment), Lost in the Sun (the sun flare is
            // the focal moment), and Ground Rule Double (dead ball, no relay).
            if (!nar.isTexasLeaguer && !nar.isOutfieldAssist && !nar.isOutfieldAssist3B && !nar.isLostInSun && !isGRD) {
              const ofbColor = (1 - bt) === 0 ? '#1e40af' : '#b91c1c';
              triggerOFBackup(ofk, ba.target, ba.dur + 500, ofbColor, thisPlayId);
            }
          }
          ...
          if (nar.isLostInSun && ba.target) {
            ...
            schedT(() => {
              ...
              schedT(() => setSunFlare(null), 1400);
            }, ba.dur - 80);
            schedT(() => setLostInSunBadge({ batter, fielder: fielderName, team: bt }), ba.dur - 60);
            schedT(() => setLostInSunBadge(null), ba.dur + 3200);
            schedT(() => triggerCrowdReaction('error', bt), ba.dur + 250);
          }
          ...
          if (nar.isDroppedFly && ba.target) {
            ...
            schedT(() => setDroppedFlyBadge({ batter, fielder: fielderName, errCode, team: bt }), ba.dur - 20);
            schedT(() => setDroppedFlyBadge(null), ba.dur + 3200);
            schedT(() => triggerCrowdReaction('error', bt), ba.dur + 250);
          }
        }
        animateBall(ba.path, ba.dur, () => {
          if (isGRD) {
            ...
            schedT(() => { setBallPos(null); setMovingFielder(null); processOutcome(oc, card, nar, true, res); }, 700);
          } else if (nar.isInsideThePark && oc === 'homeRun') {
            ...
            // here for the visual.
            schedT(() => animateBall(linePts(ba.target, F.SS, 16), 650, () => animateBall(linePts(F.SS, F.HP, 16), 600, () => { setBallPos(null); setMovingFielder(null); processOutcome(oc, card, nar, true, res); })), 400);
            ...
            // Pitcher backs up home (Backlog #163) — as the relay throw begins
            // its journey from OF → SS → HP, the pitcher sprints from the mound
            // to a position just behind the catcher to back up the throw. The
            // animation duration is timed to the full relay (~740ms = 360 + 380)
            // plus a small head start, so the pitcher arrives at the backup
            // position just as the ball reaches the catcher. After the play
            // closes out (throw back to the mound), the backup sprite clears.
            schedT(() => {
              const pbColor = bt === 0 ? '#b91c1c' : '#1e40af'; // defensive team color
              const pbStart = F.P;
              const pbEnd = { x: F.HP.x, y: F.HP.y + 18 }; // ~18px behind home plate
              animatePitcherHomeBackup(pbStart, pbEnd, 820, pbColor);
              setPitcherHomeBackupBadge({ team: 1 - bt });
              if (sndRef.current) playSoundLocal('pitcherBackup');
              schedT(() => { if (playIdRef.current === thisPlayId) setPitcherHomeBackupBadge(null); }, 3200);
            }, 80);
            schedT(() => {
              setOutfieldAssistBadge({ batter, fielder: nar.oaFielder, runner: nar.oaRunner, team: bt });
              schedT(() => setOutfieldAssistBadge(null), 3500);
                ...
                animateBall(linePts(F.SS, F.HP, 16), 380, () => {
                  ...
                  if (nar.helmetOff) {
                    ...
                    schedT(() => {
                      ...
                      if (sndRef.current) schedT(() => playSoundLocal('helmetThump'), 720);
                      schedT(() => setFlyingHelmet(null), 850);
                    }, 80);
                  }
                  schedT(() => {
                    animateBall(linePts(F.HP, F.P, 16), 480, () => {
                      setBallPos(null); setMovingFielder(null);
                      // Clear the pitcher-home-backup sprite once the throw
                      // returns to the mound — the pitcher has finished his
                      // backup duty. (Backlog #163)
                      setPitcherHomeBackup(null);
            ...
            schedT(() => {
              ...
              schedT(() => setOutfieldAssist3BBadge(null), 3500);
              animateBall(linePts(ba.target, F.SS, 16), 360, () => {
            ...
            if (nar.isInfieldSingle && oc === 'single') {
              ...
              schedT(() => {
                ...
                schedT(() => setUmpire1BPhase('idle'), 1200);
              }, 200 + 380);
              schedT(() => animateBall(linePts(ba.target, F.B1, 16), 380, () => animateBall(linePts(F.B1, F.P, 16), 480, () => { setBallPos(null); setMovingFielder(null); processOutcome(oc, card, nar, true, res); })), 200);
              ...
              schedT(() => animateBall(linePts({ x: F.B1.x + 24, y: F.B1.y + 18 }, F.P, 16), 600, () => { setBallPos(null); setMovingFielder(null); processOutcome(oc, card, nar, true, res); }), 1700);
            } else {
              schedT(() => animateBall(linePts(ba.target, F.SS, 16), 550, () => animateBall(linePts(F.SS, F.P, 16), 550, () => { setBallPos(null); setMovingFielder(null); processOutcome(oc, card, nar, true, res); })), 600);
            }
          } else if (isG) {
            ...
            if (nar.isFieldersChoice) {
              ...
              schedT(() => setFieldersChoiceBadge(null), 3200);
            }
            schedT(() => {
              ...
              if (pitcherCovers) {
                ...
                schedT(() => { if (playIdRef.current === thisPlayId) setPitcherCoverBadge(null); }, 3200);
                if (sndRef.current) schedT(() => playSoundLocal('glovePop'), 400);
              } else {
              ...
              if (nar.isBangBang && !r1) {
                const runnerColor = bt === 0 ? '#b91c1c' : '#1e40af';
                schedT(() => { setBangBangPlay({ color: runnerColor }); schedT(() => setBangBangPlay(null), 1100); }, 280);
                setBangBangBadge({ team: bt });
                schedT(() => setBangBangBadge(null), 3000);
                if (sndRef.current) schedT(() => playSoundLocal('bangBang'), 380);
              ...
              animateBall(linePts(ba.target, cb, 16), 400, () => {
                if (oc === 'doublePlay' && r1) {
                  if (nar.isTriplePlay) {
                    ...
                    schedT(() => setTripleplayBadge(null), 4500);
                  ...
                  schedT(() => {
                    ...
                    schedT(() => setChallengeBadge(null), 2200);
            ...
            schedT(() => { setBallPos(null); setMovingFielder(null); processOutcome(oc, card, nar, true, res); }, 900);
            ...
            schedT(() => setFanCatch(null), 1700);
            schedT(() => { setBallPos(null); setMovingFielder(null); processOutcome(oc, card, nar, true, res); }, 600);
      ...
      }, 1100 + stepOutDelay + shakeOffDelay + cleatKnockDelay + browWipeDelay + checkRunnerDelay + signNodDelay + rubberKickDelay);
    } else schedT(() => setAnnounceText(nar.text), 1100 + stepOutDelay + shakeOffDelay + cleatKnockDelay + browWipeDelay + checkRunnerDelay + signNodDelay + rubberKickDelay);
  }
  ...
  function processOutcome(oc, card, nar, animated = false, result = null) {
    ...
    if (nar.fireTipCap) {
      ...
      schedT(() => setPitcherTipCap(false), 700 + 1400);
      schedT(() => setTipCapBadge(null), 700 + 2400);
    }
    ...
    if ((oc === 'strike' || oc === 'foulBall') && _preStrikesCU === 1 && (g.count?.strikes ?? 0) === 2) {
      ...
      schedT(() => setChokeUpBadge(null), 1600);
    }
    ...
    if (isHit && preRISP && preOuts === 2) {
      ...
      schedT(() => setClutchBadge(null), 3000);
    }
    ...
    if (!nar.isDroppedFly && !nar.isThrowError && ['single', 'double', 'triple', 'homeRun'].includes(oc)) {
      ...
      if (S.hitTypes.size === 4) {
        ...
        triggerFireworks();
        schedT(() => triggerFireworks(), 600);
        if (sndRef.current) schedT(() => playSoundLocal('walkOff'), 500);
        schedT(() => setAnnounceText(`🚴 ${bat} HAS HIT FOR THE CYCLE! 🚴`), 800);
        schedT(() => setCycleWatch(null), 6000);
        ...
        schedT(() => setCycleWatch(null), 4000);
      }
    ...
    if (oc === 'strikeout' || isCalledK) {
      ...
      if (!isDropped3rd) {
        ...
        schedT(() => setUmpirePhase('idle'), 1200);
      }
      ...
      if (fpEligible) {
        ...
        schedT(() => setFistPumpAct(null), 1100);
        setFistPumpBadge({ batter: bat, pitcher: fpPitcherName, team: fpDefIdx });
        schedT(() => setFistPumpBadge(null), 2800);
        if (sndRef.current) schedT(() => playSoundLocal('fistPump'), 120);
      }
    }
    ...
    if (nar.isEjection && oc === 'strikeout') {
      ...
      schedT(() => {
        // Walk-out animation
        const __gen = gameGen; // Backlog #165: kill this walker if the game is reset mid-walk
        const sStart = performance.now();
        const tickWalk = (now) => {
          if (__gen !== gameGen) { setEjectionAct(null); setEjectionUmpireActive(false); return; }
            ...
            schedT(() => {
              ...
              const tickBack = (now2) => {
                ...
                if (rawT2 < 1) requestAnimationFrame(tickBack);
                  ...
                  schedT(() => setEjectionBadge(null), 600);
                }
              };
    ...
    if ((oc === 'strikeout' || isCalledK) && !isDropped3rd && !nar.isEjection && !gs.current.gameOver && Math.random() < 0.3) {
      ...
      schedT(() => {
        if (gs.current.gameOver) return;
        const __gen = gameGen; // Backlog #165: kill the bat boy if the game is reset mid-jog
        // Stage 1: walk out from dugout to home plate
        const sStart = performance.now();
        const tickOut = (now) => {
          if (__gen !== gameGen) { setBatBoyAct(null); return; }
    ...
    if (oc === 'homeRun') {
      ...
      if (res.runs === 4) {
        ...
        schedT(() => triggerFireworks(), 600);
        schedT(() => triggerFireworks(), 1200);
        if (sndRef.current) schedT(() => playSoundLocal('walkOff'), 1000);
        schedT(() => setAnnounceText('🏆 GRAND SLAM! 🏆'), 1500);
      }
    }
    ...
    if (oc === 'homeRun') {
      schedT(() => triggerCrowdReaction(res.runs === 4 ? 'grandSlam' : 'homeRun', bt), 600);
    } else if ((oc === 'strikeout' && !isDropped3rd) || isCalledK) {
      schedT(() => triggerCrowdReaction('strikeout', bt), 400);
    } else if (oc === 'error') {
      schedT(() => triggerCrowdReaction('error', bt), 400);
    } else if (oc === 'doublePlay') {
      schedT(() => triggerCrowdReaction('doublePlay', bt), 400);
    }
    ...
    if (isWildPitch) {
      ...
      if (sndRef.current) schedT(() => playSoundLocal('wildPitch'), 400);
      schedT(() => { addLog(wpText, 'play'); setAnnounceText(wpText); rerender(); }, 600);
    }
    ...
    if (isClosePlay) {
      ...
      if (sndRef.current) schedT(() => playSoundLocal('slideHome'), 600);
      ...
      schedT(() => { addLog(plateText, 'play'); setAnnounceText(plateText); }, 800);
      // Flying helmet (Backlog #96) — ~50% chance the helmet pops off on the
    ...
    if (currentLead !== prevLeadRef.current && (lcAwayScore + lcHomeScore) > 0) {
      if (currentLead === null && prevLeadRef.current !== null) {
        ...
        schedT(() => setLeadChangeBadge(null), 3500);
    ...
    if (bt === 1 && g.inning >= 9 && homeScore > awayScore) {
      ...
      if (sndRef.current) schedT(() => playSoundLocal('walkOff'), 800);
      schedT(() => { setAnnounceText(`WALK-OFF! ${teams[1].name} win!`); }, isClosePlay ? 2500 : 500);
      setAutoPlay(false); procRef.current = false; schedT(() => setShowPostGame(true), 3500); rerender(); return;
    }
    if (bt === 0 && g.inning >= 9 && nO >= 3 && homeScore > awayScore) {
      g.gameOver = true;
      g.lob[bt] += g.bases.filter(Boolean).length; // LOB for the final (game-ending) half-inning, which bypasses transitionHalf
      g.message = `${teams[1].name} win! Final: ${homeScore}-${awayScore}`;
      setAutoPlay(false); procRef.current = false; schedT(() => setShowPostGame(true), 2000); rerender(); return;
    }
    ...
    } else if (nO >= 3) schedT(() => transitionHalf(), isClosePlay ? 3500 : 1500);
    else {
  ...
  function transitionHalf() {
    ...
    const crowdWaveEligible = !g.gameOver && !is7thStretch && !isMiddleOfFifth
      ...
      && Math.random() < 0.25;
    // Beach ball in the stands (Backlog #151) — atmospheric crowd moment shown
    // during mid/late half-inning transitions in innings 3-8 (~16% per eligible
    // transition). Suppressed during the stretch frame, the middle-of-the-5th
    // grounds-crew drag, any game-ending half, and when the crowd wave is
    // already firing this transition (the two are stands-wide overlays and
    // shouldn't collide — the else-if chain already enforces this, but the
    // explicit guard keeps the intent clear).
    const beachBallEligible = !g.gameOver && !is7thStretch && !isMiddleOfFifth
      && !crowdWaveEligible
      && g.inning >= 3 && g.inning <= 8
      && Math.random() < 0.16;
    // Mascot dance on the dugout roof (Backlog #153) — between-innings bit
    // shown during some half-inning transitions in innings 2-8 (~14% per
    // eligible transition). Mutually exclusive with the crowd wave and the
    // beach ball (one stands/atmosphere overlay per transition), and
    // suppressed during the stretch frame, the middle-of-the-5th grounds-crew
    // drag, and any game-ending half — same suppression family as the others.
    const mascotEligible = !g.gameOver && !is7thStretch && !isMiddleOfFifth
      && !crowdWaveEligible && !beachBallEligible
      && g.inning >= 2 && g.inning <= 8
      && Math.random() < 0.14;
    ...
    if (plateSweepEligible) {
      ...
      schedT(() => setUmpirePhase('idle'), 2300);
      ...
      if (sndRef.current) schedT(() => playSoundLocal('plateSweep'), 150);
    }
    ...
    if (organChargeEligible) {
      setOrganCharge(true);
      if (sndRef.current) schedT(() => playSoundLocal('organCharge'), 250);
      ...
      schedT(() => setOrganCharge(false), 2400);
    }
    // Outfielders warm-up tosses (Backlog #160) — ~70% chance per eligible
    // transition that two OFs play catch in shallow OF during the inning
    // summary card. Suppressed when the game is over (no need to warm up
    // for a post-game modal), during the 7th-inning stretch (the long
    // stretch animation owns the moment), during the middle-of-the-5th
    // grounds-crew drag (the field is being groomed), and during the
    // crowd wave / mascot / beach-ball (stands-wide overlays — keep one
    // atmospheric focus). The ofWarmup is a foreground field overlay,
    // not a stands overlay, so it coexists fine with organ CHARGE and
    // umpire plate sweep (both happen at the plate / off-field). The
    // toss timeline fits inside the 2500ms inning summary window: three
    // tosses x ~950ms = ~2850ms, but the SVG fades out at 92% of duration
    // so the visible ball is gone by ~2620ms — safely before the
    // summary clears and the new at-bat begins. Pure cosmetic.
    const ofWarmupEligible = !g.gameOver && !is7thStretch && !isMiddleOfFifth
      && !crowdWaveEligible && !beachBallEligible && !mascotEligible
      && Math.random() < 0.7;
    if (ofWarmupEligible) {
      // Pick two adjacent OFs to throw to (LF↔CF or CF↔RF — never LF↔RF
      // which is too far for a casual warmup toss). 50/50 split between
      // the two pairings.
      const pairing = Math.random() < 0.5 ? ['LF', 'CF'] : ['CF', 'RF'];
      // Defensive team is `1 - g.half` (the team that will be taking the
      // field for the upcoming half-inning, since g.half hasn't toggled
      // yet). Color them in the defensive team's color so it reads as
      // "this team is taking the field for the next half."
      const nextDefIdx = 1 - g.half;
      const wuColor = nextDefIdx === 1 ? '#b91c1c' : '#1e40af';
      setOfWarmupKey(k => k + 1);
      setOfWarmup({ fromKey: pairing[0], toKey: pairing[1], color: wuColor, ts: Date.now() });
      if (sndRef.current) schedT(() => playSoundLocal('ofWarmup'), 200);
      // Clear ~150ms before the inning summary clears so the ball is gone
      // before the new at-bat begins drawing.
      schedT(() => setOfWarmup(null), 2350);
    }
    schedT(() => {
      setInningSummary(null);
      const doTransition = () => {
        // LOB: credit the batting team (g.half, still pre-toggle here) for any
        // runners stranded on base as this half-inning ends.
        g.lob[g.half] += g.bases.filter(Boolean).length;
        ...
        pileRef.current = []; setDrawnPile(pileRef.current); setOutcome(null); setReplayIdx(-1); setHalfStartIdx(playHistLenRef.current); rerender();
      ...
      if (is7thStretch) {
        ...
        schedT(() => { setSeventhStretch(false); doTransition(); }, 4000);
        ...
        schedT(() => { setGroundsCrewActive(false); doTransition(); }, 3500);
        ...
        schedT(() => { setCrowdWaveActive(false); doTransition(); }, 3600);
      } else if (beachBallEligible) {
        // Beach ball in the stands (Backlog #151) — bump the key so the SVG
        // <animateMotion>/<animate> elements remount and start fresh, then show
        // the ball for ~3.4s before resuming play. A soft crowd "whoop"
        // (reusing the fanCheer sound) plays as it appears. Pure cosmetic.
        setBeachBallKey(k => k + 1);
        setBeachBallActive(true);
        if (sndRef.current) schedT(() => playSoundLocal('fanCheer'), 200);
        schedT(() => { setBeachBallActive(false); doTransition(); }, 3400);
      } else if (mascotEligible) {
        // Mascot dance on the dugout roof (Backlog #153) — bump the key so the
        // SVG animations remount fresh, pick a dugout side at random (x≈112 is
        // the home/left dugout roof, x≈508 the away/right), and let the mascot
        // dance for ~3.4s before resuming play. The bouncy 'mascotDance' riff
        // plays as it pops up. Pure cosmetic.
        setMascotKey(k => k + 1);
        setMascotAct({ x: Math.random() < 0.5 ? 112 : 508 });
        if (sndRef.current) schedT(() => playSoundLocal('mascotDance'), 250);
        schedT(() => { setMascotAct(null); doTransition(); }, 3400);
      } else {
        doTransition();
      }
    }, 2500);
  ...
  function replayPlay(idx) {
    ...
    // Check-swing appeal replay (Backlog #150) — mirror the cosmetic appeal cues
    // (base-umpire signal + badge + sound) at a timing that matches live play.
    // The mechanical outcome (strikeout if ruled a swing, ball otherwise) is
    // already baked into p.outcome, so it replays correctly through the standard
    // path; this just re-fires the appeal layer on top, independent of the
    // ball-flight branch below.
    if (p.isCheckSwing) {
      schedT(() => {
        setUmpire1BPhase(p.csRuledSwing ? 'outCall' : 'safeCall');
        setCheckSwingBadge({ batter: p.batter, team: bt, ruledSwing: !!p.csRuledSwing });
        if (sndRef.current) playSoundLocal('checkSwing');
        schedT(() => setUmpire1BPhase('idle'), 1500);
        schedT(() => setCheckSwingBadge(null), 3200);
      }, 700);
    ...
    if (p.isBalk) {
      ...
      schedT(() => {
        ...
        if (rps.length) animateRunners(rps, bt === 0 ? '#b91c1c' : '#1e40af', 800);
        schedT(() => { procRef.current = false; }, 900);
      }, 500);
      return;
    }
    ...
    if (p.isPitchClock) {
      ...
      schedT(() => setPitchClockBadge(null), 2600);
      schedT(() => { procRef.current = false; }, 800);
      return;
    }
      ...
      schedT(() => {
        ...
        animateBall(linePts(poOrigin, poBag, 14), poBallOutDur, () => {
          ...
          if (p.pickoffOut) {
            ...
            schedT(() => setPickoffBadge(null), 2600);
            schedT(() => { procRef.current = false; }, 950);
          } else {
    ...
    setDispensedCard(p.card); setDispensing(false); schedT(() => setDispensing(true), 30);
    setDrawnPile(p.pileSnapshot || []);
      ...
      schedT(() => {
        ...
        animateBall(linePts(F.P, { x: chinX, y: chinY }, 16), 500, () => {
          ...
          animateBall(linePts({ x: chinX, y: chinY }, F.C, 14), 280, () => {
            setBallPos(null);
            schedT(() => setBatterPhase('stance'), 250);
            schedT(() => { setPitchPhase('idle'); setAnnounceText(p.narration); procRef.current = false; }, 600);
          });
        });
      }, 550);
      return;
    }
    // Pitchout replay (Backlog #155) — re-route the pitch wide of the strike
    // zone toward the catcher's standing pitchout position, fire the pitchout
    // badge + sound, then animate the ball back to the pitcher. Mirrors the
    // live-play timing exactly.
    if (p.outcome === 'ball' && p.isPitchout) {
      const replayBatterSide = (PLAYER_HANDS[p.batter] === 'L' || PLAYER_HANDS[p.batter] === 'S') ? 'L' : 'R';
      const wideX = F.HP.x + (replayBatterSide === 'L' ? -22 : 22);
      const wideY = F.HP.y - 10;
      schedT(() => {
        setPitchPhase('throw'); setCatcherSigns(0);
        animateBall(linePts(F.P, { x: wideX, y: wideY }, 16), 520, () => {
          if (sndRef.current) playSoundLocal('pitchout');
          setPitchoutBadge({ team: 1 - bt });
          schedT(() => setPitchoutBadge(null), 3000);
          schedT(() => {
            animateBall(linePts({ x: wideX, y: wideY }, F.P, 16), 480, () => {
              setBallPos(null);
    ...
    schedT(() => { setPitchPhase('throw'); setCatcherSigns(0); animateBall(linePts(F.P, { x: F.HP.x, y: F.HP.y - 5 }, 16), 550); }, 550);
    ...
    schedT(() => {
      if (doesSwing(p.outcome)) {
        ...
        if (p.isFoulTip) {
          ...
          schedT(() => setFoulTipBadge(null), 3000);
        } else if (p.isCheckSwing && p.csRuledSwing) {
          // Check-swing ruled a swing (Backlog #150) — no contact; suppress the
          // 'hit' sound. The appeal cues fire from the dedicated block above.
        ...
        if (p.isDropped3rd) {
          ...
          // Sac-fly / foul-sac-fly replay mapping (Backlog #171) — mirror the
          // live runnerOc mapping so the runner scoring from 3B actually
          // animates. Previously the raw outcome (flyOut/lineOut/foulOut) was
          // passed and computeRunnerPaths returned [] (runners hold on a fly),
          // so the scoring runner never moved in the replay. isSacFly lives on
          // the persisted resultData; isFoulSacFly is a top-level flag.
          const replayIsSacFly = (p.resultData && (p.resultData.isSacFly || p.resultData.isFoulSacFly)) || p.isFoulSacFly;
          const replayRunnerOc = p.outcome === 'bunt' ? 'single' : (replayIsSacFly ? 'sacFly' : p.outcome);
          const rrps = computeRunnerPaths([...p.preBases], replayRunnerOc);
          ...
          // Force-chain runner paths replay (Backlog #164) — mirror the live
          // groundOut/doublePlay rebuild from p.preBases + the persisted
          // p.resultData.runs so replays animate exactly the runners who
          // moved (forced runners advance, non-forced runners hold).
          if (['groundOut', 'doublePlay'].includes(p.outcome) && !p.isHitAndRun && !p.isTriplePlay) {
            const pb = p.preBases;
            const prRuns = (p.resultData && p.resultData.runs) || 0;
            rrps.length = 0;
            if (pb[0]) {
              if (pb[1] && pb[2] && prRuns > 0) rrps.push([F.B3, F.HP]);
              if (pb[1]) rrps.push([F.B2, F.B3]);
              rrps.push([F.B1, F.B2]);
              rrps.push([F.HP, F.B1]);
            } else {
              if (p.outcome === 'groundOut' && prRuns > 0) rrps.push([F.B3, F.HP]);
              if (p.isProductiveOut) rrps.push([F.B2, F.B3]);
              rrps.push([F.HP, F.B1]);
            }
          }
          const finalTrotDur = ((p.isFirstToThird || p.isOutfieldAssist3B) && p.outcome === 'single') ? 3200 : trotDur;
          // Overturned-challenge replay (Backlog #171) — the play is persisted
          // as a 'single' (the post-review result), but the batter shouldn't
          // trot to 1B until AFTER the review verdict, mirroring live. Defer the
          // trot to the challenge block below; suppress the immediate one here.
          if (!(p.isChallenge && p.challengeOverturned && p.outcome === 'single')) {
    ...
    schedT(() => {
      ...
      // Overturned-challenge replay (Backlog #171) — persisted as a 'single'
      // with an infield `sub`, which would route a clean base hit to the
      // outfield. Route the ball as the original close groundout to first
      // instead, so the replay reads grounder → review → safe like live play.
      const replayBallOutcome = (p.isChallenge && p.challengeOverturned && p.outcome === 'single') ? 'groundOut' : p.outcome;
      const ba = genBallAnimLocal(replayBallOutcome, p.dir, p.sub, p.variant, p.isGRD || false);
      // Foul-territory sacrifice fly replay (Backlog #156) — mirror the live
      // ba.target override so the popup arcs to the corner infielder, not the
      // standard random foul direction. p.sub was persisted as "the first
      // baseman" or "the third baseman" for foul sac flies.
      if (p.isFoulSacFly && p.outcome === 'foulOut') {
        const fkey = INF_MAP[p.sub];
        if (fkey && F[fkey]) {
          const cornerTarget = {
            x: F[fkey].x + (p.sub === 'the first baseman' ? 14 : -14),
            y: F[fkey].y - 6,
          };
          ba.path = arcPts(F.HP, cornerTarget, 130, 28);
          ba.target = cornerTarget;
      ...
      // Foul ball off the batter's foot replay (Backlog #162) — mirror the
      // live trajectory override + recovery pose + badge + sound at identical
      // timings using the persisted p.isOffFoot flag. Batter side derives from
      // the live useMemo batterSide so the foul direction matches whichever
      // hand the persisted batter actually hits with.
      if (p.isOffFoot && p.outcome === 'foulBall') {
        // Side derives from the REPLAYED batter's hand (Backlog #171), not the
        // live batterSide — otherwise the foul skips the wrong way whenever the
        // current hitter bats from the opposite side of the replayed one.
        const ofReplaySide = (PLAYER_HANDS[p.batter] === 'L' || PLAYER_HANDS[p.batter] === 'S') ? 'L' : 'R';
        const ofSide = ofReplaySide === 'L' ? -1 : 1;
        const ofFootPt = { x: F.HP.x + ofSide * 6, y: F.HP.y + 4 };
        const ofSkipPt = { x: F.HP.x + ofSide * 22, y: F.HP.y + 16 };
        ba.path = [...arcPts(F.HP, ofFootPt, 14, 6), ...linePts(ofFootPt, ofSkipPt, 8)];
        ba.target = ofSkipPt;
        ba.dur = 700;
        schedT(() => setBatterPhase('offFoot'), 60);
        schedT(() => setBatterPhase('stance'), 60 + 1000);
        setOffFootBadge({ batter: p.batter, team: bt });
        schedT(() => setOffFootBadge(null), 2500);
        if (sndRef.current) schedT(() => playSoundLocal('fouledOffFoot'), 140);
      }
      ...
      // Wind-affected ball flight replay (Backlog #152) — mirror the live-play
      // wind badge and mid-flight gust sound for both the carried-out homer
      // and the knocked-down drive. The ball trajectory replays naturally from
      // the persisted outcome / dir / variant (the blown-back drive stored
      // variant 2, so it dies at the wall just like the live play).
      if (p.isWindAided || p.isWindBlownBack) {
        setWindBadge({ type: p.isWindAided ? 'aided' : 'blown', team: bt });
        schedT(() => setWindBadge(null), 3500);
        if (sndRef.current) schedT(() => playSoundLocal('windGust'), Math.max(0, (ba.dur || 1600) * 0.35));
      }
      // Stretched single → double replay (Backlog #159) — the play was stored
      // with outcome === 'double' (re-mapped at draw time), so the standard
      // hit-replay flow handles runner advancement / batter to 2B. We just
      // flash the emerald-to-amber badge and play the stretchDouble sound at
      // identical timings to live play (crack at t=0, footfalls, slide-thump).
      // Brief offensive-crowd reaction at +1500ms mirrors the live hustle cue.
      if (p.isStretchedDouble && p.outcome === 'double') {
        setStretchedDoubleBadge({ batter: p.batter, team: bt });
        schedT(() => setStretchedDoubleBadge(null), 3500);
        if (sndRef.current) schedT(() => playSoundLocal('stretchDouble'), 80);
        schedT(() => triggerCrowdReaction('hit', bt), 1500);
      }
      // Line-drive double play replay (Backlog #161) — mirror the live badge,
      // sound, and crowd reaction at identical timings. The standard lineOut
      // replay flow handles the catch animation; the engine-level mutation
      // (runner cleared from preBases, +2 outs) is already baked into the
      // persisted resultData so res.nextBases / outs flow correctly on replay.
      if (p.isLineOutDP && p.outcome === 'lineOut') {
        setLineOutDPBadge({
          batter: p.batter,
          runner: p.lodpRunner,
          base: p.lodpBase,
      ...
      if (['flyOut', 'lineOut', 'foulOut'].includes(p.outcome)) {
        // For a foul sac fly (Backlog #156), p.sub was persisted as a corner
        // infielder ("the first baseman" or "the third baseman") so the catch
        // visibly happens down the line. Route the ball there via INF_MAP
        // rather than defaulting to the catcher.
        const fk = (p.outcome === 'foulOut')
          ? (p.isFoulSacFly && INF_MAP[p.sub] ? INF_MAP[p.sub] : 'C')
          : (OF_MAP[p.sub] || INF_MAP[p.sub]);
        if (fk) animateFielder(fk, F[fk], ba.target, ba.dur);
        // Outfielder backing up the play replay (Backlog #154) — mirror the
        // live trigger on OF fly/line outs (suppressed on robbed HR / web gem /
        // wind-blown-back, matching the live guards).
        if (fk && OF_MAP[p.sub] && !p.isRobbedHR && !p.isWebGem && !p.isWindBlownBack) {
          const ofbColor = (1 - bt) === 0 ? '#1e40af' : '#b91c1c';
          triggerOFBackup(fk, ba.target, ba.dur, ofbColor, playIdRef.current);
        }
      }
      ...
      if (p.isInfieldFly && p.outcome === 'flyOut') {
        schedT(() => setUmpirePhase('infieldFly'), 500);
        schedT(() => setUmpirePhase('idle'), 500 + 1900);
        setInfieldFlyBadge({ batter: p.batter, fielder: p.sub, team: 1 - bt });
        schedT(() => setInfieldFlyBadge(null), 3500);
        if (sndRef.current) schedT(() => playSoundLocal('glovePop'), ba.dur);
      }
      else if (p.isInsideThePark && p.outcome === 'homeRun') { const ofk = OF_MAP[p.sub] || 'CF'; animateFielder(ofk, F[ofk], ba.target, ba.dur + 600); if (sndRef.current) schedT(() => playSoundLocal('fenceHit'), ba.dur * 0.55); }
      ...
      else if (['single', 'double', 'triple', 'error'].includes(p.outcome) && !p.isGRD) {
        const ofk = OF_MAP[p.sub] || 'CF'; animateFielder(ofk, F[ofk], ba.target, ba.dur + 500);
        // Outfielder backing up the play replay (Backlog #154) — mirror the
        // live trigger on OF hits (suppressed on Texas leaguer / outfield
        // assist / 3B assist / lost in sun, matching the live guards).
        if (!p.isTexasLeaguer && !p.isOutfieldAssist && !p.isOutfieldAssist3B && !p.isLostInSun) {
          const ofbColor = (1 - bt) === 0 ? '#1e40af' : '#b91c1c';
          triggerOFBackup(ofk, ba.target, ba.dur + 500, ofbColor, playIdRef.current);
        }
      }
      ...
      if (p.isTagUp && ['flyOut', 'lineOut'].includes(p.outcome)) {
        ...
        schedT(() => setTagUpBadge(null), 3500);
        if (sndRef.current) schedT(() => playSoundLocal('glovePop'), ba.dur);
      }
      // Foul-territory sacrifice fly replay (Backlog #156) — mirror the live
      // amber-to-emerald badge + glove pop + crowd reaction at the catch.
      // The runner-from-3B-scoring animation is handled by the standard
      // runner-path block above (sacFly path = [B3, HP]); the post-state run
      // count + bases come from the persisted resultData.
      if (p.isFoulSacFly && p.outcome === 'foulOut') {
        const fsfLast = (p.foulSacFlyRunner || '').split(' ').slice(-1)[0] || 'the runner';
        setFoulSacFlyBadge({ batter: p.batter, runner: fsfLast, fielder: p.foulSacFlyFielder || 'the fielder', team: bt });
        schedT(() => setFoulSacFlyBadge(null), 3500);
        if (sndRef.current) schedT(() => playSoundLocal('glovePop'), ba.dur);
        schedT(() => triggerCrowdReaction('hit', bt), ba.dur + 200);
      }
      ...
      if (p.isPitcherCover && p.outcome === 'groundOut') {
        const coverColor = bt === 0 ? '#1e40af' : '#b91c1c';
        schedT(() => animatePitcherCover(F.P, F.B1, 380, coverColor), Math.max(0, ba.dur - 380));
        setPitcherCoverBadge({ team: 1 - bt });
        schedT(() => setPitcherCoverBadge(null), 3200);
        if (sndRef.current) schedT(() => playSoundLocal('glovePop'), ba.dur);
        schedT(() => setPitcherCover(null), ba.dur + 700);
      }
      ...
      if (p.outcome === 'foulOut' && p.isMaskToss && ba.target) {
        ...
        schedT(() => setMaskToss(null), tossStart + 1100);
        schedT(() => setCatcherLookingUp(false), ba.dur + 400);
        schedT(() => setMaskTossBadge(null), ba.dur + 2800);
      }
      ...
      if (p.outcome === 'doublePlay' && sndRef.current) {
        if (p.isTriplePlay) {
          ...
          schedT(() => playSoundLocal('triplePlay'), ba.dur + 350);
          setTripleplayBadge({ batter: p.batter, team: bt });
          schedT(() => setTripleplayBadge(null), 4500);
          schedT(() => triggerCrowdReaction('webGem', bt), ba.dur + 700);
          schedT(() => triggerFireworks(), ba.dur + 350);
          schedT(() => triggerFireworks(), ba.dur + 1050);
        } else {
          schedT(() => playSoundLocal('doublePlayTurn'), ba.dur + 350);
        }
      }
      if (p.isHR) triggerFireworks();
      // Dropped 3rd strike replay — passed ball sound + badge, no umpire punch-out
      if (p.isDropped3rd) {
        if (sndRef.current) schedT(() => playSoundLocal('passedBall'), ba.dur);
        setPassedBallBadge({ batter: p.batter, team: bt });
        schedT(() => setPassedBallBadge(null), 3200);
      } else if (p.outcome === 'strikeout') {
        // Umpire punch-out on strikeout replay
        setUmpirePhase('punchOut'); schedT(() => setUmpirePhase('idle'), 1200);
      }
      ...
      if (p.outcome === 'error' && ba.target) { schedT(() => { setErrorBobble({ x: ba.target.x, y: ba.target.y }); if (sndRef.current) playSoundLocal('bobble'); schedT(() => setErrorBobble(null), 1300); }, ba.dur + 200); }
      ...
      if (p.isRobbedHR && ba.target) {
        ...
        schedT(() => { setRobbedHRLeap({ x: ba.target.x, y: leapY, color: leapColor }); schedT(() => setRobbedHRLeap(null), 1500); }, ba.dur - 300);
        schedT(() => setRobbedHRBadge({ batter: p.batter, fielder: fielderName, team: bt }), ba.dur - 100);
        schedT(() => setRobbedHRBadge(null), ba.dur + 3200);
        if (sndRef.current) schedT(() => playSoundLocal('hrRobbed'), ba.dur - 320);
        // Cap flies off replay (Backlog #158) — mirror the live cap-fly at
        // identical timing during the robbed-HR replay branch.
        if (p.isCapOff) {
          const capSide = Math.random() < 0.5 ? -1 : 1;
          schedT(() => {
            setCapFlyOff({ x: ba.target.x, y: leapY - 12, color: leapColor, side: capSide, ts: Date.now() });
      ...
      if (p.isLostInSun && ba.target) {
        ...
        schedT(() => setLostInSunBadge({ batter: p.batter, fielder: fielderName, team: bt }), ba.dur - 60);
        schedT(() => setLostInSunBadge(null), ba.dur + 3200);
      }
      ...
      if (p.isDroppedFly && ba.target) {
        ...
        schedT(() => setDroppedFlyBadge({ batter: p.batter, fielder: fielderName, errCode, team: bt }), ba.dur - 20);
        schedT(() => setDroppedFlyBadge(null), ba.dur + 3200);
      }
      ...
      if (p.isOutfieldAssist && p.outcome === 'single') {
        ...
        schedT(() => setOutfieldAssistBadge(null), 3500);
        if (sndRef.current) schedT(() => playSoundLocal('gunDownAtPlate'), ba.dur + 400);
        schedT(() => triggerCrowdReaction('webGem', 1 - bt), ba.dur + 600);
        // Pitcher backs up home replay (Backlog #163) — mirror the live
        // pitcher-backup sprite + badge + sound at identical timing using
        // the persisted p.isOutfieldAssist flag. The relay throw replay
        // doesn't reuse the same animateBall chain as live (since the
        // ball-flight is driven by ba.path), so we kick the backup sprite
        // off here directly. Cleared by a follow-up timer at the end of
        // the play (the OF-assist replay flow doesn't loop back through
        // animateBall for the throw-to-mound, so we auto-clear at the
        // estimated full-play duration).
        schedT(() => {
          const pbColor = bt === 0 ? '#b91c1c' : '#1e40af';
          const pbStart = F.P;
          const pbEnd = { x: F.HP.x, y: F.HP.y + 18 };
          animatePitcherHomeBackup(pbStart, pbEnd, 820, pbColor);
          setPitcherHomeBackupBadge({ team: 1 - bt });
          if (sndRef.current) playSoundLocal('pitcherBackup');
          schedT(() => setPitcherHomeBackupBadge(null), 3200);
          schedT(() => setPitcherHomeBackup(null), ba.dur + 2200);
        }, 80);
      ...
      if (p.isBangBang && !p.preBases[0]) {
        const runnerColor = bt === 0 ? '#b91c1c' : '#1e40af';
        schedT(() => { setBangBangPlay({ color: runnerColor }); schedT(() => setBangBangPlay(null), 1100); }, ba.dur - 120);
        setBangBangBadge({ team: bt });
        schedT(() => setBangBangBadge(null), 3000);
        if (sndRef.current) schedT(() => playSoundLocal('bangBang'), ba.dur - 20);
      ...
      if (p.fireTipCap) {
        ...
        schedT(() => setPitcherTipCap(false), tcDelay + 1400);
        schedT(() => setTipCapBadge(null), tcDelay + 2400);
      }
      // Manager challenge / replay review (Idea #80) — replay the review overlay,
      // badge, and verdict timing identical to live play. UPHELD challenges are
      // persisted as 'groundOut' (the original out call) and replay through the
      // standard groundOut flow. OVERTURNED challenges are persisted as 'single'
      // (the post-review result): the ball above is rerouted as the original
      // groundout to first, and here — after the verdict — we run the deferred
      // batter-ruled-safe trot to 1B (Backlog #171), mirroring live ordering.
      if (p.isChallenge) {
        ...
        schedT(() => setChallengeReview({ phase: p.challengeOverturned ? 'overturned' : 'confirmed' }), reviewDelay + 1600);
        schedT(() => { setChallengeReview(null); setChallengeBadge(null); }, reviewDelay + 2800);
        if (p.challengeOverturned && p.outcome === 'single') {
          // Ruled safe — trot the batter (and any forced runners) per the
          // single result, a beat after the verdict reveals.
          const safeTrot = computeRunnerPaths([...p.preBases], 'single');
          schedT(() => animateRunners(safeTrot, bt === 0 ? '#b91c1c' : '#1e40af', 1100), reviewDelay + 2200);
        }
      }
    ...
    schedT(() => { setPitchPhase('idle'); setBatterPhase('stance'); }, 2500);
  }
  ...
  if (phase === 'setup') {
    return (
      <div className="min-h-screen bg-gray-100 flex flex-col items-center p-10 font-sans text-gray-900">
        <h1 className="text-4xl font-black text-blue-900 mb-2 tracking-tight uppercase">⚾ Card Baseball v93</h1>
  ...
  if (phase === 'season' && seasonResults) {
    const { standings, hrLeaders, avgLeaders, rbiLeaders, obpLeaders = [], gamesPerMatchup } = seasonResults;
      ...
      <div className="min-h-screen bg-gray-100 p-6 font-sans text-gray-900">
        <div className="max-w-4xl mx-auto">
          ...
          <div className="flex justify-center gap-2 mb-6">
            {['standings', 'hr', 'avg', 'obp', 'rbi'].map(tab => (
              <button key={tab} onClick={() => setSeasonTab(tab)} className={`px-5 py-2 rounded-lg font-bold text-sm uppercase tracking-wide transition-colors cursor-pointer ${seasonTab === tab ? 'bg-blue-900 text-white shadow-lg' : 'bg-white text-gray-600 hover:bg-gray-50 border'}`}>
                {tab === 'standings' ? '🏆 Standings' : tab === 'hr' ? '💣 HR Leaders' : tab === 'avg' ? '🎯 AVG Leaders' : tab === 'obp' ? '🧤 OBP Leaders' : '📈 RBI Leaders'}
              </button>
            ))}
          </div>
          {seasonTab === 'standings' && (
          ...
          {seasonTab === 'obp' && (
            <div className="bg-white rounded-2xl shadow-xl overflow-hidden border border-gray-200">
              <table className="w-full text-left">
                <thead><tr className="bg-teal-800 text-white text-xs font-bold uppercase tracking-wider">
                  <th className="p-3 w-8">#</th><th className="p-3">Player</th><th className="p-3">Team</th><th className="p-3 text-center font-black">OBP</th><th className="p-3 text-center">PA</th><th className="p-3 text-center">H</th><th className="p-3 text-center">BB</th><th className="p-3 text-center">AVG</th>
                </tr></thead>
                <tbody>{obpLeaders.map((p, i) => (
                  <tr key={`${p.name}-${p.team}`} className={`border-b border-gray-100 ${i === 0 ? 'bg-yellow-50' : 'hover:bg-gray-50'}`}>
                    <td className="p-3 font-black text-gray-400">{i + 1}</td>
                    <td className="p-3 font-bold text-gray-900">{i === 0 && '👑 '}{p.name}</td>
                    <td className="p-3 text-gray-500 text-sm">{p.team}</td>
                    <td className="p-3 text-center font-black text-teal-700 text-lg font-mono">{p.obp.toFixed(3).replace(/^0/, '')}</td>
                    <td className="p-3 text-center text-gray-600">{p.pa}</td>
                    <td className="p-3 text-center text-gray-600">{p.h}</td>
                    <td className="p-3 text-center text-gray-600">{p.bb}</td>
                    <td className="p-3 text-center font-mono text-gray-600">{p.avg.toFixed(3).replace(/^0/, '')}</td>
                  </tr>
                ))}</tbody>
              </table>
            </div>
  ...
  return (
    <div className="min-h-screen bg-stone-100 p-2 font-sans select-none overflow-x-hidden text-gray-900 text-[10px]">
      ...
      <div className="max-w-[750px] mx-auto flex items-center justify-center gap-6 mb-2 text-blue-900 font-bold uppercase tracking-tight">
        ...
        {/* Check-swing appeal badge (Backlog #150) — 2-strike take appealed to the base umpire */}
        {checkSwingBadge && (
          checkSwingBadge.ruledSwing ? (
            <span className="animate-pulse px-2 py-0.5 rounded-full text-[10px] font-black uppercase tracking-wider shadow-sm bg-gradient-to-r from-slate-600 to-amber-500 text-white border border-slate-700" title={`${checkSwingBadge.batter} checked his swing on a 2-strike count — the catcher appealed and the base umpire ruled he went around. Strike three!`}>
              ⚖️ HE WENT AROUND — K!
            </span>
          ) : (
            <span className="animate-pulse px-2 py-0.5 rounded-full text-[10px] font-black uppercase tracking-wider shadow-sm bg-gradient-to-r from-slate-500 to-emerald-500 text-white border border-slate-700" title={`${checkSwingBadge.batter} checked his swing on a 2-strike count — the appeal goes to the base umpire, who rules no swing. Ball.`}>
              ⚖️ CHECK SWING — NO GO
            </span>
          )
        )}
        {/* Pitchout badge (Backlog #155) — flashes when the defense calls a
            pitchout in a steal threat situation. Pulsing red-to-amber pill. */}
        {pitchoutBadge && (
          <span className="animate-pulse px-2 py-0.5 rounded-full text-[10px] font-black uppercase tracking-wider shadow-sm bg-gradient-to-r from-rose-600 to-amber-500 text-white border border-rose-700" title="Pitchout! The defense called for a wide pitch — the catcher stood up to throw out any runner who broke. Counts as a ball.">
            🎯 PITCHOUT
          </span>
        )}
        {/* Wind conditions chip (Backlog #152) — persistent, non-pulsing chip
            shown for the whole game on windy days. The CF flag mirrors it. */}
        {wind && (
          <span className={`px-2 py-0.5 rounded-full text-[10px] font-black uppercase tracking-wider shadow-sm border ${wind === 'out' ? 'bg-sky-100 text-sky-800 border-sky-300' : 'bg-slate-200 text-slate-700 border-slate-400'}`} title={wind === 'out' ? 'The flags are pointing straight out to center — the wind is blowing OUT today. Deep fly balls may carry over the wall.' : 'A stiff breeze is blowing IN from center field — deep drives can get knocked down at the warning track.'}>
            💨 WIND: {wind === 'out' ? 'BLOWING OUT' : 'BLOWING IN'}
          </span>
        )}
        {/* Wind-affected play badge (Backlog #152) — a fly ball carried out by
            the gust, or a would-be homer knocked down at the track. */}
        {windBadge && (
          windBadge.type === 'aided' ? (
            <span className="animate-pulse px-2 py-0.5 rounded-full text-[10px] font-black uppercase tracking-wider shadow-sm bg-gradient-to-r from-sky-500 to-emerald-500 text-white border border-sky-700" title="The wind grabbed a routine deep fly and carried it over the wall — a wind-aided home run!">
              💨 WIND-AIDED — GONE!
            </span>
          ) : (
            <span className="animate-pulse px-2 py-0.5 rounded-full text-[10px] font-black uppercase tracking-wider shadow-sm bg-gradient-to-r from-slate-600 to-sky-500 text-white border border-slate-700" title="That ball was GONE on a calm day — but the wind blowing in knocked it down and it died at the warning track.">
        ...
        {/* Cap flies off badge (Backlog #158) — pure cosmetic flash that fires
            when an outfielder's cap pops off during a web gem or robbed-HR catch.
            Visually one of baseball's most universally recognized "great catch"
            moments. */}
        {capOffBadge && (
          <span className="animate-pulse px-2 py-0.5 rounded-full text-[10px] font-black uppercase tracking-wider shadow-sm bg-gradient-to-r from-slate-500 to-stone-600 text-white border border-slate-700" title={`The fielder's cap went flying on that spectacular catch — every fan recognizes the moment.`}>
            🧢 CAP OFF!
          </span>
        )}
        {/* Stretched single → double badge (Backlog #159) — batter rounded first
            hard on a clean single with bases empty and reached second standing up.
            Pure flavor / badge layer; the engine has already re-mapped oc to a
            double pre-CPR so all hit / RBI / stat logic flows through the
            standard double pathway. */}
        {stretchedDoubleBadge && (
          <span className="animate-pulse px-2 py-0.5 rounded-full text-[10px] font-black uppercase tracking-wider shadow-sm bg-gradient-to-r from-emerald-500 to-amber-500 text-white border border-emerald-700" title={`${stretchedDoubleBadge.batter} rounded first hard on what looked like a routine single — and STRETCHED it into a double! Hustle play.`}>
            🏃 STRETCHED DOUBLE!
          </span>
        )}
        {/* Line-drive double play badge (Backlog #161) — the fielder snared the
            line drive and threw to the bag to double off the runner who was
            off too far. One of baseball's most exciting defensive plays. */}
        {lineOutDPBadge && (
          <span className="animate-pulse px-2 py-0.5 rounded-full text-[10px] font-black uppercase tracking-wider shadow-sm bg-gradient-to-r from-emerald-600 to-rose-600 text-white border border-emerald-800" title={`${lineOutDPBadge.runner} was off the bag and got DOUBLED OFF at ${lineOutDPBadge.base === 'B1' ? 'first' : 'second'} when the fielder snared ${lineOutDPBadge.batter}'s line drive!`}>
            💥 DOUBLED OFF AT {lineOutDPBadge.base === 'B1' ? '1ST' : '2ND'}!
          </span>
        )}
        ...
        {/* Foul ball off the foot badge (Backlog #162) — batter fouled a pitch
            directly off his foot/shin. He hops, winces, leans on the bat for
            support, and shakes it off. Still counts as a strike per real MLB
            rules — pure visual + narration + pacing flavor on top of the
            standard foul-ball outcome. */}
        {offFootBadge && (
          <span className="animate-pulse px-2 py-0.5 rounded-full text-[10px] font-black uppercase tracking-wider shadow-sm bg-gradient-to-r from-amber-500 to-rose-600 text-white border border-rose-800" title={`${offFootBadge.batter} fouled the pitch directly off his foot/shin. He's hopping in pain, leaning on the bat for support... but he's okay. Still counts as a strike.`}>
            🦶 OFF THE FOOT!
          </span>
        )}
        {/* Pitcher backs up home plate badge (Backlog #163) — on outfield-assist
            gun-downs at home, the pitcher sprints from the mound to a position
            behind the catcher to back up the relay throw. Pure fundamental
            defensive play that every fan recognizes. */}
        {pitcherHomeBackupBadge && (
          <span className="animate-pulse px-2 py-0.5 rounded-full text-[10px] font-black uppercase tracking-wider shadow-sm bg-gradient-to-r from-blue-600 to-slate-700 text-white border border-blue-800" title={`Pitcher hustles from the mound to back up the catcher on the throw home — fundamental team defense. If the throw sails wide, he's there to keep the runner from advancing.`}>
        ...
        {/* Foul-territory sacrifice fly badge (Backlog #156) — the catch in
            foul ground is deep enough for the runner on 3B to tag up and
            score after the catch. Mechanically identical to a regular sac
            fly: batter is out, runner scores. */}
        {foulSacFlyBadge && (
          <span className="animate-pulse px-2 py-0.5 rounded-full text-[10px] font-black uppercase tracking-wider shadow-sm bg-gradient-to-r from-amber-500 to-emerald-600 text-white border border-emerald-700" title={`Foul-territory sac fly — ${foulSacFlyBadge.batter} popped one up down the line, ${foulSacFlyBadge.fielder} squeezed it, and ${foulSacFlyBadge.runner} tagged from third to score. Real MLB rule: a runner may tag up on any caught fly, fair or foul.`}>
            🎯 FOUL SAC FLY
          </span>
        )}
        ...
        {/* Rubber-kick badge (Backlog #157) — pre-pitch atmospheric flavor:
            the pitcher scrapes his back foot along the rubber to groom the
            dirt before delivering. Pure pre-pitch ritual flavor — no gameplay
            effect. Companion to the brow wipe, shake-off, check-the-runner,
            and the rest of the mound-side pre-pitch atmospheres. */}
        {rubberKickBadge && (
          <span className="animate-pulse px-2 py-0.5 rounded-full text-[10px] font-black uppercase tracking-wider shadow-sm bg-gradient-to-r from-slate-500 to-stone-600 text-white border border-stone-700" title="Pitcher scrapes his back foot along the rubber to groom the dirt — one of baseball's most universal between-pitch rituals.">
            👟 KICKING THE RUBBER
          </span>
        )}
        ...
        {!gs.current.gameOver && gs.current.inning >= 1 && (() => {
          ...
          const homeColorBg = '#1e40af'; // home = blue (Backlog #169 — matches the field)
          const awayColorBg = '#b91c1c'; // away = red
          ...
          return (
            <span className="inline-flex items-center gap-1.5 px-2 py-0.5 rounded-full text-[10px] font-black uppercase tracking-wider shadow-sm bg-slate-100 text-slate-700 border border-slate-300"
              ...
              <span className={`font-black ${advantageHome ? 'text-blue-800' : 'text-red-700'}`}>{advLabel}</span>
            </span>
          );
        })()}
      </div>
          ...
          <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/60" onClick={() => setShowPostGame(false)}>
            <div className="bg-white rounded-2xl shadow-2xl max-w-3xl w-full mx-4 max-h-[90vh] overflow-hidden flex flex-col" onClick={e => e.stopPropagation()} style={{ animation: 'fadeInUp 0.4s ease-out' }}>
              ...
              <div className="overflow-y-auto p-4">
                ...
                {/* Left on base (LOB) — runners stranded per team, classic box-score footer line */}
                <div className="mt-1 pt-2 border-t border-gray-100 text-[11px] text-gray-600">
                  <span className="font-black uppercase tracking-widest text-gray-500 mr-2">LOB</span>
                  <span className="font-bold text-gray-800">{teams[0].name}</span> {gs.current.lob ? gs.current.lob[0] : 0}
                  <span className="mx-2 text-gray-300">·</span>
                  <span className="font-bold text-gray-800">{teams[1].name}</span> {gs.current.lob ? gs.current.lob[1] : 0}
                </div>
              </div>
